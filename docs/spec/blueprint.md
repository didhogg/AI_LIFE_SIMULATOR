...
- **已拍板（6.76·玩家代理 · 叙事控制 · 观战/磕CP 四件套·6.75 代写/视角/观战三件深化）**：两桶归位。**A 桶（写法·自由文本·进 `_叙事设置.叙事偏好`/文风库·引擎永不解析）**：`代写范围`（门开后代到哪——纯动作/含对话/含心理）、`转述行为`（user 已说的怎么呈现——融入叙事/零复读直接承接）= 文风开关面板可开关片段；数量约束（如「3-4 个动作」）= 纯 A 桶默认值、引擎不管。**B 桶（机械结构）**：① `_叙事设置.叙事权限.玩家角色决策权限`（玩家独占｜模型可代写·需确认｜模型可代写·自动·**细化 6.75 写权限为三档**）——只管「模型能不能产出代主角的、有后果、要落账的行动」；② `_叙事设置.人称{ 视角宿主（实体引用·主角｜某char/NPC｜全知旁白）, 人称（一｜二｜三·补回第二人称「你」）, 视角锁定（锁定单一宿主｜可切换出场角色） }`（人称二元组解耦·N3；机械校验：全知×第一人称=非法、一/二人称必须有单一宿主）；③ `系统.功能开关表` 加 观战模式(bool) + `观战推进模式`（手动步进｜自动连播｜快播到事件·观战节拍器）；④ `落账记录.actor_source` 扩第四值（玩家｜玩家确认｜模型代写｜**NPC自主/系统驱动**——前三=主角决策来源戳·走写权门，第四=观战推进别人·不碰写权门）；⑤ `观战内容入主角认知?` = 派生量（在场机械判定·不存储·在场入知情名单/不在场绝不入）。**机制**：①决策门(B)管「能不能代」、代写范围(A)管「代到哪」·落账盖 actor_source；**反应/感官描写（写「你脸红了」）不算决策·不进门·不需 provenance**，与代决策（写「你拔剑砍他」）根本区别；②抢话（管 user 没说的·留白/指令→模型生成）vs 转述（管 user 已说的·融入/承接）作用域不重叠，路由由 user 本拍输入性质定（留白/元指令→抢话·完整RP→转述）；③视角宿主×人称解耦 → 主角+第一=「我」=玩家（男性向代入）/ char+第一=「我」=角色（女性向），社区预设把宿主人称焊死给不了「主角本位第一人称」，解耦全覆盖；④观战节拍器：玩家不出手→推进交 观战推进模式；⑤观战 actor_source=NPC自主·引擎驱动CP双方·主角留白（不碰写权门），事件四源 = 离场演化(6.66)/触发扫描(G-4)/关系驱动/涟漪(E4)；⑥**观战所见≠主角认知（防穿帮·最关键·6.62 真相与认知分层）**：主角不在场→所见绝不写主角知情名单（否则切回接管凭空知道），在场旁观→真目击入名单；⑦观战是**正剧**（真落账有后果·区别小剧场零写回）·走正常二审·快播/连播段二审降级机械层或抽样(N1)；⑧占位符两路：`<user>`/`<char>`=实体名占位·引擎统一解析填名 / `lastUserMessage` 类宿主宏=引擎组装器在 Y6 无历史模式注入当前切片·禁酒馆楼层直塞（旁路组装确定性律）。**bug 清单（按严重度·详规随条目·全部任务化归位见 P0-5 现阶段全量任务清单 6.76 行）**：🔴 G1 切换走指令组边界（抢话/视角/观战/二审等元层开关不许拍中途生效·排队组边界·当前拍旧档跑完·合并 6.75 B1/B5→P0-4 F1 同款）；G2 切换 forward-only（只对未来拍生效·绝不追溯改已落账·撤销走悔棋/读档）；G3 代写候选先过二审才落账·二审重写只改措辞不改落账事实（动词/数额/目标锁定）；G4 强同步审仅限代主角/需确认拍·NPC自主/观战拍先落账二审异步纠偏（防双LLM串联延迟雪崩·收窄 G3）；G5 二审语言边界（每维度标「语言无关」·机械维度照跑·语义维度对不可解析语言降级「未评估」不误退）；G6 小剧场盐隔离（独立盐源·不递增主回滚计数器·不进 tick_log·指纹排除≠盐隔离两条都要）；G7 快播=照常结算只不呈现（不跳时间·离场演化照跑·被略拍盐值照常登记 tick_log）；G8 进观战强制清主角代写权限（观战≠抢话·防幽灵主角脏数据）；G9 二审降级绑「玩家是否在看这拍」非绑观战模式（手动/连播保留语义审·仅快播降级）；G10 单拍物理审（LLM·零持久·查单拍内部）vs 舞台几何校验（机械闸·读持久舞台状态·查跨拍空间矛盾）分工、非同一维度；G11 actor_source 扩写vs新决策（决策归玩家·执行细节算描写不单独落账·模型自主新增有后果行动才标模型代写+升需确认）。🟠 M1 路由保守偏转述（模棱两可不抢话）；M2 代写=补充不重复（扣掉本拍已声明行动·防复述×半抢话写两遍）；M3 swipe 读锁定档快照（不读现档·改档算新组）；M4 需确认档悬空候选切档丢弃+确认时复校档位仍允许；M5 观战后果可见因果不可见（涟漪客观后果落账可察·私密因果链不入主角认知）；M6 二审读生效文风许可范围（明确开 NSFW 则性别描写维度让位·不否决明确文风意图）；M7 抢话×视角耦合（代写主角内心需宿主=主角或全知·否则降级只写外部言行）；M8 收藏自包含快照（存完整 html/md·不存键引用·防删键悬空）；M9 视角随媒介出块恢复（进出块恢复族·媒介专属视角=临时覆盖）；M10 防剧透层⊥主角认知（读者可见性 vs 角色认知两套独立）。🟡 L1 抢话禁×转述禁=合法（硬核自写·留白则该拍主角无行动只推NPC·给提示不静默断裂）；L2 视角宿主中途失效回退链（死亡/离场→可切换档回退在场角色或全知·锁定档强制提示重选）；L3 导入闸校验人称二元组合法性（全知×第一人称等非法并入导入闸+警示族）；L4 观战连播冲击快照稀疏化（大段连播折叠里程碑·悔棋以观战段为单元粗回滚）；L5 二审结果+维度库增删改全进指纹排除（重放不重跑二审·合并 B15/B-h）；L6 三党预设覆盖手改文本给提示；L7 小剧场可读舞台快照零写回；L8 高退回率维度提示（产品层）。🟢 N2 数量约束=纯 A 桶自由文本引擎不管。**既有红线复申**：文风库禁加「适用场景」字段；语言禁枚举·引擎永不解析语言；二审维度禁固定枚举（开放库）；小剧场收藏永不回喂+指纹排除+AI不可见；小剧场 HTML 沙箱渲染(iframe+CSP+净化)；小剧场旁路正文文风键与二审（独立输出零写回）。**落点**：B 桶可空/改动 schema（4.1 `_叙事设置.叙事权限` 改三档决策权限 + 人称加 视角锁定 + 功能开关表加 观战推进模式 / 4.8 `落账记录.actor_source` 加第四值 / `观战内容入主角认知?`=派生不存储）→ **P0-1 黄金窗口批**（细化 6.75·零迁移）；G1 切换组边界→P0-4(F1)；G6 小剧场盐隔离→P0-5；G3/G4/G7/G10/G11/M5 落账与结算管线→P0-7；G5/G9/M1/M2/M6/M7/M9/M10 二审与组装→P0-8；L3 人称二元组导入校验→P0-6；L4 连播快照折叠→P0-9；代写/转述/视角/观战节拍器/防穿帮 + 前端三党面板→P1。
- **已拍板（6.76 续·叙事控制簇硬化：P1–P12 真盲区 / MP1–MP9 多人 / SV1–SV8 持久层 / IM·PF 注入与 mod 信任 / O1–O5 多人隐私 ＋「事实/呈现两分」宪法）**：6.76 四件套从单人推到多人/存档/mod 后的硬化批次，全部为既有零件重接线、零新积木。**P 真盲区**：🔴P1 自动代写设**重大不可逆决策安全地板**——死亡/婚姻/血脉绑定/永久失去核心资产等归一类标记，无论抢话档多开一律强制降级「需确认」（凌驾抢话档之上的安全闸·类比 G7 死亡拦截引擎硬顶·不写=自动代写走火枪）；🔴P2 多人单叙事流×per-seat 视角/抢话矛盾（须拍）= 叙事按席位分别渲染（每席位一份投影·复用 C5 per-seat 投影快照·结算同源呈现分流）或 视角/人称降房间级共识（房主定·个人只调抢话）；🔴P3 叙事控制簇（文风/视角/人称/抢话档/转述/二审）整体进指纹排除（叙事层⊥判定层·B1d 叙事面同族）＋「排除员变→指纹不变」断言·玩家拨动只落偏好/演出层永不进盐（G6 小剧场盐隔离推广到整簇）；🟠P4 观战快播期主角自己的到期事件（约定库到期/D4 种子成熟）走离场演化自动结算（同 6.66·主角临时变离场实体）·结果按 M5「后果可见因果按在场分流」呈现；🟠P5 二审重写不改 actor_source、不单独落账不进谱系（措辞层二次加工·呈现非事实·二审⊄单写者链）；🟠P6 二审 retry 预算绑世代——swipe(+1世代)给新预算、同世代内自动退回仍≤1（区分玩家主动 swipe vs 二审自动退回·防狂 swipe 绕过退回≤1）；🟠P7 转述/代写注入前做**人称坐标变换**（玩家「我」→主角实体键→按当前视角宿主转人称·主角在 char 视角下变「他/对方/你」·X2/X9 占位解析含人称变换非只填名）；🟡P8 嵌套媒介（信中信/戏中戏/回忆中回忆）的文风/视角出块恢复用**栈**（进块 push 出块 pop·限最大嵌套深度）非单层覆盖；🟡P9 小剧场实例化瞬间 pin 舞台快照（snapshot-on-instantiate·同 K3·整个生命周期读 pin 不读实时舞台·防生成期正文推进致快照过期）；🟡P10 小剧场 meta 知识泄露接 6.45 玩家 meta 知识严格模式（严格模式下小剧场强制「只基于主角已知生成脑洞」·默认模式接受泄露·脑内不设防同 G9c 裸SL 口径）；🟡P11 反玛丽苏维度×故意爽文设定归 M6 统一口径（二审维度尊重生效文风/预设许可范围·作者明确开爽文则反玛丽苏让位）；🟡P12 二审严格度=0（跑但全维度静默通过·一票不退）≠ 维度全关（不跑·省调用）明文（性能与可观测性不同·禁实装者当等价）。**MP 多人**：🔴MP1 代写必须在意图屏障关闭后·抢话席位先交「授权代写」占位意图·禁屏障前抢跑（否则「回应未发生动作」因果错乱）；🔴MP2 多人观战快播=per-seat 呈现回放速度·不碰世界钟·真快播仅单人/全员观战（否则时间线撕裂/锁死）；🔴MP3 视角⊥知情·可见性永远用观看者自己的知情名单·视角宿主不得借用他人主角知情权（安全级提权洞·不写=多人偷看秘密后门）；🟠MP4 二审维度分呈现层（可 per-seat）vs 事实层（全局唯一）·per-seat 只跑呈现层；🟠MP5 掉线托管=显式例外通道（actor_source=AI托管·强保守·重大决策冻结·临时覆盖零抢话）；🟠MP6 多人 swipe 默认只重渲自己呈现·重掷共享事实走房主权/投票；🟠MP7 互斥意图由 C2 仲裁定序·败方重新语境化不改语义·物理不兼容则意图失败反馈；🟡MP8 per-seat 投影分流=元泄露·给房间「交集知情(零泄露)/per-seat(自律)」开关·默认后者；🟡MP9 小剧场=per-seat 私有旁路·不进共享叙事流。**SV 持久层**：🔴SV2 悔棋/读档还原=事实精确·呈现尽力（里程碑含渲染文本=精确还原·中间拍重放=呈现近似·重放逐位一致只承诺事实/指纹不承诺呈现文本）；🟠SV1 叙事控制设置只随时间前进不随悔棋回退·历史呈现 frozen 冻结；🟠SV3 历史拍呈现存 frozen 文本快照·禁懒渲染依赖活文风库（迁移删键也不炸）；🟠SV4 重生成=事实读 frozen + 呈现用当前设置（改视角后重生成用新视角）；🟠SV7 观战所见是 meta 知识新入口·接 6.45 严格模式与裸SL 预知同等对待；🟡SV5 叙事控制设置随 fork 复制不共享·新玩家重置默认；🟡SV6 小剧场收藏账户级全局·在状态树之外·不随 fork/悔棋；🟢SV8 二审 retry 预算绑世代·悔棋=新世代给新预算（P6 已覆盖）。**IM 注入/PF 性能/信任**：🔴IM1 二审审稿提示词=注入面（数据化喂入+输出严格 schema+无写权沙箱+重写再过机械校验+导入注入扫描）；🔴IM2 写权限/抢话档入「玩家主权不可被 mod 锁定」清单·P1 安全地板凌驾预设不可关；🔴IM5 mod 二审维度默认全局+只标记不重写·重写=引擎内置维度特权；🟠IM3 文风/维度/剧本键全入受治理键空间+pack_id 命名空间化+归并表仲裁·禁裸键（S 批同族）；🟠IM4 HTML 净化在渲染时+分享/导出时都跑·收藏内容视为不可信数据；🟠IM6 二审旋钮/维度预设设默认玩家可覆盖·无 mod 不可关维度（除引擎安全级）；🟠PF2 二审超时 fail-open 放行但代写仍过机械校验+标记未质审（机械闸硬过·质量层软降级）；🟡PF1 叙事控制簇 LLM 调用（代写/二审/per-seat 渲染）纳入每拍调用预算闸；🟡PF3 视角宿主解析失败=回退全知+警示；🟢PF4 退化输入缺省（空文风=中性默认·非法抢话档=回退最保守「玩家独占」·fail-safe 向最小授权）。**O 多人隐私真洞**：🔴O1 多人下小剧场/观战快照默认「上帝视角」会泄露他人玩家隐私——快照知识范围必须按开启席位自己的知情名单过滤·不能默认全局真相（防剧透层默认关=上帝视角只在单人安全·多人默认降为席位知情过滤·同 MP3 小剧场版·隐私级）；🟠O2 小剧场「知识范围」是独立于「内容尺度」的正交维度（尺度管多露骨·范围管基于多少真相）·默认跟随防剧透层（单人上帝/多人席位过滤）·严格模式(6.45)限「仅主角已知」（防刷预知·接 SV7）；🟠O3 小剧场提示词组装也走引擎组装器（Y6 无历史模式·占位走 X2/X9·历史按剧本经组装器切片注入递归限深1·绝不直读宿主楼层·旁路的是文风键与二审不是组装确定性律）；🟡O4 可交互「选项」=事实派生+per-seat 知情过滤呈现（能看到哪些选项=per-seat 呈现·选了会发生什么=全局事实·选中走屏障/仲裁·归空间宪法）；🟢O5 小剧场 LLM 拒答/超时=直接提示玩家重试（零写回无副作用·fail-open 兜底）。**「事实/呈现两分」宪法（三条原则·叙事控制簇总闸）**：① **空间（多人）**：结算事实全局唯一、呈现 per-seat 分流——文风/视角/文笔二审/swipe 重渲在呈现层人各一份·凡碰落账事实的（事实层二审/重掷事实/代写决策/知情真相）必须全局唯一+走屏障/仲裁/协商（建议与 C5 per-seat 投影同处实装）；② **时间（存档）**：事实可精确重放、呈现 frozen 快照还原+允许漂移（E2 事实不漂措辞可漂·历史呈现靠 frozen 文本快照不靠重渲染·当前叙事控制设置只随时间前进不随回滚）；③ **信任（mod）**：mod/导入预设对叙事控制簇只能「加内容」（文风/检测项/剧本）·不能「夺主权/拿改写权/关安全闸」——玩家主权（写权限）、引擎安全级维度（注入/净化）、安全地板（不可逆决策需确认）三者凌驾任何预设之上·mod 二审维度默认全局+只读标记·所有 mod 输入过命名空间治理+注入扫描+净化。**落点**：P1 安全地板/P3 簇进指纹排除/P5 二审⊄单写者链/P6 retry 绑世代→P0-5＋P0-7；P2 多人矛盾/MP1–MP9/O1/O4→P2 多人（接口守则与 C5 同处冻结·MP3 视角⊥知情 P0 schema 注释先钉）；SV1–SV8 持久层→P0-9；IM1–IM6/PF→P0-6 导入闸＋P0-8 二审组装＋P1 性能调平；O2/O3/O5 小剧场→P0-8；三宪法写进决议设计原则。**大白话**：这一批是把「让 AI 替你演主角、看别人谈恋爱（磕 CP）」从一个人玩、推到多人联机 + 存档读档 + 别人做的 mod 三种场景下会撞到的坑一次性补完。最要命三条——①AI 自动替你做决定时遇到「会死/结婚/卖身」这种不可逆大事必须停下来问你（P1）；②多人时别人能不能「看到」某段剧情永远按他自己角色知道多少来过滤、不能借你的角色偷看秘密（MP3/O1）；③一条总规矩：账要算得全世界一样、故事可以每个人看到的版本不同（事实/呈现两分宪法）。
