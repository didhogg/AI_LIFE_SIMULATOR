...
export type NarrativePerson = 'second' | 'first' | 'third'
export type NarrativeStyle  = 'guofeng' | 'baihua' | 'jianjie'

export const PERSON_DEFAULT: NarrativePerson = 'second'
export const STYLE_DEFAULT:  NarrativeStyle  = 'guofeng'

export const STYLE_LABELS: Record<NarrativeStyle, string> = {
  guofeng: '古风（默认）',
  baihua:  '白话直叙',
  jianjie: '简洁电报体',
}
...
/** 未知人称值安全回落默认·不抛错 */
export function sanitizePerson(v: string): NarrativePerson {
  if (v === 'second' || v === 'first' || v === 'third') return v
  return PERSON_DEFAULT
}

/** 未知文风值安全回落默认·不抛错 */
export function sanitizeStyle(v: string): NarrativeStyle {
  if (v === 'guofeng' || v === 'baihua' || v === 'jianjie') return v
 ...
 */
export function buildScriptedNarrative(
  person: NarrativePerson,
  style: NarrativeStyle,
  pcName: string,
  optionId: string,
