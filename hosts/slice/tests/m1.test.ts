...
describe('Gate①: 结构校验', () => {
  ...
  it('非 JSON 字符串 → 降级不崩', () => {
    ...
    expect((res as { reason: string }).reason).toMatch(/JSON/);
  });
...
describe('Gate③: 对账覆盖性', () => {
  const makeProposal = (amounts: number[]) =>
    TickProposalSchema.parse({
  ...
  it('叙事有 2文，提案单为空 → 漏项', () => {
    ...
    if (!res.covered) expect(res.missing).toContain(2);
  });

  it('叙事有 5文钱，提案单只有 3 → 漏项', () => {
    const res = gateCoverage('给了5文钱', makeProposal([3]));
    expect(res.covered).toBe(false);
    if (!res.covered) expect(res.missing).toContain(5);
  });
...
describe('守恒断言', () => {
  it('发出 = 收到 → 断言通过', () => {
    expect(() =>
      assertConservation([{ before_from: 30, after_from: 28, before_to: 0, after_to: 2 }])
    ).not.toThrow();
  });

  it('发出 ≠ 收到 → 断言抛错', () => {
    expect(() =>
      assertConservation([{ before_from: 30, after_from: 27, before_to: 0, after_to: 2 }])
    ).toThrow(/守恒断言失败/);
  });
});
...
describe('拍 3 黄金路径: 林九给红姨 2 文小费', () => {
  ...
  it('覆盖性通过（叙事 2文 ↔ 提案 2）', () => {
    ...
    if (!gate1.ok) throw new Error(gate1.reason);
