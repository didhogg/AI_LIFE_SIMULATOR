...
import {
  runOutputGuard,
  OUTPUT_GUARD_ROLL_HINT,
} from '../engine/outputGuard.js';
...
describe('Batch3 ② 对账闸分级失败: 可解析歧义（金额漏项）→ 单次重试', () => {
  ...
  it('金额漏项·retryNarrative 仍错 → status=retried_failed+rollHint·degraded=true', () => {
    ...
    if (!cov.covered) expect(cov.degraded).toBe(true);
  });
});
...
describe('Batch3 ④ applySliceBudget 降级顺序（拍板③铁律）', () => {
  const LORE_PART: SlicePart   = { key: 'lore',      content: '这是一段很长的 lore 谓词载荷'.repeat(5) };
  const NEARK_PART: SlicePart  = { key: 'nearK',     content: ['叙事1', '叙事2', '叙事3', '叙事4'].join('\n') };
  const CHRON_PART: SlicePart  = { key: 'chronicle', content: '[序1] 林九：进了客栈' };
  const NPC_PART: SlicePart    = { key: 'npc',       content: '红姨（老板娘）' };
  const CORE_PART: SlicePart   = { key: 'core',      content: '核心信息' };
  ...
  it('超限·lore 降后仍超→nearK 减半（降级顺序第2位）', () => {
    const bigChron: SlicePart = { key: 'chronicle', content: '编年史'.repeat(20) };
    const fourLineNeark: SlicePart = { key: 'nearK', content: ['A','B','C','D'].join('\n') };
  ...
  it('超限·lore+nearK 降后仍超→chronicle 去除（降级顺序第3位）', () => {
    const bigLore: SlicePart  = { key: 'lore',      content: 'X'.repeat(5000) };
    const bigNeark: SlicePart = { key: 'nearK',     content: 'Y'.repeat(5000) };
    const chron: SlicePart    = { key: 'chronicle', content: 'Z'.repeat(5000) };
  ...
  it('nearK 最少保留1行（不完全去除）', () => {
    const oneLineNeark: SlicePart = { key: 'nearK', content: '唯一的历史拍叙事' };
...
describe('Batch3 ⑤ N-4 玩家主权断言（不自动重生·不抬预算）', () => {
  it('soft_rejected 时只返回 rollHint·不包含重试结果（调用方不得自动重试）', () => {
    ...
    expect((r as Record<string, unknown>)['retriedOnce']).toBeUndefined();
    expect((r as Record<string, unknown>)['newNarrative']).toBeUndefined();
  });
