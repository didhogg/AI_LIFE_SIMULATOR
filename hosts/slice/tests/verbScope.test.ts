...
// 批③ Step2 · content_hash ⊥ slice 实战记账/检定（编译期断言·slice 侧 3 target）
...
describe('批③ content_hash ⊥ slice 实战核心调用（编译期断言）', () => {
  it('slice 记账 Transfer 不含 content_hash 键', () => {
    type Assert = _Expect<_NoForbiddenKeys<Transfer, ContentHashForbiddenKey>>;
    const _: Assert = true; expect(_).toBe(true);
  });
  it('slice 记账 FlowRecord 不含 content_hash 键', () => {
    type Assert = _Expect<_NoForbiddenKeys<FlowRecord, ContentHashForbiddenKey>>;
    const _: Assert = true; expect(_).toBe(true);
  });
  it('slice 检定 CheckIntent 不含 content_hash 键', () => {
    type Assert = _Expect<_NoForbiddenKeys<CheckIntent, ContentHashForbiddenKey>>;
    const _: Assert = true; expect(_).toBe(true);
  });
});
