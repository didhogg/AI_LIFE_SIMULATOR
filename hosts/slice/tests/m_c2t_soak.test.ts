...
import {
  buildWorld, PC, NPC_WANG, NPC_HONG, EXPECTED_NET_ASSET,
} from '../fixture/world.js';

const BASE_ENVELOPE = 指令信封Schema.parse({ 提案: {} });

function relPack(actorKey: string, targetKey: string, Δ方向: number, 量级: number) {
  return [{
    标签: Δ方向 < 0 ? '冲突' : '义举',
...
describe('C2-T SK-2 · 拍3 死亡 → 悲恸情绪 + _编年史入册', () => {
  ...
  it('拍3死亡后守恒不破', () => {
    ...
    expect(() =>
      assertConservation(s4.货币系统!.账户!, EXPECTED_NET_ASSET, getNetAsset),
    ).not.toThrow();
  });
});
...
describe('C2-T SK-3 · 全程 20 拍长程 soak（含死亡+关系·全程守恒+schema+序号单调）', () => {
  ...
  it('20 拍双宿主恒等：两条独立 20 拍流水线 canonicalize 逐位恒等', () => {
    function run20() {
      let s = buildWorld();
      for (let i = 0; i < 20; i++) {
        if (i === 0) s.$涟漪候选[PC] = relPack(PC, NPC_WANG, -1, 100);
        if (i === 5) {
          ({ state: s } = runTick(s, {
            tickId: `sk3-dual-t${i}`,
            injectedEnvelope: BASE_ENVELOPE,
...
describe('C2-T SK-4 · SETTLEMENT_PHASES + 标准 50 拍不漂移验证', () => {
  it('SETTLEMENT_PHASES 含 17 个阶段（含 B2 LOD调度 + P8-a 成就解锁 + P9-2 扩展参数播种）', () => {
