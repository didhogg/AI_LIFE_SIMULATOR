...
import { dispatchTool, resolveToolEntry, checkCallConstraint, validateOutputTagNamespace, routeOutputTagViaGate, } from '@ai-life-sim/core/engine/toolExecutor';
...
describe('C1-3 · 调用约束谓词 gate', () => {
    it('约束不满足（a=0, constraint=a>0）→ dispatchTool ok=false', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toContain('调用约束不满足');
    });
