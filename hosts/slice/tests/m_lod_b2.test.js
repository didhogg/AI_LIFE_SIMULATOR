...
import { scheduleLodPhase, LOD_PROMOTE_BUDGET, } from '@ai-life-sim/core/engine/lodPhase';
import { detectLodTrigger } from '@ai-life-sim/core/engine/lodScheduler';
...
function makeStateWithLod(opts) {
    const { nodeKey, npcKey, pcKey, pcAt, lodAtInit = '粗', warmUntil, } = opts;
    const base = makeBase();
    // 地图地点
    base.地图 = {
        ...(base.地图 ?? {}),
    ...
    base.LOD表[npcKey] = { 模块键: npcKey, 档位: '粗' };
    // 席位表
    base._席位表 = {
        本机: { 焦点角色键: pcKey },
    };
...
describe('B2-6 · 三条件 - 跨区', () => {
    it('detectLodTrigger 跨区命中 triggered=true', () => {
        ...
        const prev = { locKey: 'region_a', orgKeys: [], epochMin: 0, eraLabel: '' };
        const cur = { locKey: 'region_b', orgKeys: [], epochMin: 0, eraLabel: '' };
    ...
    it('scheduleLodPhase prevLocCtxs 跨区 → 新区域 promote（handleRegionCross 接通）', () => {
        ...
        // LOD表 注册 region_b
        s.LOD表['region_b'] = { 模块键: 'region_b', 档位: '粗' };
        // 上一拍 PC 在 region_a
        const prevLocCtxs = new Map([
...
describe('B2-7 · 三条件 - 纪元跨时代', () => {
    it('detectLodTrigger 年号变更 triggered=true', () => {
        const s = makeBase();
        const prev = { locKey: 'region_a', orgKeys: [], epochMin: 0, eraLabel: '太平' };
        const cur = { locKey: 'region_a', orgKeys: [], epochMin: 100, eraLabel: '建中' };
        const result = detectLodTrigger(s, prev, cur);
        expect(result.triggered).toBe(true);
    ...
    it('scheduleLodPhase 纪元跨时代 → 当前区域 promote', () => {
        ...
        s.LOD表['region_a'] = { 模块键: 'region_a', 档位: '粗' };
...
describe('B2-8 · 三条件 - 组织归属变更', () => {
    it('detectLodTrigger 组织集合差异 triggered=true', () => {
        const s = makeBase();
        const prev = { locKey: 'x', orgKeys: ['org_a'], epochMin: 0, eraLabel: '' };
        const cur = { locKey: 'x', orgKeys: ['org_b'], epochMin: 0, eraLabel: '' };
        const result = detectLodTrigger(s, prev, cur);
        expect(result.triggered).toBe(true);
    ...
    it('组织不变 triggered=false', () => {
        const s = makeBase();
        const ctx = { locKey: 'x', orgKeys: ['org_a'], epochMin: 0, eraLabel: '' };
...
describe('B2-9 · tick 集成', () => {
    ...
    it("'LOD调度' 在 '关系触发' 之前", () => {
        const lodIdx = SETTLEMENT_PHASES.indexOf('LOD调度');
        const relIdx = SETTLEMENT_PHASES.indexOf('关系触发');
        expect(lodIdx).toBeLessThan(relIdx);
    });
    it("'LOD调度' 在 '标志触发' 之后", () => {
        const flagIdx = SETTLEMENT_PHASES.indexOf('标志触发');
        const lodIdx = SETTLEMENT_PHASES.indexOf('LOD调度');
