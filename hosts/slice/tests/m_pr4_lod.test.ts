...
describe('G5: 四条件检测器各自命中', () => {
  ...
  it('G5-1 ①跨区：位置变更且区域不同 → triggered=跨区', () => {
    const s = buildLodWorld();
    const cur: LodTriggerCtx = { ...basePrev, locKey: LOC_S1 };
  ...
  it('G5-2 ①同区内移动：同区域 → triggered=false', () => {
    const s = buildLodWorld();
    // LOC_N1 → REGION_NORTH 同区
    const cur: LodTriggerCtx = { ...basePrev, locKey: REGION_NORTH };
    const r = detectLodTrigger(s, basePrev, cur);
  ...
  it('G5-3 ②纪元跨时代：年号变更 → triggered=纪元跨时代', () => {
    const s = buildLodWorld();
    const cur: LodTriggerCtx = { ...basePrev, eraLabel: '天宝' };
    const r = detectLodTrigger(s, basePrev, cur);
    expect(r.triggered).toBe(true);
  ...
  it('G5-4 ②无历法（eraLabel 为空串）→ 不触发', () => {
    const s = buildLodWorld();
    const prev2: LodTriggerCtx = { ...basePrev, eraLabel: '开元' };
    const cur: LodTriggerCtx = { ...basePrev, eraLabel: '' };
  ...
  it('G5-5 ③组织归属变更（离组织）→ triggered=组织归属变更', () => {
    const s = buildLodWorld();
    const cur: LodTriggerCtx = { ...basePrev, orgKeys: [] };
    const r = detectLodTrigger(s, basePrev, cur);
    expect(r.triggered).toBe(true);
  ...
  it('G5-6 ③组织归属变更（加入组织）→ triggered=组织归属变更', () => {
    const s = buildLodWorld();
    const cur: LodTriggerCtx = { ...basePrev, orgKeys: ['org_a', 'org_b'] };
    const r = detectLodTrigger(s, basePrev, cur);
    expect(r.triggered).toBe(true);
    expect(r.condition).toBe('组织归属变更');
  });
...
describe('G6: 穿越 + 契约执行 + 跨域隔离', () => {
  ...
  it('G6-2 属性映射：旧轴名→新轴名', () => {
    const s = buildTravelWorld();
    expect((s.NPC['pc_hero']?.属性 as Record<string, number>)['体质']).toBe(20);
    executeTraversal(s, 'domain_mortal', 'domain_spirit', 'pc_hero', {
      属性映射: { 体质: '体力' }, // 体质 → 体力
      货币处理: '保留',
      技能等价表: {},
    ...
    const attrs = s.NPC['pc_hero']?.属性 as Record<string, number>;
  ...
  it('G6-10 不存在的 pcKey → no-op 不 throw', () => {
    const s = buildTravelWorld();
    expect(() =>
      executeTraversal(s, 'domain_mortal', 'domain_spirit', 'nonexistent_pc', {
        属性映射: {},
        货币处理: '保留',
        技能等价表: {},
        携带白名单: [],
...
describe('G7: soak 守恒 + 确定性 + 黄金向量', () => {
  it('G7-1 默认 fixture 无 LOD 活动：BUNDLE_MEMBERS 守恒（不新增）', () => {
    expect(FINGERPRINT_BUNDLE_MEMBERS).toHaveLength(21);
  });
  ...
  it('G7-2 buildWorld()（原始 fixture·无 LOD 区域）：runTick 10拍 守恒', () => {
    ...
    // 守恒：净资产不变
    assertConservation(s.货币系统.账户, EXPECTED_NET_ASSET, (a) =>
      getNetAsset(a as Parameters<typeof getNetAsset>[0]),
    );
  });
  ...
  it('G7-3 LOD world 300拍 promote/demote soak：货币守恒', () => {
    ...
    let cur = s;
    for (let i = 0; i < 300; i++) {
      cur = JSON.parse(JSON.stringify(cur)) as typeof cur;
      // 奇数拍：北→南跨区
      if (i % 10 === 0) {
