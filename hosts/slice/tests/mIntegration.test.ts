...
function makeStateWithAccounts(): RootState {
...
describe('M-a · P0-6 真件集成验证', () => {
  ...
  it('computeDelta 只算不写（add op·返回 proposedValue·原 state 零改）', () => {
    ...
    const result = computeDelta(
      state,
      { path: `货币系统.账户.${PC}.持有.文`, op: 'add', value: 50 },
    );
  ...
  it('mergeInterventionDeltas 取严合并（约束 clamp 取严·内容后载覆盖）', () => {
    const pack1 = [{ path: '货币系统.账户.pc_a.持有.文', op: 'add' as const, value: 100, max_delta: 80 }];
    const pack2 = [{ path: '货币系统.账户.pc_a.持有.文', op: 'add' as const, value: 100, max_delta: 120 }];
  ...
  it('runProposalGate reject：M2 越权授权源 → fail-closed·Gate③-M2', () => {
    ...
    const result = runProposalGate(
      ...
      state,
      'seat-local',
      '天命',  // ← 非法授权源（VALID_OVERWRITE_AUTH_SOURCES = ['系统','裁判','玩家确认']）
      [[{ path: `货币系统.账户.${PC}.持有.文`, op: 'add', value: 30 }]],
    );
