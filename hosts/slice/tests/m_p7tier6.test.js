...
// ── P7-6a 跨域结账 ────────────────────────────────────────────────────────────
import { makeTriTickKey, compareTriTickKeys, sortByTriTickKey, computeSupplementInterval, crossDomainOneShot, } from '@ai-life-sim/core/engine/crossDomain';
// ── P7-6b 结算序 ──────────────────────────────────────────────────────────────
import { SETTLEMENT_PHASES } from '@ai-life-sim/core/engine/tick';
// ── P7-6c 知情写入口 ──────────────────────────────────────────────────────────
import { KNOWLEDGE_CONSUMER_TYPES, KNOWLEDGE_CONSUMER_REGISTRY, KNOWLEDGE_ENTRY_CHANNELS, expandKnowledgeSelector, knowledgeWrite, } from '@ai-life-sim/core/engine/knowledgeWrite';
// ── P7-6d 指令组锚点 ──────────────────────────────────────────────────────────
import { pinGroupAnchor, assertGroupAnchorExists, verifyGroupReplayIdempotency, } from '@ai-life-sim/core/engine/groupAnchor';
// ── P7-6e Resolver 签名 ───────────────────────────────────────────────────────
import { CombatResolver } from '@ai-life-sim/core/interfaces/combatResolver';
// ── P7-6f 历法 ───────────────────────────────────────────────────────────────
import { isNaturalMonthBoundary, computeCalendarContinuation, nextMonthStart, gregorianToEpochMin, } from '@ai-life-sim/core/engine/time';
// ── P7-6g 断言② 依赖 ─────────────────────────────────────────────────────────
...
// ── P7-6h AA6 负向控制 ───────────────────────────────────────────────────────
import { fillEffectPackHash, computeEffectPackSetHash, } from '@ai-life-sim/core/engine/effectGate';
// ═══════════════════════════════════════════════════════════════════════════════
...
describe('P7-6a · 跨域同拍序键三元定序稳定', () => {
    ...
    it('相等键 → compareTriTickKeys = 0', () => {
        const key = { globalTick: 999, domainId: 'x', seedId: 'y' };
...
describe('P7-6e · Resolver 三段契约签名冻结', () => {
    ...
    it('CombatResolver.step 接受 ExternalRoundEvent[] 作为第三参（类型守卫）', () => {
        ...
        expect(() => CombatResolver.step({}, [], externalEvents)).toThrow('未实装');
    });
});
// ═══════════════════════════════════════════════════════════════════════════════
...
describe('P7-6h · AA6 负向控制：改 effect packs → 哈希变；改无关项 → 哈希不变', () => {
    ...
    it('AA6 负向：仅改非 effect 的 state 字段 → 已有 hash 不影响 effect 集哈希', () => {
        ...
        void _stateA;
        void _stateB; // only state changes, not packs
