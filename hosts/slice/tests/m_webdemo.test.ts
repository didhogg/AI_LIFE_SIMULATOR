...
import { filterMenuCandidates, type MenuFilterCandidate } from '../engine/menuFilter.js';
...
describe('D1 prompt-dump — 近K历史/live账本/NPC态/lore', () => {
  ...
  it('近K历史注入 userPrompt（narrativeHistory 有内容）', () => {
    ...
    expect(userPrompt).toContain('近期叙事');
  ...
  it('lore 谓词命中时注入 systemPrompt', () => {
    ...
    (stateWithLore as unknown as Record<string, unknown>)['_lore知识库'] = {
      lore_inn: {
        触发谓词: '',  // 无谓词 = 恒真
...
describe('D2 reconcileGate 分级失败', () => {
  ...
  it('金额漏项 + 重试叙事覆盖 → retried_covered', () => {
    ...
    const result = runReconcileGate(
      '林九付了三文钱买了酒菜。',   // 首次：三 ≠ 5
      proposal5,
      '林九取出五文钱买了酒菜。',   // 重试：五文 = 5文 ✓
    );
...
describe('D3 知情过滤 — POV 切换', () => {
  ...
  it('PC POV → filterSecretsForPOV 返回空集（不知情 S1）', () => {
    ...
    const visible = filterSecretsForPOV(secrets as Parameters<typeof filterSecretsForPOV>[0], PC);
    expect(Object.keys(visible).length).toBe(0);
  });

  it('NPC_WANG POV → filterSecretsForPOV 返回 S1（知情）', () => {
    const secrets = state.全局?.秘密库 ?? {};
    const visible = filterSecretsForPOV(secrets as Parameters<typeof filterSecretsForPOV>[0], NPC_WANG);
...
describe('D3 反人格标签 Anti-Labeling Directive', () => {
  ...
  it('systemPrompt 静态模板不含抽象性格形容（禁止词未被滥用为正向断言）', () => {
    ...
    const npcSection = lines.slice(
      lines.findIndex(l => l.includes('在场人物')),
      lines.findIndex(l => l.includes('## 地点') || l.includes('## 已知秘密') || l.includes('## 近期')) + 1
      || lines.length,
    );
...
describe('D3 NSFW 物理隔离', () => {
  ...
  it('指纹 manifest 总条目 = 84（demo 代码不进指纹）', () => {
    const total =
...
describe('D4 存档读写 — FullArchiveHeader', () => {
  ...
  it('JSON 往返恒等（序列化/反序列化保留所有字段）', () => {
    const h = createFullArchiveHeader(SAVE_SEED);
    const roundTripped = JSON.parse(JSON.stringify(h)) as FullArchiveHeader;
  ...
  it('[P0-9 侦察] 旧存档 MinArchiveHeader 缺少 RULE_VERSION（需 migrate 补全）', () => {
    ...
    const loaded = JSON.parse(oldSave) as MinArchiveHeader;
