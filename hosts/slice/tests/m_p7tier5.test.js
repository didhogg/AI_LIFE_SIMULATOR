...
import { scanDeathIntercept, DEATH_INTERCEPT_TRIGGER, } from '@ai-life-sim/core/engine/deathIntercept';
import { expandVerbTarget, applyVerbToTargets, VERB_TARGET_WILDCARD, VERB_TARGET_WILDCARD_ZH, } from '@ai-life-sim/core/engine/verbExpand';
import { fillEffectPackHash, isEffectPackHashStale, runEffectGates, computeEffectPackSetHash, } from '@ai-life-sim/core/engine/effectGate';
import { freezeImportanceScore, assertImportanceFrozen, readFrozenScore, } from '@ai-life-sim/core/engine/importanceFreeze';
import { checkSovereigntyFloor, autoFloorAuthorization, isFloorEvent, } from '@ai-life-sim/core/engine/sovereigntyFloor';
import { covenantCreate, covenantUpdate, covenantAppendTerm, covenantRelease, verbWriteToTargets, } from '@ai-life-sim/core/engine/covenantWrite';
// ── P7-5a · G7 死亡拦截器硬顶 ───────────────────────────────────────────────
...
describe('P7-5b · V3 展开器 runtime', () => {
    ...
    it('applyVerbToTargets：按展开序依次写入', () => {
        ...
        const log = [];
        applyVerbToTargets(targets, (acc, key) => { acc.push(key); return acc; }, log);
...
describe('P7-5c · H-c-4 五道闸钳制', () => {
    it('_前缀路径 → 拒绝（闸③）·ok=false', () => {
        const pack = { deltas: [{ path: '_内部状态', op: 'set', value: 100 }] };
    ...
    it('$前缀路径 → 拒绝（闸③）', () => {
        const pack = { deltas: [{ path: '$会话状态', op: 'set', value: 1 }] };
        const result = runEffectGates(pack);
        expect(result.ok).toBe(false);
    });
    it('白名单非空·path 不在白名单 → 拒绝（闸②）', () => {
        const pack = { deltas: [{ path: '角色.体力', op: 'add', value: 10 }] };
        const wl = new Set(['角色.魅力']);
        const result = runEffectGates(pack, wl);
        expect(result.ok).toBe(false);
        expect(result.errors[0]).toContain('白名单[②]');
    });
    it('max_delta=50 · value=100 → 截断到 50（闸④）', () => {
        const pack = { deltas: [{ path: '角色.体力', op: 'add', value: 100, max_delta: 50 }] };
        const result = runEffectGates(pack);
    ...
    it('max_delta=50 · value=30 → 通过·不截断', () => {
        const pack = { deltas: [{ path: '角色.体力', op: 'add', value: 30, max_delta: 50 }] };
        const result = runEffectGates(pack);
        expect(result.ok).toBe(true);
    ...
    it('ok=false → 调用方应原子拒绝整包（闸⑤契约验证）', () => {
        const pack = { deltas: [
                { path: '角色.体力', op: 'add', value: 10 }, // 正常
                { path: '_内部状态', op: 'set', value: 1 }, // 被拒
            ] };
        const result = runEffectGates(pack);
...
describe('P7-5f · V3 写入口 verbWriteToTargets', () => {
    ...
    it('V3 + 展开器集成：通配符 → 所有实体 Unicode 序写入', () => {
        ...
        const visited = [];
        verbWriteToTargets(expanded, (acc, k) => { acc.push(k); return acc; }, visited);
