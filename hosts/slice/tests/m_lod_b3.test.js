...
import { z } from 'zod';
import { clearLodRegistry, registerLodMount, getLodMount, dispatchLodGenerate, NPC_LOD_DESCRIPTOR, registerNpcLodMount, } from '@ai-life-sim/core/engine/lodMount';
...
describe('B3-2 · 通用骨架路径（无生成器 → 种子视图骨架 → S.parse 落桶）', () => {
    ...
    it('generic path: skeleton written via 写入目标', () => {
        ...
        s.LOD表[TARGET_NODE] = { 模块键: TARGET_NODE, 档位: '粗' };
    ...
    it('generic path: 索引器返空 → 写入目标 不被调用', () => {
        ...
        const s = makeBase();
        s.LOD表['any_node'] = { 模块键: 'any_node', 档位: '粗' };
    ...
    it('多 descriptor 各自独立分派', () => {
        ...
        registerLodMount({
            ...
            写入目标: (_s, _ek, data) => { results.push(data.x); },
        });
        registerLodMount({
            模块键: 'mod_b',
...
describe('B3-3 · S.parse 失败 fail-closed', () => {
    it('S.parse 失败 → 写入目标 不被调用·不抛', () => {
        ...
        const s = makeBase();
        s.LOD表['strict_node'] = { 模块键: 'strict_node', 档位: '粗' };
    ...
    it('自定义生成器返回非法值 → S.parse 失败 → 写入目标 不被调用', () => {
        ...
        const s = makeBase();
        s.LOD表['bg_node'] = { 模块键: 'bg_node', 档位: '粗' };
...
describe('B3-4 · 注册幂等', () => {
    it('同键后注册覆盖前者', () => {
        ...
        expect(result.success && result.data.v).toBe('B');
    });
...
describe('B3-6 · 自定义生成器路径（NPC descriptor·materializeCoarseNode delegate）', () => {
    ...
    it('promoteNode 后 NPC 已实体化 → 索引器返空 → dispatchLodGenerate no-op（幂等）', () => {
        ...
        s.LOD表['npc1'] = { 模块键: 'npc1', ...(s.LOD表['npc1'] ?? {}), 档位: '实体' };
        const keys = NPC_LOD_DESCRIPTOR.索引器(s, 'loc1');
...
describe('B3-8 · 三条件 跨区（state 快照 → detectLodTrigger → handleRegionCross）', () => {
    it('prev locKey 跨区 → handleRegionCross·新区域 promote', () => {
        ...
        s._席位表['s1'] = { 席位ID: 's1', 焦点角色键: 'pc1' };
        // LOD表 注册 loc_b
        s.LOD表['loc_b'] = { 模块键: 'loc_b', 档位: '粗' };
