...
import { filterSecretsForPOV, entityKnowsSecret, } from '@ai-life-sim/core/engine/knowledgeFilter';
import { assemblePrompt } from '../assemble.js';
import { buildWorld, PC, NPC_WANG, NPC_HONG, SECRET_S1 } from '../fixture/world.js';
// ── 测试夹具：秘密库 ────────────────────────────────────────────────────────────
function makeSecret(knowers) {
    return {
        母题: '藏匿通缉犯',
...
describe('M4 ③ assemblePrompt + visibleSecrets 注入', () => {
    ...
    it('王掌柜 POV → prompt 含秘密节', () => {
        const visible = filterSecretsForPOV((state.全局?.秘密库 ?? {}), NPC_WANG);
