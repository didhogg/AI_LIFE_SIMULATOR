...
//   PR0-4  manifest=85 / schemaKeys=53 守恒（C2-6 additive 不改任何数组）
//   PR0-5  双宿主快照恒等：C2-6 新字段全空跑·canonicalize 输出逐位不变
//   PR0-6  factFragment 种子条目：访问阈值/有锚布尔 默认值正确
import { describe, it, expect } from 'vitest';
import { RootSchema, BLUEPRINT_KEYS, 玩法预设Schema, 地图Schema, 全局Schema, } from '../../../packages/core/schema/index.js';
import { FINGERPRINT_BUNDLE_MEMBERS, FINGERPRINT_PRESET_FIELDS, FINGERPRINT_SNAPSHOT_FIELDS, FINGERPRINT_EXCLUDED_FIELDS, } from '../../../packages/core/engine/fingerprintManifest.js';
...
describe('PR0-2 · 新地图字段默认 undefined · 地图节点解析正常', () => {
    it('地点节点：新四字段解析均为 undefined', () => {
        ...
        const loc = m.地点['空节点'];
        expect(loc.区域映射).toBeUndefined();
        expect(loc.区域人口).toBeUndefined();
        expect(loc.人口密度).toBeUndefined();
        expect(loc.区域资源紧张度).toBeUndefined();
    });
    it('地图节点带新字段值 → 正确解析', () => {
        ...
        expect(loc.区域映射).toBe('华北大区');
        expect(loc.区域人口).toBe(500000);
        expect(loc.人口密度).toBe(120.5);
        expect(loc.区域资源紧张度).toBe(35);
    });
});
describe('PR0-3 · 全局._factFragment种子库 留位', () => {
    it('全局 Schema 解析：_factFragment种子库 默认 undefined', () => {
        const g = 全局Schema.parse({});
        expect(g._factFragment种子库).toBeUndefined();
    });
    it('factFragment 种子条目：访问阈值 default=0·有锚布尔 default=true', () => {
        ...
        const seed = g._factFragment种子库['test:关系:001'];
    ...
    it('无锚 factFragment（造谣）：有锚布尔 = false 合法', () => {
        ...
        const seed = g._factFragment种子库['fake:声誉:007'];
        expect(seed.有锚布尔).toBe(false);
        expect(seed.narrativeFrame).toBe('谣言：李商人欺诈顾客');
    });
});
describe('PR0-4 · manifest=88 / schemaKeys=54 守恒（G2-2: +媒介传播面 → BUNDLE21）', () => {
    ...
    it('EXCLUDED_FIELDS = 52（G2-2 新字段不进 EXCL）', () => {
