...
function makeMinimalState(opts = {}) {
    return RootSchema.parse({
        $存档种子: opts.seed ?? 0,
        NPC: Object.fromEntries(Object.entries(opts.npcs ?? {}).map(([k, v]) => [k, {
                姓名: k,
                位置: v.位置,
                存活状态: v.存活状态 ?? '在世',
                属性: v.属性 ?? {},
...
describe('G2-1 D2 · 多源达阈采纳（简单 vs 复杂传播二分）', () => {
    ...
    function makeMultiBridgeWorld(numBridges, label) {
        const npcs = {
            'src': { 位置: 'loc_scene' },
...
describe('G2-1 D3 · seeded 确定性', () => {
    ...
    function makeLowTrustWorld(seed) {
        const s = makeMinimalState({
            seed,
            npcs: {
                'src': { 位置: 'loc_a' },
