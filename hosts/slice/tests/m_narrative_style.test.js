...
//
// T1 切换 narrativeStyle → state diff 空 · 指纹88 · schemaKeys54 · 黄金向量不变
...
import { applyPersonStyle, buildScriptedNarrative, sanitizePerson, sanitizeStyle, PERSON_DEFAULT, STYLE_DEFAULT, STYLE_LABELS, PERSON_LABELS, } from '../../web-debug/narrativeStyle.js';
import { SnapshotStore, } from '../../web-debug/aohpDebugConsole2.js';
// ──────────────────────────────────────────────────────────────────────────────
// T1. 切换 narrativeStyle → 全恒等
// ──────────────────────────────────────────────────────────────────────────────
describe('T1 切换 narrativeStyle → state diff 空 · 指纹88 · schemaKeys54', () => {
    it('applyPersonStyle 对 state 完全只读（3 文风切换后 JSON.stringify 不变）', () => {
        const state = buildWorld();
        const before = JSON.stringify(state);
        const styles = ['guofeng', 'baihua', 'jianjie'];
    ...
    it('切换文风后 buildWorld() state 仍通过 RootSchema 校验（schemaKeys 52）', () => {
        ...
        expect(Object.keys(RootSchema._def.shape()).length).toBe(54);
    });
    it('切换文风后指纹字段总数恒等 84', () => {
    ...
    it('切换文风后同一 tickId 产出的 state 逐位恒等（黄金向量不受文风影响）', () => {
        ...
        applyStyle('guofeng');
        const r1 = runTick(structuredClone(buildWorld()), { tickId: 'style:t1:0' });
        applyStyle('baihua');
        const r2 = runTick(structuredClone(buildWorld()), { tickId: 'style:t1:0' });
        applyStyle('jianjie');
        const r3 = runTick(structuredClone(buildWorld()), { tickId: 'style:t1:0' });
...
// ──────────────────────────────────────────────────────────────────────────────
describe('T2 切换 narrativePerson → state diff 空 · 指纹88 · schemaKeys54', () => {
    it('applyPersonStyle 对 state 完全只读（3 人称切换后 JSON.stringify 不变）', () => {
        const state = buildWorld();
        const before = JSON.stringify(state);
        const persons = ['second', 'first', 'third'];
    ...
    it('切换人称后同一 tickId 产出的 state 逐位恒等（黄金向量不受人称影响）', () => {
        ...
        applyPerson('second');
        const r1 = runTick(structuredClone(buildWorld()), { tickId: 'person:t2:0' });
        applyPerson('first');
        const r2 = runTick(structuredClone(buildWorld()), { tickId: 'person:t2:0' });
        applyPerson('third');
        const r3 = runTick(structuredClone(buildWorld()), { tickId: 'person:t2:0' });
        expect(JSON.stringify(r1.state)).toBe(JSON.stringify(r2.state));
        expect(JSON.stringify(r2.state)).toBe(JSON.stringify(r3.state));
    });
});
...
describe('T4 $meta 渲染参数写入快照后重放 → state/指纹/黄金向量逐位恒等', () => {
    ...
    it('重放同 tickId 序列 state 逐位恒等（渲染参数不污染确定性）', () => {
        ...
        const afterA = runTick(structuredClone(base), { tickId: 'replay:t4:0' }).state;
        // 序列 B：person=third, style=jianjie
        applyPersonStyle('sys', 'third', 'jianjie');
        const afterB = runTick(structuredClone(base), { tickId: 'replay:t4:0' }).state;
        expect(JSON.stringify(afterA)).toBe(JSON.stringify(afterB));
    });
    it('重放后 schemaKeys 54 · 指纹88 不变', () => {
        const base = buildWorld();
        runTick(structuredClone(base), { tickId: 'replay:t4:1' });
        runTick(structuredClone(base), { tickId: 'replay:t4:1' });
        const allKeys = new Set([
            ...FINGERPRINT_BUNDLE_MEMBERS,
            ...FINGERPRINT_PRESET_FIELDS,
...
describe('T5 未知文风/人称值回落默认 · 不抛错', () => {
    ...
    it('applyPersonStyle 传入未知值不抛错（安全回落）', () => {
        expect(() => 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        applyPersonStyle('sys', 'bad', 'bad')).not.toThrow();
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const result = applyPersonStyle('sys', 'bad', 'bad');
        expect(result.length).toBeGreaterThan(0);
    });
    it('buildScriptedNarrative 传入未知值不抛错（安全回落）', () => {
        expect(() => 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        buildScriptedNarrative('bad', 'bad', '林九', '对话:wang')).not.toThrow();
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const result = buildScriptedNarrative('bad', 'bad', '林九', '对话:wang');
        expect(result.length).toBeGreaterThan(0);
    });
