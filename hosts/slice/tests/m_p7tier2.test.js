...
import { buildWorld, EXPECTED_NET_ASSET, PC, NPC_WANG, NPC_HONG, LOC_KEY, INITIAL_PC_BALANCE, INITIAL_WANG_BALANCE, } from '../fixture/world.js';
// ── helpers ──────────────────────────────────────────────────────────────────
...
describe('P0-7-T2 · ③ 结算序显式', () => {
    it('SETTLEMENT_PHASES 包含预期阶段（含 B2 LOD调度 + P7-6b 媒介拍末取材 + 提案落账 + C2-4 死亡感知发射 + C2-5 感知情绪化 + 编年史入册 + P8-a 成就解锁 + P9-2 扩展参数播种 = 17 个）', () => {
...
describe('P0-7-T2 · ⑥ 守恒不变量（Σ净值=230 贯穿所有拍）', () => {
    ...
    it('50 拍后 货币系统守恒：Σ净值 === 230', () => {
        ...
        expect(() => assertConservation(s.货币系统.账户, 230, getNetAsset)).not.toThrow();
    });
    it('单拍后守恒验证通过', () => {
        const { state: s1 } = runTick(makeWorld(), { tickId: 'cons-single', spanMinutes: 1440 });
        expect(() => assertConservation(s1.货币系统.账户, 230, getNetAsset)).not.toThrow();
    });
});
// ── ⑦ 50 拍黄金主线 fixture ──────────────────────────────────────────────────
