...
import type { VersionSegmentHead } from '@ai-life-sim/core/engine/segment';
...
function makeState(opts: { presetId?: string; seed?: number } = {}): RootState {
...
describe('PR1-F1 · swap 确定性逐位恒等', () => {
  it('同 inputs 两次 swapPreset → state 深度相等', () => {
    ...
    // 段记录完全相同
    const segs1 = (r1.state._存档头.版本段记录 ?? []) as unknown as VersionSegmentHead[];
    const segs2 = (r2.state._存档头.版本段记录 ?? []) as unknown as VersionSegmentHead[];
...
describe('PR1-F2 · 段链完整', () => {
  ...
  it('A→B 两次 swap → 链长=2·verifySegmentChain valid', () => {
    ...
    const segs = (r2.state._存档头.版本段记录 ?? []) as unknown as VersionSegmentHead[];
  ...
  it('三次 swap → 链长=3·每段 段序号 连续', () => {
    ...
    const segs = (s._存档头.版本段记录 ?? []) as unknown as VersionSegmentHead[];
...
describe('PR1-F3 · 状态隔离', () => {
  it('旧预设私有 namespace 条目在切换后不残留', () => {
    ...
    const s0 = {
      ...
      受治理键空间注册表: {
        键条目: [
          { 规范键: 'preset_a_handler', 命名空间: 'sideEffect句柄' as const, 来源包: 'preset_a' },
        ],
      },
    } as RootState;
  ...
  it('WORLD_OWNED_STATE_KEYS 在 swap 后不变', () => {
    ...
    for (const key of WORLD_OWNED_STATE_KEYS) {
      const before = s0[key as keyof RootState];
      const after = r.state[key as keyof RootState];
  ...
  it('旧预设携带的 namespace 条目 swap 后被新预设域替换', () => {
    ...
    const s0 = {
      ...
      受治理键空间注册表: {
        键条目: [
          { 规范键: 'handler_a', 命名空间: 'sideEffect句柄' as const, 来源包: 'preset_a' },
          { 规范键: 'shared_handler', 命名空间: 'cascade句柄' as const, 来源包: 'system' },
        ],
      },
    } as RootState;
...
describe('PR1-F4 · detach 干净 + 幂等 + dangling fail-closed', () => {
  ...
  it('unloadPreset → 受治理键空间注册表 preset_a 条目已清除', () => {
    ...
    const s0 = {
      ...
      受治理键空间注册表: {
        键条目: [
          { 规范键: 'my_handler', 命名空间: 'sideEffect句柄' as const, 来源包: 'preset_a' },
          { 规范键: 'other', 命名空间: 'cascade句柄' as const, 来源包: 'other_mod' },
        ],
      },
    } as RootState;
    const s1 = unloadPreset(s0, 'preset_a');
  ...
  it('dangling fail-closed：lore side_effects 引用待卸预设句柄 → throw', () => {
    ...
    const s0 = {
      ...
      } as unknown as RootState['_lore知识库'],
    } as RootState;
  ...
  it('dangling fail-closed：cascade_on_change 引用 → throw', () => {
    ...
    const s0 = {
      ...
      受治理键空间注册表: {
        键条目: [
          { 规范键: 'cascade_handler', 命名空间: 'cascade句柄' as const, 来源包: 'preset_a' },
        ],
      },
      _lore知识库: {
        条目: [{
...
describe('PR1-F6 · 300拍 soak 守恒不破', () => {
  it('100拍(presetA) → swap(presetB) → 100拍 → swap(presetA) → 100拍·守恒持续成立', () => {
    ...
    expect(() =>
      assertConservation(accounts, EXPECTED_NET_ASSET, getNetAsset),
    ).not.toThrow();
