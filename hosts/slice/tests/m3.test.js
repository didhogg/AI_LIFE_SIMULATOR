...
import { SnapshotRingBuffer, computeChecksum, serializeArchive, deserializeArchive, assertClosedAccount, RING_K, } from '../engine/snapshot.js';
...
describe('M3 Ring Buffer（容量 K）', () => {
    ...
    it(`默认容量 RING_K=${RING_K}`, () => {
        ...
        for (let i = 1; i <= RING_K + 1; i++)
            ring.push(makeSnap(i));
...
describe('M3 ④ 关账态门规', () => {
    it.each([
        '结算中', '等待呈现', '等待选择', '关账中',
    ])('非关账态"%s"→assertClosedAccount 抛出', (state) => {
...
describe('M3 拍6 悔棋剧本', () => {
    it('② 悔棋后重放：冻结盐值→D20 结果逐位恒等', () => {
        ...
        expect(Object.fromEntries(rewound.balances)).toEqual({ pc_linjiu: 30, npc_wang: 200 });
