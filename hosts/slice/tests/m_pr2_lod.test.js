...
describe('F1 · materializeCoarseNode 确定性 + 幂等', () => {
    ...
    it('F1-4 不同 key → 不同属性（种子区分）', () => {
        ...
        const allEqual = a.体质 === b.体质 && a.智慧 === b.智慧 && a.感知 === b.感知 &&
            a.魅力 === b.魅力 && a.心理 === b.心理;
...
describe('F5 · 默认 fixture 零漂移', () => {
    ...
    it('F5-3 newsToCognition(空观察者) → 认知档案不变', () => {
        ...
        const news = { 主体: 'nobody', 标签: 'test', 极性: '正', 强度: 50, 维度: '关系', Δ方向: 1, 量级: 60, 来源: 'test' };
    ...
    it('F6-2 soak 双跑逐位恒等（seeded 可复现）', () => {
        ...
        function runSoak() {
            ...
            for (let tick = 0; tick < RUNS; tick++) {
                s = structuredClone(s);
                if (tick % 5 === 0)
                    newsToCognition(s, news, [NPC_WANG], tick * 100);
                if (tick === 20)
                    triggerLodGate(s, [COARSE_SOAK2], SEED);
