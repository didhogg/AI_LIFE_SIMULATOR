...
import { clearLodRegistry, getLodMount, registerProductionLodMounts, } from '@ai-life-sim/core/engine/lodMount';
import { promoteNode, demoteNode, startWarmWindow, checkWarmWindow, LOD_WARM_WINDOW_DEFAULT, } from '@ai-life-sim/core/engine/lodScheduler';
// ── constants ─────────────────────────────────────────────────────────────────
const SEED = 42;
...
describe('B4-1 · 迁移 round-trip', () => {
    ...
    it('仅有 保温到期拍号（无 LOD态）→ 迁入 LOD表·档位默认粗', () => {
        ...
        expect((result['_系统']['migration_version'])).toBe(2);
    });
});
// ── B4-2 · 幂等：二次迁移 no-op ───────────────────────────────────────────────
describe('B4-2 · 迁移幂等', () => {
    it('二次 backfillLodTableMapState → no-op（migration_version 不再 bump）', () => {
        ...
        const pass1 = backfillLodTableMapState(raw);
        const ver1 = (pass1['_系统']['migration_version']);
        expect(ver1).toBe(4);
        const pass2 = backfillLodTableMapState(pass1);
...
describe('B4-7 · 保温窗口 LOD表 路径', () => {
    ...
    it('demoteNode 后 保温到期拍号 从 LOD表 清除', () => {
        const s = makeMapState();
        s.LOD表[NODE_KEY] = { 模块键: NODE_KEY, 档位: '实体' };
