...
describe('A1 groupNodesByLocation — 节点按地点分组', () => {
  ...
  it('无 NPC state 返回空数组', () => {
    ...
    const emptyState = { ...state, NPC: Object.create(null) as any };
  ...
  it('无位置 NPC 归入「（无位置）」组', () => {
    const state = buildWorld();
    // 将所有 NPC 清空位置
    const modifiedNPC = Object.fromEntries(
      Object.entries(state.NPC).map(([k, v]) => [k, { ...v, 位置: '' }]),
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const modState = { ...state, NPC: modifiedNPC as any };
...
describe('恒等门 — 指纹84 / schemaKeys52 守恒', () => {
  it('指纹字段总数恒等 84', () => {
