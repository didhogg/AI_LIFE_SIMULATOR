...
import { runActionInDualMode, runValidationChain, runTickWithDiff, DEMO_RAW_CANDIDATES, } from '../../web-debug/aohpDebugConsole.js';
// ──────────────────────────────────────────────────────────────────────────────
...
describe('T2 runActionInDualMode forceFailure — isFallback/usedDefault=true', () => {
    it('forceFailure → isFallback=true', async () => {
        const state = buildWorld();
        const res = await runActionInDualMode(state, PC, '对话:npc_wang', DEMO_RAW_CANDIDATES, 'demo', { forceFailure: true });
...
describe('T3 scriptedNarrative 字面恒等 — UI 不二次改写', () => {
    it('scriptedNarrative 原值透传', async () => {
        ...
        const res = await runActionInDualMode(state, PC, '对话:npc_wang', DEMO_RAW_CANDIDATES, 'demo', { scriptedNarrative: prose });
...
describe('T4 越权 optionId → usedDefault=true', () => {
    it('知情受限选项 → usedDefault=true（走 permitted 第一项）', async () => {
        const state = buildWorld();
        const res = await runActionInDualMode(state, PC, '询问:npc_wang', DEMO_RAW_CANDIDATES, 'demo');
        expect(res.usedDefault).toBe(true);
    });
});
// ──────────────────────────────────────────────────────────────────────────────
...
// T8. 指纹84 / schemaKeys52 守恒
// ──────────────────────────────────────────────────────────────────────────────
describe('T8 指纹84 / schemaKeys52 守恒', () => {
