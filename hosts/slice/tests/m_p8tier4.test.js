...
// ── AOHP 语义键 ────────────────────────────────────────────────────────────────
import { buildOptionId, buildMenuOptionIds, sortedOptionIds, } from '@ai-life-sim/core/engine/aohp';
// ── 指纹边界 ──────────────────────────────────────────────────────────────────
...
// ── 菜单知情过滤 ───────────────────────────────────────────────────────────────
import { filterMenuCandidates, MENU_FILTER_ROLL_HINT, } from '../engine/menuFilter.js';
...
function lintAntiLabeling(text) {
    return {
...
describe('Batch4 ② sortedOptionIds 进指纹边界（排序后·顺序无关）', () => {
    ...
    it('AOHP選項id集 在 FINGERPRINT_PRESET_FIELDS 内（进指纹断言）', () => {
        expect(FINGERPRINT_PRESET_FIELDS.includes('AOHP選項id集')).toBe(true);
    });
    it('AOHP選項id集 不在 FINGERPRINT_EXCLUDED_FIELDS 内（不是排除项）', () => {
        expect(FINGERPRINT_EXCLUDED_FIELDS.includes('AOHP選項id集')).toBe(false);
    });
...
describe('Batch4 ④ 轻量 lint 断言（Anti-Labeling·抽象标签/数值检出）', () => {
    it('clean 叙事文本：无抽象标签·无数字 → lint 通过', () => {
        const { labels, hasDigits } = lintAntiLabeling('王掌柜扯了扯袖口，视线扫过门口，低声催促伙计添热水。');
...
describe('Batch4 ⑤ NSFW Ring0/UI defer P1 + 调试覆盖接口预留', () => {
    ...
    it('调试覆盖接口仅在 web-debug 宿主（函数只在 hosts/web-debug/index.ts 中定义）', () => {
        ...
        import('../../hosts/slice/index.js').then(sliceModule => {
            expect(sliceModule['isDebugNsfwOverrideActive']).toBeUndefined();
        }).catch(() => { });
    });
});
