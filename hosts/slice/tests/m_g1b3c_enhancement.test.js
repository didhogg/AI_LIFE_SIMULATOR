...
import { buildWorld, PC, NPC_WANG, SAVE_SEED, } from '../fixture/world.js';
import { inspectMenu, runValidationChain, runTickWithDiff, runActionInDualMode, DEMO_RAW_CANDIDATES, } from '../../web-debug/aohpDebugConsole.js';
// ──────────────────────────────────────────────────────────────────────────────
// 本地 tryMapToOptionId（与 main.ts 逻辑一致，供单元测试直接验证）
// ──────────────────────────────────────────────────────────────────────────────
function localTryMapToOptionId(text, menuWithIds) {
    const t = text.trim();
    if (!t)
...
describe('TB2 tryMapToOptionId — displayText 匹配', () => {
    it('输入是 displayText 子串 → display matchType', () => {
        ...
        if (!target || !target.displayText)
            return; // 无 displayText 选项则跳过
...
describe('TB4 tryMapToOptionId — 无匹配 → 纯RP·不写账', () => {
    ...
    it('纯RP: runActionInDualMode(__rp_only__) → usedDefault=true（不在 permittedIds）', async () => {
        const state = buildWorld();
        const res = await runActionInDualMode(state, PC, '__rp_only__', DEMO_RAW_CANDIDATES, 'demo', { scriptedNarrative: '独自站在街头，思绪飘远。' });
    ...
    it('纯RP: scriptedNarrative 透传·不写账（state 引用不变）', async () => {
        ...
        const res = await runActionInDualMode(state, PC, '__rp_only__', DEMO_RAW_CANDIDATES, 'demo', { scriptedNarrative: prose });
