...
import { materializeCoarseNode, triggerLodGate, isCoarseNode, } from '@ai-life-sim/core/engine/lodEngine';
import { promoteNode, tryDemoteNode, startWarmWindow, LOD_WARM_WINDOW_DEFAULT, } from '@ai-life-sim/core/engine/lodScheduler';
// ── 常量 ──────────────────────────────────────────────────────────────────────
const SEED = 42;
...
function makeRawWithLodField(npcKey, lodState, migrationVersion = 0) {
    return {
        NPC: {
...
/** 已迁移 raw（无 LOD档位）*/
function makeRawNoLodField(npcKey, migrationVersion = 1) {
    return {
        NPC: {
...
function makeLocState() {
    ...
    s.LOD表 ??= {}; // R6 opt-in
    s.LOD表[NPC_KEY] = { 模块键: NPC_KEY, 档位: '粗' };
    return s;
}
...
describe('B5-0 · backfillLodTableNpcState 迁移 round-trip', () => {
    ...
    it('NPC 实体态 round-trip：LOD档位=实体 → LOD表[k].档位=实体·NPC 无 LOD档位·version +1', () => {
        ...
        const npcRec = result['NPC'];
        expect('LOD档位' in npcRec[NPC_KEY]).toBe(false);
        expect((result['_系统']['migration_version'])).toBe(6);
    });
    it('二次迁移 no-op：JSON 逐位恒等·migration_version 不再 bump', () => {
        const raw = makeRawWithLodField(NPC_KEY, '粗', 0);
        const pass1 = backfillLodTableNpcState(raw);
        const ver1 = pass1['_系统']['migration_version'];
        expect(ver1).toBe(1);
        const pass2 = backfillLodTableNpcState(pass1);
        const ver2 = pass2['_系统']['migration_version'];
...
describe('B5-1 · 老存档双段迁移', () => {
    it('地图地点含 LOD态 + NPC含 LOD档位 → 顺序两段都跑·migration_version +2', () => {
        ...
        const afterMap = backfillLodTableMapState(raw);
        expect((afterMap['_系统']['migration_version'])).toBe(8);
        const afterNpc = backfillLodTableNpcState(afterMap);
        expect((afterNpc['_系统']['migration_version'])).toBe(9);
        // 两段均写入 LOD表
        const lodTable = afterNpc['LOD表'];
        expect(lodTable[LOC_KEY]['档位']).toBe('实体');
        expect(lodTable[LOC_KEY]['保温到期拍号']).toBe(99);
        expect(lodTable[NPC_KEY]['档位']).toBe('粗');
        // NPC 中 LOD档位 已清除
        const npcRec = afterNpc['NPC'];
        expect('LOD档位' in npcRec[NPC_KEY]).toBe(false);
        // 地图地点散落字段已清除
        const locs = afterNpc['地图']['地点'];
...
describe('B5-5 · 迁移往返幂等', () => {
    it('双 NPC 各有 LOD档位 → 迁移后 JSON 逐位恒等·二次不再 bump', () => {
        ...
        const pass1 = backfillLodTableNpcState(raw);
        expect((pass1['_系统']['migration_version'])).toBe(21);
        const lodTable = pass1['LOD表'];
        expect(lodTable[NPC_KEY]['档位']).toBe('粗');
        expect(lodTable[NPC_KEY_2]['档位']).toBe('实体');
        // 二次迁移：JSON 逐位恒等
        const pass2 = backfillLodTableNpcState(pass1);
        expect(JSON.stringify(pass1)).toBe(JSON.stringify(pass2));
        expect((pass2['_系统']['migration_version'])).toBe(21);
    });
    it('迁移后 RootSchema.parse 成功·NPC 无 LOD档位 字段', () => {
        const raw = makeRawWithLodField(NPC_KEY, '粗', 0);
        const migrated = backfillLodTableNpcState(raw);
