...
import { deriveEffectivePrice, applyDriftCandidate, computeDecayFactor, hasActiveWar, ECONOMY_PRICE_CLAMP_LO, ECONOMY_PRICE_CLAMP_HI, } from '@ai-life-sim/core/engine/economyEngine';
...
describe('F3 · 修正系数衰减（P13）', () => {
    ...
    it('F3-4 衰减后有效价格向基线收敛（拍号大→偏差小）', () => {
        ...
        const s0 = baseStateWithPrice(100, 0);
        s0._tick.拍计数 = 0;
        const s10 = baseStateWithPrice(100, 0);
        s10._tick.拍计数 = 10;
        const s50 = baseStateWithPrice(100, 0);
        s50._tick.拍计数 = 50;
    ...
    it('F3-5 同 tick 双运行结果逐位恒等（闭式守恒）', () => {
        ...
        for (const tick of [0, 5, 20, 100]) {
            const s1 = baseStateWithPrice(50, 20);
            s1._tick.拍计数 = tick;
            const s2 = baseStateWithPrice(50, 20);
            s2._tick.拍计数 = tick;
