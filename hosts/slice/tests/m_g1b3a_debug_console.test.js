...
import { buildWorld, PC, NPC_WANG, NPC_HONG, SECRET_S1, SAVE_SEED, } from '../fixture/world.js';
// 测试目标函数（从 web-debug 相对引用）
import { inspectMenu, runValidationChain, runTickWithDiff, runActionInDualMode, TimeController, DEMO_RAW_CANDIDATES, PHASE6_THRESHOLD, } from '../../web-debug/aohpDebugConsole.js';
import { buildDebugFixtureSmall, buildDebugFixtureMedium, buildDebugFixtureLarge, SMALL_SEED, MEDIUM_SEED, LARGE_SEED, DEBUG_FIXTURES, getDebugFixture, } from '../../web-debug/fixtures/debugFixtures.js';
// ──────────────────────────────────────────────────────────────────────────────
// T1. 菜单过滤前后 + 原因码
// ──────────────────────────────────────────────────────────────────────────────
describe('T1 菜单生成检视 — 过滤前后/原因码', () => {
    ...
    it('无 secretRef 选项均在 permitted', () => {
        ...
        for (const c of noSecret) {
            expect(r.filterResult.permitted.some(p => p.verb === c.verb && p.targetEntityId === c.targetEntityId)).toBe(true);
        }
    });
...
describe('T3 校验链 — 合法 option_id 全通过', () => {
    ...
    it('给钱:npc_hong:5文 + 匹配 proposal — 含 GATE_PASS + runTick 成功', () => {
        const r = runValidationChain('给钱:npc_hong:5文', state, PC, DEMO_RAW_CANDIDATES, '林九取出五文钱递给红姨，换来一碗热茶。', { transfers: [{ from: PC, to: NPC_HONG, amount: 5, reason: '给钱' }], checks: [], knowledge: [] });
        expect(r.passed).toBe(true);
        const gateStep = r.steps.find(s => s.stepName === 'effect闸');
...
describe('T4 校验链 — effect 闸拒绝', () => {
    ...
    it('赊账叙事 → reconcileGate hard_rejected → GATE_REJECTED', () => {
        const r = runValidationChain('还账:npc_wang:8文', state, PC, DEMO_RAW_CANDIDATES, '林九赊账了八文钱的酒菜。', { transfers: [{ from: PC, to: NPC_WANG, amount: 8, reason: '赊账' }], checks: [], knowledge: [] });
        expect(r.passed).toBe(false);
...
describe('T6 LLM 失败注入 — 兜底不崩', () => {
    ...
    it('forceFailure=true → isFallback=true 且 usedDefault=true', async () => {
        const r = await runActionInDualMode(state, PC, '对话:npc_wang', DEMO_RAW_CANDIDATES, 'demo', { forceFailure: true });
    ...
    it('forceFailure=true → 走 permitted 第一项作为默认 option', async () => {
        const r = await runActionInDualMode(state, PC, '询问:npc_wang', DEMO_RAW_CANDIDATES, 'demo', { forceFailure: true });
        expect(r.usedDefault).toBe(true);
    ...
    it('demo 模式（非失败注入）→ isFallback=false · usedDefault=false', async () => {
        const r = await runActionInDualMode(state, PC, '对话:npc_wang', DEMO_RAW_CANDIDATES, 'demo');
        expect(r.isFallback).toBe(false);
        expect(r.usedDefault).toBe(false);
        expect(r.optionId).toBe('对话:npc_wang');
    });
    it('demo 模式自定脚本叙事 → narrative 正确注入', async () => {
        const r = await runActionInDualMode(state, PC, '对话:npc_wang', DEMO_RAW_CANDIDATES, 'demo', { scriptedNarrative: '王掌柜点头示意。' });
...
describe('T7 场景 fixture 确定性 — 两次调用逐位恒等', () => {
    ...
    it('整世界 fixture 关系边数 > 大陆 fixture（规模递增）', () => {
        const edgeCount = (s) => Object.values(s.NPC).reduce((acc, npc) => acc + npc.关系.length, 0);
        expect(edgeCount(buildDebugFixtureLarge())).toBeGreaterThan(edgeCount(buildDebugFixtureMedium()));
    });
