...
import { ModalStackController, MODAL_STACK_MAX_DEPTH, Ring2GenerationTracker, deterministicLottery, IntentBarrier, } from '../engine/concurrency.js';
...
import { SnapshotRingBuffer, } from '../engine/snapshot.js';
...
function lt(from, to, amount, eventId, reason = '') {
    return { from, to, amount, reason, eventId };
}
function makeTxnGroup(groupId, transfers, arrivalOrder, preEval) {
...
describe('P7-4a · 级联 worklist 不动点收敛', () => {
    it('简单链 A→B→C：3 条目处理·3 轮·无重复', () => {
        ...
        const apply = (e) => {
            if (e.entityKey === 'A')
                return [{ entityKey: 'B', triggerId: 'tB', triggerType: '日期' }];
            if (e.entityKey === 'B')
                return [{ entityKey: 'C', triggerId: 'tC', triggerType: '标志' }];
            return [];
        };
    ...
    it('visited 防环：A→B→A 不无限迭代·2 条目处理', () => {
        ...
        const apply = (e) => {
            if (e.entityKey === 'A')
                return [{ entityKey: 'B', triggerId: 't2', triggerType: '关系' }];
            if (e.entityKey === 'B')
                return [{ entityKey: 'A', triggerId: 't1', triggerType: '关系' }]; // 回路
            return [];
        };
...
describe('P7-4d · 确定性抽签', () => {
    ...
    it('不同种子 → 可产生不同结果', () => {
        ...
        const results = new Set(Array.from({ length: 20 }, (_, i) => deterministicLottery(i * 7 + 1, candidates)));
...
describe('P7-4e · TxnGroup 全通过', () => {
    it('全通过 → 正确落账', () => {
        ...
        const group = makeTxnGroup('g-1', [lt(PC, NPC_HONG, 2, 'g1-e1')], 1, preEval);
    ...
    it('到达序排序 ascending', () => {
        const g = (id, order) => makeTxnGroup(id, [], order, {});
...
describe('P7-4e · C1 closure', () => {
    const 全局 = 全局Schema.parse({
        约定库: {
            ...
            'cov-dsl': {
                形式: '合同',
                条款: [{ 内容: 'DSL', 标的: { v: '1.0', expr: 'state.x + 1' } }],
                约束力: 80,
            },
            'cov-empty': {
                形式: '空约定',
