...
import {
  runValidationGate,
  deriveBeliefFromState,
} from '../engine/validationGate.js';
import {
...
const SLICE_PC_POV: ValidationSlice = {
...
const SLICE_WANG_POV: ValidationSlice = {
...
const SLICE_WITH_ABSENT: ValidationSlice = {
  ...SLICE_PC_POV,
  presentNpcNames: ['王掌柜'],
...
describe('P0-8 ② PANGeA 叙事语义校验', () => {
  ...
  it('纠偏重试：首次失败+重试叙事通过 → valid=true·retriedOnce=true', () => {
    const result = validateNarrativeSemantics(
      NARRATIVE_OVERREACH, SLICE_PC_POV,
      NARRATIVE_VALID, // 重试叙事合法
    );
    expect(result.valid).toBe(true);
...
describe('P0-8 ④ WhatELSE 动机校验', () => {
  it('情绪矛盾 fixture → 检出 MotivationConflict(type=情绪矛盾)', () => {
    const dummyBeliefState: BeliefState = { povKey: PC, 感知: [], 推理: [], 信念: [] };
    const anchor = buildMotivationAnchor(
      NPC_WANG, '王掌柜', dummyBeliefState,
      ['警惕', '好奇'],      // topEmotions: 警惕（负面）
      ['不满'],              // recentMemoryTags
    );
...
describe('P0-8 ⑤ 校验闸·见真相输出过滤·物理隔离', () => {
  ...
  it('纠偏重试链路：校验闸 retryNarrative 合法 → valid=true', () => {
    const result = runValidationGate(
      NARRATIVE_OVERREACH, BASE_STATE,
      { povKey: PC, retryNarrative: NARRATIVE_VALID },
    );
    expect(result.valid).toBe(true);
    expect(result.retriedOnce).toBe(true);
  });
  ...
  it('双失败软拒：校验闸 retryNarrative 也越权 → softReject 出现', () => {
    const result = runValidationGate(
      NARRATIVE_OVERREACH, BASE_STATE,
      { povKey: PC, retryNarrative: NARRATIVE_OVERREACH, retryMode: 'single_retry_soft_reject' },
    );
    expect(result.valid).toBe(false);
    expect(result.softReject).toBeDefined();
    expect(result.softReject!.ui提示).toContain('重 Roll');
