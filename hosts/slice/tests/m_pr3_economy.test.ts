...
import type { 経済生成規則Type } from '@ai-life-sim/core';
...
/** 最小経済生成規則对象 */
function makeEconomyRule(overrides: Partial<経済生成規則Type> = {}): 経済生成規則Type {
  return {
...
describe('F1 · deriveEffectivePrice 确定性 + 幂等', () => {
  it('F1-1 相同输入两次结果逐位恒等', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0.5, 供需权重: 0.3 });
    const r1 = deriveEffectivePrice(baseStateWithPrice(60, 40), rule, REGION_ID, CATEGORY_A);
    const r2 = deriveEffectivePrice(baseStateWithPrice(60, 40), rule, REGION_ID, CATEGORY_A);
    expect(r1).toBe(r2);
  });

  it('F1-2 同 state 两次调用结果不变（纯函数）', () => {
    const rule = makeEconomyRule();
    const s = baseStateWithPrice(50, 20);
    expect(deriveEffectivePrice(s, rule, REGION_ID, CATEGORY_A))
      .toBe(deriveEffectivePrice(s, rule, REGION_ID, CATEGORY_A));
  });

  it('F1-3 纯函数：调用后 state 不变', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0.8 });
    const s = baseStateWithPrice(80, 0);
    const before = JSON.stringify(s.地图.区域物价);
    deriveEffectivePrice(s, rule, REGION_ID, CATEGORY_A);
...
describe('F2 · 退化守卫', () => {
  ...
  it('F2-3 区域物价无该品类·品类基线也无 → 返回 0', () => {
    ...
    const rule = makeEconomyRule({ 品类基线: {} });
    expect(deriveEffectivePrice(s, rule, REGION_ID, CATEGORY_A)).toBe(0);
  });
  ...
  it('F2-5 标准 buildWorld（无经济规则）两次调用结果不漂移', () => {
    const w1 = buildWorld();
    const w2 = buildWorld();
    // 退化路径：无区域物价 → 0 → 无漂移
...
describe('F3 · 修正系数衰减（P13）', () => {
  ...
  it('F3-4 衰减后有效价格向基线收敛（拍号大→偏差小）', () => {
    ...
    const p0  = deriveEffectivePrice(s0,  rule, REGION_ID, CATEGORY_A);
    const p10 = deriveEffectivePrice(s10, rule, REGION_ID, CATEGORY_A);
    const p50 = deriveEffectivePrice(s50, rule, REGION_ID, CATEGORY_A);
  ...
  it('F3-5 同 tick 双运行结果逐位恒等（闭式守恒）', () => {
    const rule = makeEconomyRule({ 衰减率: 0.03 });
    for (const tick of [0, 5, 20, 100]) {
      ...
      expect(deriveEffectivePrice(s1, rule, REGION_ID, CATEGORY_A))
        .toBe(deriveEffectivePrice(s2, rule, REGION_ID, CATEGORY_A));
    }
  });
});
...
describe('F4 · 信号单调性 + 钳制', () => {
  it('F4-1 资源紧张度↑ → 有效价格单调不降（正权重）', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0.5, 供需权重: 0, 战时修正权重: 0 });
    const pLow  = deriveEffectivePrice(baseStateWithPrice(10, 0), rule, REGION_ID, CATEGORY_A);
    const pHigh = deriveEffectivePrice(baseStateWithPrice(90, 0), rule, REGION_ID, CATEGORY_A);
    expect(pHigh).toBeGreaterThanOrEqual(pLow);
  });

  it('F4-2 供需正值↑ → 有效价格单调不降', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0, 供需权重: 0.5, 战时修正权重: 0 });
    const pNeg = deriveEffectivePrice(baseStateWithPrice(0, -80), rule, REGION_ID, CATEGORY_A);
    const pPos = deriveEffectivePrice(baseStateWithPrice(0,  80), rule, REGION_ID, CATEGORY_A);
    expect(pPos).toBeGreaterThanOrEqual(pNeg);
  });

  it('F4-3 战时激活 → 有效价格不低于和平时', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0, 供需权重: 0, 战时修正权重: 0.5 });
    const sPeace = baseStateWithPrice(0, 0);
    const sWar   = baseStateWithPrice(0, 0);
    sWar.战争状态 ??= {};  // R6 opt-in
    sWar.战争状态['war_01'] = { 战争名: '测试战争', 参战方: [], 战争目标: '', 状态: '交战', 激活: true };
    expect(deriveEffectivePrice(sWar,   rule, REGION_ID, CATEGORY_A))
      .toBeGreaterThanOrEqual(deriveEffectivePrice(sPeace, rule, REGION_ID, CATEGORY_A));
  });

  it('F4-4 极端正信号 → 有效价格不超 HI × 基线（上钳制）', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0.9, 供需权重: 0.9, 战时修正权重: 0.9 });
    const s = baseStateWithPrice(100, 100);
    s.战争状态 ??= {};  // R6 opt-in
    s.战争状态['w'] = { 战争名: '极限战', 参战方: [], 战争目标: '', 激活: true };
    const p = deriveEffectivePrice(s, rule, REGION_ID, CATEGORY_A);
    expect(p).toBeLessThanOrEqual(Math.ceil(BASE_PRICE * ECONOMY_PRICE_CLAMP_HI));
  });

  it('F4-5 极端负信号 → 有效价格不低于 LO × 基线（下钳制）', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0, 供需权重: 0.9, 战时修正权重: 0 });
...
describe('F5 · 漂移候选再基线（P14）', () => {
  it('F5-1 漂移超阈 → 写入候选基线', () => {
    ...
    const rule = makeEconomyRule({ 资源紧张度权重: 0.9, 供需权重: 0, 战时修正权重: 0 });
    const s = baseStateWithPrice(100, 0);
    applyDriftCandidate(s, rule, REGION_ID, CATEGORY_A);
  ...
  it('F5-2 候选基线 = deriveEffectivePrice 返回值', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0.9, 供需权重: 0, 战时修正权重: 0 });
    const s = baseStateWithPrice(100, 0);
    const expected = deriveEffectivePrice(s, rule, REGION_ID, CATEGORY_A);
    applyDriftCandidate(s, rule, REGION_ID, CATEGORY_A);
  ...
  it('F5-3 漂移未超阈 → 不写候选基线', () => {
    ...
    const rule = makeEconomyRule({ 资源紧张度权重: 0, 供需权重: 0, 战时修正权重: 0 });
    const s = baseStateWithPrice(100, 50);
    applyDriftCandidate(s, rule, REGION_ID, CATEGORY_A);
    expect(s.地图.区域物价[REGION_ID]?.[CATEGORY_A]?.候选基线).toBeUndefined();
  });

  it('F5-4 applyDriftCandidate 不修改规则对象', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0.9 });
  ...
  it('F5-5 无匹配 region → no-op（候选基线不存在）', () => {
    const rule = makeEconomyRule({ 资源紧张度权重: 0.9 });
    const s = buildWorld();
    applyDriftCandidate(s, rule, 'nonexistent_region', '品类');
...
describe('F6 · 300 拍 soak', () => {
  it('F6-1 300 拍守恒持续成立（派生调用不破货币守恒）', () => {
    let s = buildWorld();
    const rule = makeEconomyRule({ 资源紧张度权重: 0.3, 供需权重: 0.2, 衰减率: 0.01 });
    for (let i = 0; i < 300; i++) {
  ...
  it('F6-2 双跑 300 拍有效价格逐位恒等', () => {
    const runN = () => {
      let s = buildWorld();
      const rule = makeEconomyRule({ 资源紧张度权重: 0.3, 衰减率: 0.005 });
      const prices: number[] = [];
      for (let i = 0; i < 300; i++) {
        ({ state: s } = runTick(s, { tickId: `dual:${i}`, spanMinutes: 1440 }));
        prices.push(deriveEffectivePrice(s, rule, REGION_ID, CATEGORY_A));
      }
      return prices;
    };
    const r1 = runN();
