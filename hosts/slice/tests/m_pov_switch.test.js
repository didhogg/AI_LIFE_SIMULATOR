...
import { povInspect, comparePOVs, computePovPersonalityProjection, } from '../../web-debug/aohpDebugConsole2.js';
// ──────────────────────────────────────────────────────────────────────────────
...
describe('T1 切换 povEntityKey → 可见集·认知档案随之变化', () => {
    ...
    it('切换 povEntityKey 后可见集从 PC 变为 NPC_WANG（输出差异化）', () => {
        ...
        // 整体结果不等
        expect(JSON.stringify(rPC.visibleSecretIds.sort())).not.toBe(JSON.stringify(rWang.visibleSecretIds.sort()));
    });
...
describe('T3 切 POV 不影响 operatorKey', () => {
    it('povInspect 不读取/写入 operatorKey 字段（state 无 operatorKey 顶层字段）', () => {
        ...
        expect(state['operatorKey']).toBeUndefined();
    });
