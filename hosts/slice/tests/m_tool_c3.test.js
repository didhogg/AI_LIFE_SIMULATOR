...
import { dispatchTool, executeRollDice, MAX_EXPLOSION_DEPTH, } from '@ai-life-sim/core/engine/toolExecutor';
...
describe('C3-4 · G0 重定基报告', () => {
    ...
    it('lore 金向量路径不经过 rngFor（指纹计算路径与 roll_dice 独立）', () => {
        ...
        const toolRelatedBundleMembers = FINGERPRINT_BUNDLE_MEMBERS.filter(m => m.includes('工具') || m.includes('tool'));
...
describe('C3-6 · 无 rollDiceArgs 骨架占位', () => {
    it('不传 rollDiceArgs → ok=true, rollDice=undefined', () => {
        ...
        if (r.ok)
            expect(r.rollDice).toBeUndefined();
    });
