...
loadDotEnv();
import { buildWorld, PC, NPC_WANG, NPC_HONG, LOC_NAME, RECIPE, RECIPE_KEY, SAVE_SEED, CREDIT_AMOUNT, CREDIT_REASON, } from "./fixture/world.js";
...
const narrativeHistory = []; // 叙事记忆：喂回 LLM，剧情才能前进
const actionHistory = []; // 动作序列：让叙事感知动作间的转折，NPC 才能对「刚做完X又来做Y」有反应
...
const 时段表 = ["清晨", "上午", "晌午", "午后", "傍晚", "入夜"];
const TICKS_PER_DAY = 时段表.length;
function gameClock(tc) {
...
function takeSnapshot() {
    ...
    if (auxStack.length > WEB_RING_CAP)
        auxStack.shift();
}
// ── 试算平衡（会计恒等式·双分录自检闸）────────────────────────────────────────
...
function assertTrialBalance() {
    ...
    if (net !== INITIAL_NET_WORTH) {
        throw new Error(`净资产守恒失败：现金${cash}+应收${wangReceivable}−应付${debt}=${net}，应为${INITIAL_NET_WORTH}`);
    }
}
// ── sink 只进不出守卫（P0-7 P7-1c）────────────────────────────────────────────
function assertSinkNotFrom(transfers) {
...
async function narrativeWithFilter(action, checkInfo) {
    ...
    const recap = `【当前账目】林九 ${linjiuBal} 文、王掌柜 ${wangBal} 文、红姨 ${hongBal} 文。\n` +
        `【赊欠】${debtLine}\n` +
    ...
    try {
        switch (action) {
            case "对话": {
                ...
                const narrative = await narrativeWithFilter(`拍${tickCount}：林九与在场人物交谈。`);
            ...
            case "给钱": {
                ...
                if ((balances.get(PC) ?? 0) < 2) {
                    return {
                        narrative: "林九摸了摸钱袋，指尖只触到空荡荡的布角——一文铜钱也不剩了。这钱，今日是给不出去了。",
                        status: getStatus(),
                        error: "林九余额不足，无法给钱",
                    };
                }
                ...
                const narrative给钱 = await narrativeWithFilter(`拍${tickCount}：林九掏出2文铜钱递给红姨，作为跑腿酬劳。`);
            ...
            case "检定": {
                ...
                tickCount++;
                lifecycle = "结算中";
                const checkResult = runD20Check(SAVE_SEED, tickCount, RECIPE_KEY, RECIPE.attrBonus, RECIPE.dc, header);
                ...
                const narrative = await narrativeWithFilter(`拍${tickCount}：林九向王掌柜请求赊账，说起了当年旧情。`, checkInfo);
                lastNarrative = narrative;
                narrativeHistory.push(narrative);
            ...
            case "还账": {
                ...
                const narrative = await narrativeWithFilter(`拍${tickCount}：林九取出${repay}文铜钱，归还先前向王掌柜赊欠的账。`);
                lastNarrative = narrative;
                narrativeHistory.push(narrative);
