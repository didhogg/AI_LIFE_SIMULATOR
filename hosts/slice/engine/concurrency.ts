...
export class ModalStackController {
  private _stack:        ModalEntry[] = [];
  private _epochCounter: number       = 0;
   ...
   */
  pop(epoch: number): string | null {
    const top = this._stack[this._stack.length - 1];
    if (!top || top.epoch !== epoch) return null;  // stale epoch → discard
    this._stack.pop();
    return this._stack[this._stack.length - 1]?.mode ?? '';
  }

  snapshot(): ModalEntry[] { return [...this._stack]; }
  restore(entries: ModalEntry[]): void { this._stack = [...entries]; }
}
...
export class Ring2GenerationTracker {
  ...
  validate(callId: string, generation: number): boolean {
 ...
 */
export function deterministicLottery(seed: number, candidates: readonly string[]): string {
...
export class IntentBarrier {
  private readonly _seats:    ReadonlySet<string>;
  private          _received: Map<string, unknown>;
  ...
  register(seatId: string, intent: unknown): void {
  ...
  /** 放行并清空（下一节拍从头收集）*/
  flush(): Map<string, unknown> {
    const result   = new Map(this._received);
    this._received = new Map();
    return result;
  }

  get registeredCount(): number { return this._received.size; }
  get totalSeats(): number      { return this._seats.size;    }
}
