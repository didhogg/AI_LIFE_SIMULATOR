...
import { validateNarrativeSemantics, DEFAULT_RETRY_MODE, } from '@ai-life-sim/core/engine/narrativeValidator';
...
function buildMotivationAnchors(state, beliefState, povKey) {
    ...
    for (const [key, npc] of Object.entries(state.NPC ?? {})) {
        if (key === povKey)
            continue;
        if (npc.位置 !== povLoc)
            continue; // 只建在场 NPC 的动机锚
        const name = npc.姓名 ?? key;
        const emotions = (npc.情绪栈 ?? [])
            .slice(-3).map(e => e.情绪名 ?? '').filter(Boolean);
        const memTags = (npc.记忆 ?? [])
            .filter(m => (m.重要度 ?? 0) >= 2)
            .slice(-5)
            .map(m => m.情绪色彩 ?? '')
            .filter(Boolean);
...
export function runValidationGate(narrative, state, opts, beliefState) {
    ...
    const fullSecrets = (state.全局?.秘密库 ?? {});
    const povFiltered = filterSecretsForPOV(fullSecrets, povKey);
    const effectiveBeliefState = beliefState ?? deriveBeliefState(state.认知档案?.[povKey], povFiltered, povKey, 'narrative');
    // PANGeA 语义校验
...
export function deriveBeliefFromState(state, povKey) {
    const fullSecrets = (state.全局?.秘密库 ?? {});
    const povFiltered = filterSecretsForPOV(fullSecrets, povKey);
    const cogArchive = state.认知档案?.[povKey];
    return deriveBeliefState(cogArchive, povFiltered, povKey, 'narrative');
}
