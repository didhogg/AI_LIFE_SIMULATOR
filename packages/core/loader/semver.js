...
function parseComparator(raw) {
    ...
    if (!m)
        throw new Error(`semver: cannot parse comparator "${raw}"`);
    const op = m[1] ?? '=';
    const v = parseSemver(m[2]);
    return { op, v };
}
function applyComparator(version, op, bound) {
...
export function intersect(rangeA, rangeB) {
    const a = rangeA.trim();
    const b = rangeB.trim();
    if (a === '')
        return b;
