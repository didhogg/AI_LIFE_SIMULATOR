...
// ─── Minimal structural input type (compatible with z.infer<typeof mod注册表Schema>) ──

/**
 * Mod entry shape — superset of mod条目Schema fields used by the loader.
 * All fields except 依赖 are optional so Step-1 tests (`{ 依赖: [...] }`) stay valid.
 *
 * pack_id is used for conflict matching (冲突[] references pack_ids).
 * Falls back to record key when absent (pre-B2 strictness; B2 will make pack_id required).
 */
export interface ModEntry {
  依赖: string[];
  pack_id?: string;   // conflict target identity; fallback = record key
...
export type ModRegistry = Record<string, ModEntry>;

// ─── Output types ─────────────────────────────────────────────────────────────

export interface ModGraphResult {
  /** All SCCs; each group sorted by codepoint; groups sorted by their min key. */
  sccs: string[][];
  /** Record keys whose 依赖[] contains themselves. */
  selfLoops: string[];
  /** SCCs with size > 1 (true dependency cycles). */
  cycles: string[][];
  /**
   * Dependency targets referenced in 依赖[] but absent from the registry.
   * Sorted by codepoint.  No edge is built to these missing nodes.
   */
  danglingDeps: string[];
}
...
function sortCP(arr: readonly string[]): string[] {
...
interface PreScanResult {
  ...
  danglingDeps: Set<string>;
}

function preScan(registry: ModRegistry, knownKeys: Set<string>): PreScanResult {
...
//
// Explicit work-stack to avoid call-stack overflow on large registries.
// Follows the standard Tarjan SCC invariants:
//   - Back-edge: lowlink[v] = min(lowlink[v], index[w])   (w on stack, w visited)
//   - Tree-edge:  after child w fully processed, lowlink[v] = min(lowlink[v], lowlink[w])
//   - SCC root: lowlink[v] == index[v]
    ...
    while (callStack.length > 0) {
      ...
      if (frame.neighborIdx < neighbors.length) {
        ...
        if (!indexMap.has(w)) {
          ...
          if (wIdx < cur) lowlink.set(node, wIdx);
        }
        ...
        if (callStack.length > 0) {
          ...
          if (nodeLow < parentLow) lowlink.set(parent, nodeLow);
        }
...
export function buildModGraph(registry: ModRegistry): ModGraphResult {
...
// ═══════════════════════════════════════════════════════════════════════════════
// B1·K1 Step 2 — Kahn topological sort on condensation DAG + conflict detection
// ═══════════════════════════════════════════════════════════════════════════════
//
// Design decisions (固定口径 — do not change without updating tests):
//
//   A. Priority tie-break: 优先级数值大者后加载 (ascending sort = lower loads first).
//      Rationale: 蓝图 §6.52「后载覆盖先载·玩家自调最高」— high-priority mod loads last
//      and thus overrides earlier mods.  Actual override semantics stay B5; this step
//      only fixes the ordering.
//
//   B. Kahn multi-eligible tie-break chain:
//      1. topology constraint (Kahn in-degree — already enforced)
//      2. SCC effective priority (max 优先级 of members) — ascending (lower first)
//      3. SCC min record-key — codepoint ascending
//
//   C. Conflict detection ORDER: sort first, then detect conflicts.
//      Conflicts do NOT affect the topological order.
//
//   D. Rejection cascade: self-loops cascade (nodes that depend on a self-loop node
//      are also rejected).  Disabled mods (启用=false) do NOT cascade — they are
//      simply absent; their dependents still load.
//
//   E. Conflict matching: effectiveId = entry.pack_id ?? recordKey.
//      冲突[] entries are matched against effectiveIds of active mods.

// ─── Step 2 types ─────────────────────────────────────────────────────────────

/** A detected conflict pair; both mods are enabled + non-rejected. */
export interface ConflictPair {
  a: string;   // record key (codepoint-smaller of the two)
  b: string;   // record key (codepoint-larger)
}

export interface LoadOrderResult {
  /**
   * Record keys in deterministic load order.
   * Excludes disabled mods and rejected mods (self-loops + cascade).
   */
  flattenedLoadOrder: string[];
  /**
   * Same as flattenedLoadOrder, but grouped by SCC unit.
   * Each group's members are in codepoint order; groups are in load order.
   */
  orderedGroups: string[][];
  /**
   * Record keys excluded due to self-loop or cascade (a dep was rejected).
   * Sorted by codepoint.
   */
  rejected: string[];
  /**
   * Record keys of disabled mods (启用===false).
   * Not counted as rejected; their dependents still load.
   * Sorted by codepoint.
   */
  disabled: string[];
  /**
   * Conflict pairs detected after topological sort (see decision C).
   * Sorted by (a, b) codepoint.
   */
  conflicts: ConflictPair[];
  /**
   * Full graph result from the original (unfiltered) registry.
   * Contains selfLoops, cycles, danglingDeps of all mods including disabled.
   */
  graph: ModGraphResult;
}

// ─── Internal helpers ─────────────────────────────────────────────────────────

function effectiveId(key: string, entry: ModEntry): string {
  return entry.pack_id ?? key;
}

function isEnabled(entry: ModEntry): boolean {
  return entry.启用 !== false;
}

function entryPriority(entry: ModEntry): number {
  return entry.优先级 ?? 0;
}

// ─── computeLoadOrder ─────────────────────────────────────────────────────────

export function computeLoadOrder(registry: ModRegistry): LoadOrderResult {
  ...
  for (const [key, entry] of Object.entries(registry)) {
    if (isEnabled(entry)) enabledSet.add(key);
  ...
  const rejectedSet = new Set<string>(graph.selfLoops.filter(k => enabledSet.has(k)));
  ...
  const activeRegistry: ModRegistry = {};
  ...
  const dagAdj: Set<number>[] = Array.from({ length: n }, () => new Set<number>());
  ...
  function sccSortKey(idx: number): { pri: number; minKey: string } {
  ...
  function cmpSCCKey(a: number, b: number): number {
  ...
  while (queue.length > 0) {
    ...
    for (const neighbor of dagAdj[current]!) {
      inDegree[neighbor]!--;
      if (inDegree[neighbor] === 0) newlyZero.push(neighbor);
    }
    if (newlyZero.length > 0) {
      ...
      for (const idx of newlyZero) {
        ...
        while (lo < hi) {
          const mid = (lo + hi) >>> 1;
          if (cmpSCCKey(queue[mid]!, idx) <= 0) lo = mid + 1;
          else hi = mid;
        }
  ...
  // ── 10. Flatten result ───────────────────────────────────────────────────────
  const orderedGroups: string[][] = topoOrder.map(i => activeGraph.sccs[i]!);
  const flattenedLoadOrder: string[] = orderedGroups.flat();
  ...
  const effectiveIdToKey = new Map<string, string>();
  for (const key of activeKeys) {
    const entry = registry[key]!;
  ...
  for (const key of flattenedLoadOrder) {
    ...
    for (const cid of (entry.冲突 ?? [])) {
      ...
      if (otherKey === undefined || otherKey === key) continue;
...
export function assertNoSelfLoops(result: ModGraphResult): void {
  if (result.selfLoops.length > 0) {
    throw new Error(
      `K1: mod 自环检出 [${result.selfLoops.join(', ')}]，导入被拒绝`,
    );
  }
}

export function assertAcyclic(result: ModGraphResult): void {
...
export function assertNoConflicts(result: LoadOrderResult): void {
