...
import { parseChineseDateToEpochMin, getTickMinutes, makePeriodConverter, writeEpochMinute, } from '../engine/time.js';
...
function normalizeTraitEntry(raw) {
...
function merge特质(base, ...sources) {
    const result = {};
    for (const [k, v] of Object.entries(base))
        result[k] = normalizeTraitEntry(asRec(v));
...
function migrate状态标签(v31, 约束v31, 压力值, p2e) {
    const result = {};
...
function migrateNpcMemory(raw, p2e) {
...
function migrateWorkMemItem(raw, p2e) {
...
function migrateArchiveItem(raw, p2e) {
...
// ── 延时种子 migration ─────────────────────────────────────────────────────────
function migrateSeed(raw, p2e, log) {
    const 成熟日cycle = asNum(raw['成熟日'], -1);
    const 成熟日 = 成熟日cycle === -1 ? 0 : p2e(成熟日cycle); // 0 = 立即成熟（无到期约束）
...
// ── Secrets migration ─────────────────────────────────────────────────────────
function migrateSecretsToPool(entityKey, 秘密索引, 受制于, 秘密库, log) {
...
// ── 家族树 migration ───────────────────────────────────────────────────────────
function migrate家族树(主角键, 血缘, 树v31, 出生日期) {
...
// ── 认知档案 building ──────────────────────────────────────────────────────────
function build认知档案(主角键, npcV31, worldEpochMin) {
    const 档案 = {};
    ...
    for (const [npcKey, npcVal] of Object.entries(npcV31)) {
        ...
        for (const tag of asArr(npc['标签'])) {
            const s = asStr(tag);
            if (s)
                印象.push({ 标签: s, 极性: '中', 强度: 50, 来源: '迁移推定', 获知时间: writeEpochMinute(worldEpochMin), 衰减速率: 0 });
        }
        const 性格 = asStr(npc['性格']);
        if (性格)
            印象.push({ 标签: 性格, 极性: '中', 强度: 50, 来源: '迁移推定', 获知时间: writeEpochMinute(worldEpochMin), 衰减速率: 0 });
        const 背景 = asStr(npc['背景']);
        if (背景)
            印象.push({ 标签: 背景, 极性: '中', 强度: 30, 来源: '迁移推定', 获知时间: writeEpochMinute(worldEpochMin), 衰减速率: 0 });
        // 旧世界无姓名雾·默认已知姓名（视觉指代会让所有人变成「那个人」）
        protagonistView[npcKey] = { 了解度: Math.min(100, asNum(npc['关系深度'])), 误差表: {}, 印象, 时效: 0, 姓名知识: '已知姓名' };
    }
    if (Object.keys(protagonistView).length > 0)
        档案[主角键] = protagonistView;
    return 档案;
}
// ── 货币系统 migration ────────────────────────────────────────────────────────
function migrate货币(v31, worldEpochMin, log) {
    ...
    if (typeof 景气raw === 'number') {
        ...
        if (s && !(s in PROSPERITY_MAP))
            log.push({ level: 'warn', path: '货币系统.市场状态.大盘景气', msg: `未识别档位 "${s}"，落默认 50` });
    }
    const 换汇登记 = asArr(v31['换汇登记']).map(entry => {
        if (typeof entry === 'string')
            return { 时间: writeEpochMinute(worldEpochMin), 从: '', 到: '', 金额: 0 };
        const e = asRec(entry);
        const t = asNum(e['时间']);
...
// ── Protagonist NPC ───────────────────────────────────────────────────────────
function migrate主角NPC(主角, root, 主角键, worldEpochMin, p2e, log) {
    ...
    const 特质 = merge特质(asRec(主角['特质']), asRec(主角['正面天赋']), asRec(主角['负面天赋']), asRec(主角['隐藏天赋']), asRec(主角['先天缺陷']));
    const 状态标签 = migrate状态标签(asRec(主角['状态标签']), asRec(root['约束状态']), asNum(心理v31['压力值']), p2e);
    const 情绪栈 = [...asArr(主角['情绪栈'])];
...
// ── Simple NPC ────────────────────────────────────────────────────────────────
function migrateSimpleNPC(npcKey, npc, 主角键, worldEpochMin, p2e, 主角关系, log) {
    ...
    const 忠诚 = {};
    if (向背str)
        忠诚[主角键] = { $真实值: LOYALTY_MAP[向背str] ?? 50, 伪装度: 0 };
    ...
    for (const skillEntry of asArr(能力档v31['技能'])) {
        ...
        if (名)
            技能[名] = { 熟练度: asNum(se['熟练度']), 等级: 0, 类别: '通用', 来源: 'V3.1迁移', 施放: {} };
    }
    const 上次互动周期 = asNum(npc['上次互动周期'], 0);
    const 死亡周期 = asNum(npc['死亡周期'], -1);
    const 死亡时间 = 死亡周期 === -1 ? 0 : p2e(死亡周期); // 0 = 健在
    if (asStr(npc['性格']))
        log.push({ level: 'info', path: `NPC.${npcKey}.性格`, msg: '缺口#6: 派生量，已丢弃' });
    return {
...
// ── Child → NPC ───────────────────────────────────────────────────────────────
function migrateChild(childKey, child, 主角键, worldEpochMin, p2e, 主角关系, log, 主角世代) {
...
function migrate组织信念轴(org) {
    const 信念Raw = asRec(org['信念']);
    if (!('强制度' in 信念Raw) && !('异端容忍' in 信念Raw))
        return org;
    const 属性轴 = { ...asRec(org['属性轴']) };
    const 信念New = { ...信念Raw };
    ...
    delete 信念New['异端容忍'];
    const result = { ...org, 信念: 信念New };
    if (Object.keys(属性轴).length > 0)
        result['属性轴'] = 属性轴;
    return result;
}
// ── buildV41Raw ───────────────────────────────────────────────────────────────
export function buildV41Raw(input) {
    const log = [];
    const root = asRec(input);
    // Idempotency: V4.1 input → pass through (second migrate call)
    if (asStr(root['_系统版本']) === '4.1')
        return { raw: root, log };
    ...
    if (子嗣键列表.length > 0) {
        ...
        const 当前婚姻 = asArr(家庭v31['婚姻']).find(m => {
            const it = asRec(m);
            return asNum(it['终止'], -1) === -1 && asStr(it['配偶']);
        });
    ...
    const 冷却表v41 = {};
    for (const [k, v] of Object.entries(asRec(仲裁器v31['冷却表'])))
        冷却表v41[k] = p2e(asNum(v));
    ...
    const 播报队列v41 = asArr(仲裁器v31['播报队列']).map(item => {
        const it = asRec(item);
        if (asStr(it['渠道']) === '')
            return { 渠道: '系统', 内容: asStr(it['内容']), 播报id: asStr(it['播报id']), 重要度: asStr(it['重要度'], '普通'), 发生时间: asNum(it['发生时间']), 已读: asBool(it['已读']) };
        return it;
    });
    // ── $隐藏记忆库 ───────────────────────────────────────────────────────────────
    ...
    const raw = {
        ...
        组织实体: (() => { const out = {}; for (const [k, v] of Object.entries(asRec(root['组织实体'])))
            out[k] = migrate组织信念轴(asRec(v)); return out; })(),
        组织关系网: 组织关系网v41,
...
export function applyPrefixRenames(raw) {
    if ('_状态机' in raw)
        return raw; // idempotent guard
    const result = { ...raw };
...
function migrate内容分级位置(raw) {
    ...
    if ('内容分级' in pref)
        return raw; // 幂等：已迁移则跳过
    const mappedVal = 内容分级映射[asStr(oldVal)] ?? 'off';
    const newFunctionSwitch = { ...功能开关表 };
...
export function backfillPackId(raw) {
    const modReg = asRec(raw['mod注册表']);
    if (Object.keys(modReg).length === 0)
        return raw; // 空注册表 → 幂等 no-op
    ...
    if (!anyChanged)
        return raw; // 全已对齐 → 幂等 no-op，不 bump migration_version
    const sys = asRec(raw['_系统']);
...
export function backfill货币账户PerEntity(raw) {
...
export function backfillLodTableNpcState(raw) {
    ...
    for (const k of npcKeys) {
        ...
        const existing = asRec(newLodTable[k]);
        const entry = { 模块键: k, ...existing, 档位: npc['LOD档位'] };
        newLodTable[k] = entry;
...
export function backfillSeedSourcePkgName(raw) {
    ...
    if (!anyChanged)
        return raw; // same reference = complete no-op
    const sys = asRec(raw['_系统']);
    const newSys = { ...sys, migration_version: asNum(sys['migration_version']) + 1 };
...
export function checkS3WriteGate(state, log) {
    const areas = [
        ['NPC', state.NPC],
...
export function checkL3PersonGate(state, log) {
...
export function checkS6UnregisteredHandlerRefs(state, log) {
    ...
    if (sideEffectSet.size === 0 && cascadeSet.size === 0 && interceptorSet.size === 0)
        return;
    ...
    function scanNode(path, node, depth) {
        ...
        if (sideEffectSet.size > 0 && Array.isArray(rec['side_effects'])) {
            for (const h of rec['side_effects']) {
        ...
        if (cascadeSet.size > 0 && Array.isArray(rec['cascade_on_change'])) {
            for (const h of rec['cascade_on_change']) {
        ...
        for (const k of Object.keys(rec).sort()) {
            if (k === 'side_effects' || k === 'cascade_on_change' || k === '解除通道')
                continue;
...
export function migrate(input) {
    ...
    // Mods already tombstoned (self-loop/cascade) are still checked — idempotent merge.
    {
        ...
        if (engineVersion !== '') {
            ...
            const semverTombs = { ...(state._mod墓碑库 ?? {}) };
            let changed = false;
            ...
            if (changed)
                state = { ...state, _mod墓碑库: semverTombs };
        }
    }
    ...
    // Empty registry → flattenedLoadOrder = [] → modPaths = {} → trivially passes.
    {
        ...
        const failing = ['checkA', 'checkB', 'checkC'].filter(k => !dr[k].pass);
        if (failing.length > 0) {
    ...
    const _gwViolations = assertGovernedKeysNormalized(state);
