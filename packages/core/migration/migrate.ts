...
export function checkMotifRegistration(state: RootState, log: MigLog[]): void {
  const entries = state.受治理键空间注册表.键条目 ?? [];
  const motifEntries = entries.filter(e => e.命名空间 === '母题');
  if (motifEntries.length === 0) return; // fast exit: no motif registry → fail-open

  const registeredMotifs = new Set(
    motifEntries.filter(e => e.停用 !== true).map(e => e.规范键),
  );

  function warnMotif(path: string, motif: string): void {
  ...
  // _叙事设置.母题权重 keys
  const 权重 = (state._叙事设置 as unknown as Record<string, unknown> | undefined)?.['母题权重'];
  if (权重 !== null && typeof 权重 === 'object' && !Array.isArray(权重)) {
    for (const k of Object.keys(权重).sort()) warnMotif('_叙事设置.母题权重', k);
  }

  // 全局.母题 (open string)
  const 全局 = state.全局 as unknown as Record<string, unknown> | undefined;
  if (typeof 全局?.['母题'] === 'string' && 全局['母题'] !== '') {
    warnMotif('全局.母题', 全局['母题'] as string);
  }

  // _lore知识库: recursive scan for '母题' and '母题标签' fields
  const lore = state._lore知识库 as unknown as Record<string, unknown> | undefined;
  ...
  function scanNode(path: string, node: unknown, depth: number): void {
    ...
    if (typeof rec['母题'] === 'string' && rec['母题'] !== '') {
      warnMotif(`${path}.母题`, rec['母题'] as string);
    }
    // 母题标签 array
    if (Array.isArray(rec['母题标签'])) {
      (rec['母题标签'] as unknown[]).forEach((m, i) => {
        if (typeof m === 'string' && m !== '') warnMotif(`${path}.母题标签[${i}]`, m);
      });
    }
    // Recurse (skip already-handled keys)
    for (const k of Object.keys(rec).sort()) {
      if (k === '母题' || k === '母题标签') continue;
      scanNode(`${path}.${k}`, rec[k], depth + 1);
    }
  }
...
export function checkDisabledRuleKeyRefs(state: RootState, log: MigLog[]): void {
  const HANDLER_NS = new Set(['sideEffect句柄', 'cascade句柄', '拦截器句柄'] as const);
  const entries = state.受治理键空间注册表.键条目 ?? [];
  const disabledKeys = new Set(
    entries.filter(e => e.停用 === true && !HANDLER_NS.has(e.命名空间 as 'sideEffect句柄' | 'cascade句柄' | '拦截器句柄'))
      .map(e => e.规范键),
  );
  if (disabledKeys.size === 0) return; // fast exit: no disabled non-handler keys
  ...
  function scanNode(path: string, node: unknown, depth: number): void {
    ...
    if (typeof node === 'string') return; // guard (shouldn't reach here via recursion)
    if (Array.isArray(node)) {
      node.forEach((item, i) => {
...
// 绝不 throw；绝不 mutate state；迭代 sorted keys 保证重放恒等。
export function checkPackIdAliases(state: RootState, log: MigLog[]): void {
  const entries = state.受治理键空间注册表.键条目 ?? [];
  const pkgEntries = entries.filter(e => e.命名空间 === 'mod包');
  if (pkgEntries.length === 0) return; // fast exit: no mod包 registry → fail-open

  const registeredPkgs = new Set(
    pkgEntries.filter(e => e.停用 !== true).map(e => e.规范键),
  );

  function warnPkg(path: string, pkgId: string): void {
  ...
  if (延时种子 !== null && 延时种子 !== undefined && typeof 延时种子 === 'object' && !Array.isArray(延时种子)) {
    for (const key of Object.keys(延时种子 as object).sort()) {
      const entry = (延时种子 as Record<string, unknown>)[key];
  ...
  // Scan 行动卡库[*]._来源包
  const 行动卡 = state.行动卡库 as unknown as Record<string, unknown> | undefined;
  if (行动卡 !== null && 行动卡 !== undefined && typeof 行动卡 === 'object' && !Array.isArray(行动卡)) {
    for (const key of Object.keys(行动卡 as object).sort()) {
      const entry = (行动卡 as Record<string, unknown>)[key];
      if (entry !== null && typeof entry === 'object' && !Array.isArray(entry)) {
        const 来源包 = (entry as Record<string, unknown>)['_来源包'];
...
export function migrate(input: unknown): MigrateResult {
  ...
  {
    const populated = populateGoverneKeyRegistry(
      state.mod注册表,
      state.受治理键空间注册表,
      (key, ns, winner, loser) => {
        log.push({
