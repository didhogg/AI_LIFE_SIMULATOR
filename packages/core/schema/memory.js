...
export const 日程Schema = z.record(z.string(), // 槽位键（上午/下午/晚上/自定义）
z.array(意图条目Schema)).default({});
// ══════════════════════════════════════════
// 行动卡库（单源，取代旧双轨行动卡片池）
// ══════════════════════════════════════════
...
// ══════════════════════════════════════════
// 渲染模式枚举（6.69）
export const 渲染模式枚举 = ['直读流', '占位整达', '静默'];
...
export const intervention_pack_v1Schema = z.object({
    ...
    const M3_FORWARD_ONLY = ['编年史.序号', '落账记录.序号'];
    for (const [i, delta] of (data.deltas ?? []).entries()) {
        ...
        if (M3_FORWARD_ONLY.includes(delta.path) && delta.op === 'sub') {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ['deltas', i, 'op'],
