...
export const 社会角色库Schema = z.record(z.string().regex(社会角色ID正则, { message: '社会角色ID 须为蛇形 /^[a-z][a-z0-9_]*$/' }), 社会角色定义条目Schema).default({});
// ── 投影函数（三个·还原旧三表·供等价金测·dormant·不进 hashJudgmentBundle） ──────────────
// 投影→旧 社会角色定义表（record<角色ID, {名称, 描述?}>）
export function 投影社会角色定义表(lib) {
    const result = {};
...
export function 投影社会角色效应量表(lib) {
    const result = {};
    for (const [id, entry] of Object.entries(lib)) {
        if (!Object.prototype.hasOwnProperty.call(lib, id))
            continue;
