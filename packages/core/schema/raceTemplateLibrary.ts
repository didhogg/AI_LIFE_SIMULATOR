...
export const 种族模板库Schema = z.record(
  z.string().regex(种族模板ID正则, { message: '种族模板ID 须为蛇形 /^[a-z][a-z0-9_]*$/' }),
  种族模板定义条目Schema,
).default({});

// ── 种族数据体 Type（投影函数返回类型·与旧 种族模板Schema value 等价） ─────────────────
export type 种族数据体Type = {
  寿命基准: number;
  衰老系数: number;
  发育阶段表: z.infer<typeof 发育阶段Schema>[];
  遗传参数: Record<string, number>;
  最小生育年龄分钟: number;
};
...
export function 投影种族模板库(lib: 种族模板库Type): Record<string, 种族数据体Type> {
  const result: Record<string, 种族数据体Type> = {};
  for (const [id, entry] of Object.entries(lib)) {
    ...
    result[id] = 体 as 种族数据体Type;
  }
  return result;
}

export type 种族模板定义条目Type = z.infer<typeof 种族模板定义条目Schema>;
export type 种族模板库Type       = z.infer<typeof 种族模板库Schema>;
