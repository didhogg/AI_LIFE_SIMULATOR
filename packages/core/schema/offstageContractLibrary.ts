...
export const 离场演化契约库Schema = z.record(
  z.string().regex(离场演化契约ID正则, { message: '离场演化契约ID 须为蛇形 /^[a-z][a-z0-9_]*$/' }),
  离场演化契约定义条目Schema,
).default({});

export type 离场演化契约定义条目Type = z.infer<typeof 离场演化契约定义条目Schema>;
export type 离场演化契约库Type       = z.infer<typeof 离场演化契约库Schema>;
