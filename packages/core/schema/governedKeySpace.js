...
// ══════════════════════════════════════════
export const JS保留键黑名单 = Object.freeze(['__proto__', 'constructor', 'prototype']);
export function 是JS保留键(key) {
    return JS保留键黑名单.includes(key);
}
// ══════════════════════════════════════════
...
// ══════════════════════════════════════════
// 命名空间枚举（封闭·32 项·到此锁定；B2·S4 解锁：+mod包；B6 +拦截器句柄；UI库 +UI组件；工具库 +工具；成就库 +成就；物品库 +物品；媒体库 +媒体；剥离① +学业制式/职级体系/实体模板/文风/二审维度/小剧场剧本；剥离② +选项集；剥离③ +种族模板/战术包/叙事分发/母题词汇/母题配额/离场演化契约/社会角色）
// ══════════════════════════════════════════
export const 命名空间枚举 = [
    '币种', // 引用既有 economy.ts:71 币种定义键·本步不重建
    '单位', // economy.ts:9 嵌币种内·入册为子空间
    '稀有度', // 统一 actor.ts:70 + map.ts:14 两处·消重（取值命名空间·与 '物品' 定义引用命名空间正交）
    ...
    '工具', // 工具库·装配层·路由面·解析器键=工具库·不进 hashJudgmentBundle
    '成就', // 成就库·装配层·定义层·解析器键=成就库·不进 hashJudgmentBundle
    '物品', // 物品库·装配层·定义层·解析器键=物品库·不进 hashJudgmentBundle
    '媒体', // 媒体库·装配层·渲染面+传播面占位·解析器键=媒体库·整库不进 hashJudgmentBundle
    '学业制式', // 学业制式库·装配层·定义层·解析器键=学业制式库·不进 hashJudgmentBundle
    '职级体系', // 职级体系库·装配层·定义层·解析器键=职级体系库·不进 hashJudgmentBundle
    '实体模板', // 实体模板库（冰箱）·装配层·黑洞面·解析器键=实体模板库·不进 hashJudgmentBundle
    '文风', // 文风库（冰箱）·装配层·渲染面 opaque·解析器键=文风库·不进 hashJudgmentBundle
    '二审维度', // 二审维度库·装配层·事实层（检测方式/越界类型 typed）·不进 hashJudgmentBundle
    '小剧场剧本', // 小剧场剧本库·装配层·渲染面 opaque·解析器键=小剧场剧本库·不进 hashJudgmentBundle
    '选项集', // 选项集库·装配层·动词选项条目 by-set 组织·投影回填 动词选项集→进 PRESET 指纹·不进 hashJudgmentBundle
    // ── 剥离③ 新增（记录 data 表·by-ID 冰箱化） ───────────────────────────────────────
    '种族模板', // 种族模板库·装配层·世代钳制·解析器键=种族模板库·进 hashJudgmentBundle（投影回填）
    '战术包', // 战术包库·装配层·战斗辅助·解析器键=战术包库·不进 hashJudgmentBundle
    '叙事分发', // 叙事分发库·装配层·叙事面·解析器键=叙事分发库·不进 hashJudgmentBundle
    '母题词汇', // 母题词汇库·装配层·叙事面·解析器键=母题词汇库·不进 hashJudgmentBundle
    '母题配额', // 母题配额库·装配层·判定面·解析器键=母题配额库·进 hashJudgmentBundle（投影回填）
    '离场演化契约', // 离场演化契约库·装配层·P2 offstageSettler·解析器键=离场演化契约库·不进 hashJudgmentBundle
    '社会角色', // 社会角色库·装配层·定义层·解析器键=社会角色库·不进存档·不进 hashJudgmentBundle
];
// ══════════════════════════════════════════
...
// ══════════════════════════════════════════
export const 仲裁策略枚举 = ['先到先得', '来源包优先', '显式优先级', '报错待人工'];
