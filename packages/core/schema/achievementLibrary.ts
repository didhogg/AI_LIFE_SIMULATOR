...
export const 成就库Schema = z.record(
  z.string().regex(成就ID正则, { message: '成就ID 须为蛇形 /^[a-z][a-z0-9_]*$/' }),
  成就条目Schema,
).default({});

export type 成就条目Type = z.infer<typeof 成就条目Schema>;
export type 成就库Type   = z.infer<typeof 成就库Schema>;
