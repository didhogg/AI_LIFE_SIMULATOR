...
export function getSchemaTopKeys(): Set<string> {
  return new Set(Object.keys(RootSchema.shape));
}

/** Returns the authoritative blueprint key list. */
export function getBlueprintKeys(): Set<string> {
  return new Set<string>(BLUEPRINT_KEYS);
}

/** Keys declared in blueprint 4.0 but absent from RootSchema. */
export function getMissingFromSchema(): string[] {
...
export function getExtraInSchema(): string[] {
