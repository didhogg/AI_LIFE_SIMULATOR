 ...
 */
export function computeDecayFactor(decayRate, tick) {
    if (decayRate <= 0)
        return 1;
 ...
 */
export function deriveEffectivePrice(state, economyRule, regionId, category, formulaConfig) {
 ...
 * @param category     品类键
 */
export function applyDriftCandidate(state, economyRule, regionId, category, formulaConfig) {
    const rule = economyRule;
    if (!rule)
        return;
