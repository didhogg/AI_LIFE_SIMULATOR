...
function validatePath(path) {
    ...
    for (const seg of path.split('.')) {
        if (seg === '') {
            throw new ComputeDeltaError('invalid-path-segment', `路径含空段（连续点号或首尾点号）: 「${path}」`);
        }
        if (是JS保留键(seg)) {
            throw new ComputeDeltaError('reserved-key-segment', `路径段命中 JS 保留键「${seg}」`);
        }
        if (!路径段命名正则.test(seg)) {
            throw new ComputeDeltaError('invalid-path-segment', `路径段「${seg}」不符合命名正则（允许 \\p{L}\\p{N}_）`);
        }
        // Gate③ defense-in-depth: _/$ prefixed segments are B5.6 read-only derived fields.
        if (seg.startsWith('_') || seg.startsWith('$')) {
            throw new ComputeDeltaError('forbidden-prefix', `路径段「${seg}」以 _ 或 $ 开头·只读派生字段禁写`);
        }
    }
}
 ...
 */
export function computeDelta(state, entry, opts) {
    ...
    if (!lookup.found) {
        throw new ComputeDeltaError('path-not-found', `路径「${path}」在当前 state 中不存在·禁 auto-vivify`);
    }
    ...
    if (op === 'set') {
        if (!typesMatch(currentValue, value)) {
            ...
            throw new ComputeDeltaError('type-mismatch', `set op 类型不匹配: 目标「${path}」为 ${cur}·value 为 ${val}`);
        }
        return { path, proposedValue: value };
    }
    // 6. 'add' / 'sub': numeric fields only, integer-safe.
    if (typeof currentValue !== 'number') {
        throw new ComputeDeltaError('type-mismatch', `${op} op 要求目标字段为 number·「${path}」实为 ${typeof currentValue}`);
    }
    if (typeof value !== 'number') {
        throw new ComputeDeltaError('type-mismatch', `${op} op 要求 value 为 number·实为 ${typeof value}`);
    }
    if (!Number.isInteger(value)) {
        throw new ComputeDeltaError('non-integer-value', `${op} op 要求 value 为整数·实为 ${value}（账本=整数文·禁 float 漂移）`);
    }
    ...
    if (max_delta !== undefined) {
        if (!Number.isInteger(max_delta) || max_delta < 0) {
            throw new ComputeDeltaError('non-integer-value', `max_delta 须为非负整数·实为 ${max_delta}`);
        }
 ...
 */
export function setAtPath(state, path, value) {
