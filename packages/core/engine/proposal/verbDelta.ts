// verbDelta — 从 injectedEnvelope 派生 K5 Δ pack 列表（additive·零 schema 改动）
// 触发条件：host 不提供 injectedPacks 且 injectedEnvelope 存在时，由 tick.ts Phase 提案落账调用。
// 本期亲测：仅"转移"支出方向·双方已持有基准币种·纯数字平衡 Δ（和=0）·全程过五道闸。
// 后续 additive：收款方向 / 多币种 / 缺键 gated init / 其余 9 动词实装。
// callRegistry 非 effect seam → switch(动作类别) + TODO 注释（future-registry: G4 就绪时迁移）。
...
function derive转移(
  ...
  if (!receiver || !seatId || receiver === seatId) return [];
  ...
  const accounts = (state.货币系统?.账户 ?? {}) as Record<
    string,
    { 持有: Record<string, number> }
  >;
  const payerHolding  = accounts[seatId]?.持有 ?? {};
  const receiverHolding = accounts[receiver]?.持有 ?? {};
  if (!(ccy in payerHolding) || !(ccy in receiverHolding)) return [];
...
function derive赋予(
  ...
  if (!recipient || !seatId || recipient === seatId) return [];
  ...
  if (!(ccy in sinkHolding) || !(ccy in recipientHolding)) return [];
