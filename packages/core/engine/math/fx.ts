...
/** 发起方视角：'buy' = 花出基准/源币买入目标；'sell' = 卖出源币换取基准/目标 */
...
export function convertFx(
  ...
  if (amount <= 0) throw new Error(`convertFx: amount 必须 > 0，收到 ${amount}`);

  // 取基准汇率（缺省 = 1 表示该币种就是基准）
  const rateFrom = rateMap[from] ?? 1;
  const rateTo   = rateMap[to]   ?? 1;

  if (rateFrom <= 0) throw new Error(`convertFx: 无效汇率 [${from}] = ${rateFrom}`);
  if (rateTo   <= 0) throw new Error(`convertFx: 无效汇率 [${to}] = ${rateTo}`);
 ...
 */
export function assertNoDirectCrossRate(directPairs: string[]): void {
  if (directPairs.length > 0) {
    throw new Error(
      `H3 汇率单锚约束违反：禁止直连汇率对，拒收：${directPairs.join(', ')}。所有换算必须经由基准币种两跳。`,
    );
  }
}
