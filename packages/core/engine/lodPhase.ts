...
import { locRegion, computeResourceFactor, type LocRecord } from './tick.js';
import {
  promoteNode,
  tryDemoteNode,
  handleRegionCross,
 ...
 * 结果 clamp 至 [-1,1]。
 */
export function resolveSensitivity(preset: 玩法预设Type | undefined, nodeKey: string): number {
  const strategy = preset?.模块绑定策略;
  if (!strategy) return 0;
  const perModule = strategy[nodeKey]?.敏感度;
  if (perModule !== undefined) return Math.max(-1, Math.min(1, perModule));
  const global = strategy['*']?.敏感度;
  if (global !== undefined) return Math.max(-1, Math.min(1, global));
  return 0;
}
 ...
 */
export function resolveTriggerPred(preset: 玩法预设Type | undefined, nodeKey: string): string | undefined {
  const strategy = preset?.模块绑定策略;
...
function buildLodDriftCtx(
  ...
  const 漂移: Record<string, number> = { 资源紧张度: rawDrift * sensMultiplier };
...
export function scheduleLodPhase(
  ...
  // ── B2.5 · Pass 1: PC 在场候选 + 条件④ 谓词驱动连续计数更新 ──
  // 条件④ promote 唯一路径 = detectLodTrigger 连续偏离分支（偏离③ 已删除）
  // pass1 drift 分支仅更新连续计数，不推 promoteCandidates
  ...
  const driftCounterUpdates: Array<{ nodeKey: string; newCount: number; newBaseline?: number }> = [];

  for (const nodeKey of Object.keys(s.LOD表)) {
    if (!locs[nodeKey]) continue; // LOD-B4b: NPC 键跳过
    ...
    if (pcPresent) {
      ...
      let newCount: number;
      let newBaseline: number | undefined;
