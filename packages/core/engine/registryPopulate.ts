...
import type {
  受治理键空间注册表Type,
  键条目Type,
} from '../schema/governedKeySpace.js';

// ─── Local mod shape needed for populate (subset of mod条目Schema) ─────────────

interface ModPopEntry {
  pack_id: string;
  启用?: boolean | undefined;
  优先级?: number | undefined;
  命名空间键声明?: 键条目Type[] | undefined;
}
...
export function populateGoverneKeyRegistry(
  modRegistry: Record<string, ModPopEntry>,
  existingRegistry: 受治理键空间注册表Type,
  onConflict?: (key: string, ns: string, winner: string, loser: string) => void,
): 受治理键空间注册表Type {
  ...
  const derived = new Map<string, 键条目Type>();
  ...
  for (const mod of sortedMods) {
    ...
    for (const decl of mod.命名空间键声明 ?? []) {
      ...
      if (existingSet.has(key)) continue; // hand-crafted entries always win; no conflict log
      if (derived.has(key)) {
  ...
  if (derived.size === 0) return existingRegistry;
