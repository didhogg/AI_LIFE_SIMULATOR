...
export function pickMutuallyExclusive(candidates, _tickId, tickCount, channel, salt) {
    if (candidates.length === 0)
        throw new Error('互斥组候选为空');
...
export function rollProbability(probability, // [0,1]
_tickId, tickCount, channel, salt) {
    if (probability <= 0)
        return false;
    if (probability >= 1)
        return true;
