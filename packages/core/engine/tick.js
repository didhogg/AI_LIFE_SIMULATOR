...
import { locRegion, buildRegionGraph, bfsRegionHops } from './regionGraph.js';
...
function _buildTickFP(config) {
    const r = (k) => resolveFormula(k, config);
    return {
...
const HIERARCHY_EDGE_TYPES_DEFAULT = new Set(['层级', '隶属']);
...
export function runTick(state, input) {
    ...
    const _formulaConfigBase = { enabled: true, ctx: {} };
    const _formulaConfig = Object.assign(_formulaConfigBase, input.formulaPresetConfig !== undefined
        ? { presetNumbers: input.formulaPresetConfig }
        : undefined, _dslState?.公式override表 !== undefined
    ...
    runPhase('阈值触发', () => {
        if (!input.effectPacks?.length)
            return; // 金向量短路守卫：undefined/[] → 精确 no-op
        for (const pack of input.effectPacks) {
            ...
            if (r.resolved.some(d => M3_HARD_EXCLUDED_PREFIXES.some(p => d.path.startsWith(p))))
                continue;
    ...
    runPhase('关系触发', () => {
        ...
        for (const [npcKey, npc] of Object.entries(s.NPC)) {
            for (const rel of npc.关系) {
                ...
                if (score < _fp.relRippleThreshold)
                    continue;
    ...
    runPhase('扩展参数播种', () => {
        ...
        for (const npcKey of Object.keys(s.NPC).sort()) { // 码点序·非 localeCompare
            ...
            for (const itemKey of Object.keys(npc.物品).sort()) { // 码点序·非 localeCompare
                ...
                seedExtensionParams(itemInstance.扩展参数, itemDef.变量模板);
            }
        }
    });
 ...
 */
function buildOrgChildGraph(orgNet, tp) {
    const children = new Map();
    if (!orgNet)
        return children;
    for (const edge of Object.values(orgNet)) {
        if (!edge.边类型 || !tp.组织层级边类型集.has(edge.边类型))
            continue;
 ...
 */
function appendToChronicle(s, nowEpochMin, fp) {
    const 编年史 = s.全局?._编年史;
    if (!编年史)
        return;
