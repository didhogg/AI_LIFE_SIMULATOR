 ...
 */
export function promoteNode(state, nodeKey, seed) {
 ...
 */
export function checkWarmWindow(state, nodeKey, currentTick) {
 ...
 * 同一地点 → false（优化短路）。
 */
export function detectCrossRegion(state, prevLocKey, curLocKey) {
    if (prevLocKey === curLocKey)
        return false;
    const locs = state.地图?.地点 ?? {};
 ...
 */
export function detectLodTrigger(state, prev, cur) {
    const locs = state.地图?.地点 ?? {};
    // ① 位置跨区
 ...
 */
export function executeTraversal(state, fromDomainId, toDomainId, pcKey, contract) {
    // ① 切世界域
    const fromDomain = state.世界域[fromDomainId];
    if (fromDomain)
        fromDomain.封存状态 = true;
 ...
 */
export function isCrossDomainAccess(sourceDomain, currentDomainId) {
