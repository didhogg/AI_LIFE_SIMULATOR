 ...
 */
export function openSegment(存档头, params) {
    ...
    const hashParams = { 前段哈希 };
    if (params.引擎版本 !== undefined)
        hashParams.引擎版本 = params.引擎版本;
    if (params.Schema版本 !== undefined)
        hashParams.Schema版本 = params.Schema版本;
    if (params.难度系数组指纹 !== undefined)
        hashParams.难度系数组指纹 = params.难度系数组指纹;
    const 段头指纹 = computeSegmentHeadHash(hashParams);
    const newSeg = { 段序号, 段头指纹, 前段哈希 };
    if (params.引擎版本 !== undefined)
        newSeg.引擎版本 = params.引擎版本;
    if (params.Schema版本 !== undefined)
        newSeg.Schema版本 = params.Schema版本;
    if (params.难度系数组指纹 !== undefined)
        newSeg.难度系数组指纹 = params.难度系数组指纹;
 ...
 */
export function verifySegmentChain(存档头) {
    const records = 存档头.版本段记录 ?? [];
 ...
 */
export function shouldOpenNewSegment(存档头, current) {
    const records = 存档头.版本段记录 ?? [];
    if (records.length === 0)
        return true;
    const last = records[records.length - 1];
    return (last.引擎版本 !== current.引擎版本 ||
