 ...
 */
export function freezeImportanceScore(currentEntry, newScore) {
    if (currentEntry.权重_冻结) {
        throw new Error(`L-21 违规: 记忆重要度分已冻结（当前值: ${currentEntry.权重 ?? 50}），禁止重算`);
    }
 ...
 */
export function assertImportanceFrozen(entry, memoryId) {
    if (!entry.权重_冻结) {
        throw new Error(`L-21 守卫: 记忆「${memoryId}」重要度分未冻结（权重: ${entry.权重 ?? 50}）——创建流程须先调 freezeImportanceScore`);
    }
}
/**
 * 安全读取已冻结重要度分。
 ...
 */
export function readFrozenScore(entry, defaultValue = 50) {
    if (!entry.权重_冻结)
        return defaultValue;
