...
export function projectStateCtx(
  ...
  const 自定义变量Rec: Record<string, number> = {};
  if (npc?.扩展参数) {
    for (const [k, v] of Object.entries(npc.扩展参数)) {
      if (typeof v === 'number') 自定义变量Rec[k] = v;
    }
  }
  const 自定义变量: Readonly<Record<string, number>> = 自定义变量Rec;
  ...
  const LOD态: Readonly<Record<string, number>> = LOD态Rec;
