 ...
 */
export function resolveEffectivePredicate(
  完整键: string,
  base: string,
  全局开关: boolean,
  作者控制表: Readonly<Record<string, boolean>> | undefined,
  玩家控制表: Readonly<Record<string, boolean>> | undefined,
  override表: Readonly<Record<string, string>> | undefined,
): string {
  // ① 作者红线：false 不可破（玩家覆盖无效）
  if (作者控制表?.[完整键] === false) return base;
  // ② 三态叠加：玩家 > 作者 > 全局
  const 有效开关: boolean = 玩家控制表?.[完整键] ?? 作者控制表?.[完整键] ?? 全局开关;
 ...
 */
export function readGlobalDslSwitch(功能开关表: Record<string, unknown>): boolean {
