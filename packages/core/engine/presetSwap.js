...
export function swapPreset(state, newPreset, opts = {}, resolvedRules) {
    ...
    if (opts.engineVersion !== undefined)
        segParams.引擎版本 = opts.engineVersion;
    if (opts.schemaVersion !== undefined)
        segParams.Schema版本 = opts.schemaVersion;
    // 构造精确类型的视图（避免 Zod 推断的 T|undefined optional 与 VersionSegmentHead 的类型冲突）
    const prevSegsTyped = prevSegRecords;
    const segView = prevSegsTyped.length > 0 ? { 版本段记录: prevSegsTyped } : {};
    const needNewSeg = shouldOpenNewSegment(segView, segParams);
    const newSegRecords = needNewSeg
        ? openSegment(segView, segParams)
        : prevSegsTyped;
    // ⑥ 验链（断链=D4警示·不throw）
    const chainResult = verifySegmentChain({ 版本段记录: newSegRecords });
    if (!chainResult.valid) {
        warnings.push(`[D4] 版本段哈希链断链 brokenAt=${chainResult.brokenAt}: ${chainResult.message}`);
    }
 ...
 */
export function unloadPreset(state, presetId) {
    // 幂等：presetId 未在任何域挂载 → no-op
    const isMounted = Object.values(state.世界域).some((d) => d.玩法预设引用 === presetId);
    if (!isMounted)
        return state;
    // dangling fail-closed：检测卸载后会暴露的孤儿引用
    const danglingRefs = findDanglingRefs(state, presetId);
    if (danglingRefs.length > 0) {
        throw new Error(`[PR-1] unloadPreset: preset "${presetId}" 有孤儿引用，拒绝卸载。refs: ${danglingRefs.join(' | ')}`);
    }
    return {
        ...state,
        世界域: Object.fromEntries(Object.entries(state.世界域).map(([k, v]) => [
            k,
 ...
 */
function removePresetFromRegistry(registry, presetId) {
    const entries = registry.键条目 ?? [];
    const filtered = entries.filter((e) => e.来源包 !== presetId);
    if (filtered.length === entries.length)
        return registry; // nothing removed
