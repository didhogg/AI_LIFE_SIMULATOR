...
const _INDIRECT_FACTOR_DEFAULT: number = fixedExp(-0.6931471805599453);

export const FORMULA_REGISTRY: Readonly<Record<FormulaPointKey, FormulaPointDescriptor>> = {
  // ── tick.ts R1 社会动力学 13 ─────────────────────────────────────────────────
...
/**
 * 公式 override 解析配置（caller 构建后传入各引擎函数）。
 * presetNumbers: 作者预设数字（来自 resolve() formulaPresetConfig·typed 数字）
 * playerDsl:     玩家 DSL 串（来自 $AI创作状态.公式override表）
 * enabled:       全局开关（缺省 true）
 * ctx:           DSL 求值上下文（缺省空 → 常量公式正常·路径引用返回 0 fail-safe）
 */
export interface FormulaResolveConfig {
  presetNumbers?: Partial<Record<FormulaPointKey, number>>;
  playerDsl?: Readonly<Record<string, string>>;
  enabled?: boolean;
  ctx?: DslContext;
}
 ...
 */
export function resolveEffectiveFormula(
  key: FormulaPointKey,
  defaultValue: number,
  presetConfigNumber: number | undefined,
...
export function resolveFormula(
  key: FormulaPointKey,
  config?: FormulaResolveConfig,
): number {
  const desc = FORMULA_REGISTRY[key];
  if (!config) return desc.defaultValue;
  return resolveEffectiveFormula(
    key,
    desc.defaultValue,
    config.presetNumbers?.[key],
