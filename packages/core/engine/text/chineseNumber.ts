...
/**
 * 规范货币单位集合（可直接与提案 amount 比对）。
 * 仅「文」与「文钱」在 slice 世界中等价且无换算歧义。
 */
 ...
 *
 * 两段抽取：
 *   段①  number + (文钱|文)    → 规范单位，isCanonicalUnit = true
 *   段②  number + 其他货币字符  → 不可确认单位，isCanonicalUnit = false
 *   段①覆盖的文本位置不参与段②。
 *
...
export function extractMoneyAmounts(text: string): MoneyAmount[] {
  ...
  // 段① 规范单位（文/文钱）
  const re1 = new RegExp(`([${VALID_CHARS}]+)\\s*(文钱|文)`, 'g');
  ...
  // 段② 不可确认单位
  const re2 = new RegExp(`([${VALID_CHARS}]+)\\s*([${UNCONFIRMED_UNIT_CHARS}]+)`, 'g');
