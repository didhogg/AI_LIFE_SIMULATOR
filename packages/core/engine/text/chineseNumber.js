...
export const CHINESE_NUMBER_RULE_VERSION = 3;
// ── 版本 3：入口归一 ──────────────────────────────────────────────────────────
 ...
 */
export function prepareNarrative(raw) {
...
export const RE_CHANGE_GIVING = /找(?:[你我他她们]|了?零)|退(?:[你我他她]|给)/;
/**
 * 债权性质词：赊账/欠款/欠了/欠着/借了/借给 → 非即时现金，不得记为普通现金支出。
 * 注：匹配精准，避免"借此/借道"等非金融"借"。
 */
export const RE_DEBT = /赊账|赊[了的]|欠款|欠[了着的]|借[了给]/;
/** 代付性质词：垫付/代付/先垫/帮付 → 属代付/债权，不记普通现金。 */
export const RE_ADVANCE = /垫付|代付|先垫|帮付/;
/**
 * 总价/合计词：共/一共/共计/合计/总计/总价 + 紧随数字 → 疑似合计，防与分项重复计数。
 * 用 lookahead 限制：后面必须紧跟数字/汉字数字，排除"共同/共享"等用法。
 */
export const RE_TOTAL = /(?:一共|共计|合计|总计|总价|共)\s*(?:[约计为]?\s*)(?=[\d零〇一壹二贰两俩三叁四肆五伍六陆七柒八捌九玖十拾百佰千仟万萬])/;
// ── 版本 2：单位分类 ──────────────────────────────────────────────────────────
/**
 ...
 */
export const CANONICAL_UNITS = new Set(['文', '文钱']);
...
export function isCanonicalUnitIn(unit, canonicalUnits) {
    return canonicalUnits.has(unit);
}
// ── parseChineseAmount（版本 1 规则不变）────────────────────────────────────
export function parseChineseAmount(token) {
    if (!token)
        return null;
    let total = 0;
    let segment = 0;
 ...
 */
export function extractMoneyAmountsFor(text, canonicalUnits = CANONICAL_UNITS, unconfirmedUnitChars = UNCONFIRMED_UNIT_CHARS) {
