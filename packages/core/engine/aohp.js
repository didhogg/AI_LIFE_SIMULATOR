...
export function buildMenuOptionIds(options) {
    ...
    const annotated = options.map(opt => {
        ...
        const { displayText: _dt, salientArgs: _sa, ...restFields } = opt;
    ...
    const keyGroups = new Map(); // baseKey → Set<payloadHash>
    for (const { baseKey, payloadHash } of annotated) {
        const group = keyGroups.get(baseKey) ?? new Set();
 ...
 */
export function sortedOptionIds(options) {
