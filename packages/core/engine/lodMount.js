...
/** 幂等注册（同键覆盖）。 */
export function registerLodMount(descriptor) {
    _registry.set(descriptor.模块键, descriptor);
}
/** 清空注册表（仅供测试隔离使用）。 */
export function clearLodRegistry() {
    _registry.clear();
}
/** 按模块键查找描述符（测试用）。 */
export function getLodMount(模块键) {
