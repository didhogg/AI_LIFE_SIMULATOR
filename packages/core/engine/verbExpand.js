...
import { evalPred } from './dsl/eval.js';
/** 通配符常量（展开到全部实体键） */
 ...
 */
export function expandVerbTarget(slot, entityKeys, entityContextResolver) {
    if (!slot)
        return [];
 ...
 */
export function applyVerbToTargets(expandedTargets, applyFn, acc) {
    let result = acc;
