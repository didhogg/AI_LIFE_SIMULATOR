...
export const M3_HARD_EXCLUDED_PREFIXES = Object.freeze(['_', '$']);
...
export const DELTA_OPS = ['set', 'add', 'sub', 'clamp', 'lock'];
/**
 * 判定路径是否命中 M3 硬排除：首段以「_」或「$」开头即拒收。
 * 纯函数·确定性·无副作用。
 */
export function isM3HardExcluded(path) {
 ...
 * 纯函数·确定性·无副作用。
 */
export function isM3ForwardOnlyViolation(path, op) {
    return M3_FORWARD_ONLY_PATHS.includes(path) && op === 'sub';
}
/**
 ...
 */
export function getM3Violation(path, op, oldValue, newValue) {
    ...
    if (M3_FORWARD_ONLY_PATHS.includes(path) &&
        op === 'set' &&
        oldValue !== undefined &&
        newValue !== undefined) {
