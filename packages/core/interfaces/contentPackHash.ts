...
/** 已启用包集合条目形状（B6 消费时替换为具体联合类型）*/
export type 已启用包条目 = { readonly content_hash?: string };
export type 已启用包集合 = ReadonlyArray<已启用包条目>;

/**
 * 聚合全部已启用包的 content_hash，产出「生效中内容包集哈希」。
 * 空集合或全部 content_hash 缺失 → '' (fail-open 确定性占位)。
 */
export function 聚合生效中内容包集哈希(packs: 已启用包集合): string {
  const hashes = [
    ...new Set(
      packs
 ...
 */
export function computeEffectPackHash(pack: Record<string, unknown>): string {
  const rest: Record<string, unknown> = {};
  for (const key of Object.keys(pack)) {
    if (key !== 'content_hash') rest[key] = pack[key];
  }
