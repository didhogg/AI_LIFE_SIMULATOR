...
function normAliasArray(aliases) {
    if (!Array.isArray(aliases))
        return { value: aliases, changed: false };
    let changed = false;
...
function normalizeRegistryEntries(registry) {
    ...
    const newArr = arr.map((entry) => {
        ...
        if (!keyChanged && !aliasChanged)
            return entry;
        changed = true;
        const updated = { ...entry, 规范键: normKey };
        if (aliasChanged)
            updated['别名'] = normAlias;
        return updated;
    });
    if (!changed)
        return { value: registry, changed: false };
...
function normalizeMergeEntries(mergeTable) {
    if (!isRec(mergeTable))
        return { value: mergeTable, changed: false };
 ...
 * 对既有已规范化数据为 no-op → 指纹/黄金向量零漂移。
 */
export function normalizeRegistryKeyNames(raw) {
...
function collectRegViolations(registry, out) {
    if (!isRec(registry))
        return;
    const arr = registry['键条目'];
...
function collectMergeViolations(mergeTable, out) {
    if (!isRec(mergeTable))
        return;
    const arr = mergeTable['归并条目'];
    if (!Array.isArray(arr))
 ...
 */
export function assertGovernedKeysNormalized(raw) {
    const violations = [];
