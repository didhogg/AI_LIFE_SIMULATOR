...
function mergeMaxDelta(a: number | undefined, b: number | undefined): number | undefined {
 ...
 */
export function mergeInterventionDeltas(
  packsInLoadOrder: ReadonlyArray<ReadonlyArray<K5DeltaEntry>>,
): K5DeltaEntry[] {
  // Map key = "${path}\0${op}" — null byte cannot appear in valid path/op strings.
  const merged = new Map<string, K5DeltaEntry>();
