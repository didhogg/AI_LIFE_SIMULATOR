 ...
 */
export function isLegalCharTransition(
  from: string | undefined,
  to: string,
  opts: { hasRevivalFlag: boolean },
): boolean {
  if (from === undefined) return true;
  if (from === '在世' && (to === '失踪' || to === '已故')) return true;
  if (from === '失踪' && (to === '在世' || to === '已故')) return true;
  if (from === '已故' && to === '在世') return opts.hasRevivalFlag;
 ...
 * - 持有/遗失 同态写：合法（幂等）
 */
export function isLegalItemTransition(
  from: string | undefined,
  to: string,
): boolean {
  if (from === undefined) return true;
  if (from === '销毁') return false;
  if (from === '持有' && (to === '遗失' || to === '销毁')) return true;
  if (from === '遗失' && (to === '持有' || to === '销毁')) return true;
  if (from === to) return true;
  return false;
}
