...
export type ValidOverwriteAuthSource = (typeof VALID_OVERWRITE_AUTH_SOURCES)[number];

// 天命通道来源标志：mod 宣称此授权源但走普通通道即为越权
export const DESTINY_CHANNEL_SOURCE = '天命' as const;

// ── M2 判定结果类型 ────────────────────────────────────────────────────────────
export type M2ViolationResult =
  | { readonly violation: false }
  | { readonly violation: true; readonly reason: '覆写授权越权'; readonly 诊断: string };
 ...
 */
export function checkM2Violation(授权源: string): M2ViolationResult {
  if ((VALID_OVERWRITE_AUTH_SOURCES as readonly string[]).includes(授权源)) {
...
// ── 墓碑条目类型（与 mod墓碑条目Schema strict() 兼容·不 import schema） ────────────
export type M2TombstoneEntry = {
  readonly 记录键: string;
 ...
 * 相同输入恒同输出（幂等）。
 */
export function writeM2Tombstone(
  记录键: string,
  pack_id?: string,
  诊断?: string,
): M2TombstoneEntry {
