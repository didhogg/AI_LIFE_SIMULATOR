...
describe('P7.b-1 · number 直通', () => {
    it('单条 number delta 原样输出', () => {
        const r = resolveDeltaValues([{ path: '属性.体质', op: 'add', value: 5 }], CTX);
    ...
    it('max_delta 保留', () => {
        const r = resolveDeltaValues([{ path: 'x', op: 'set', value: 10, max_delta: 3 }], CTX);
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
...
describe('P7.b-3 · DSL path 表达式', () => {
    it('属性.体质 → 60（路径求值）', () => {
        const r = resolveDeltaValues([{ path: 'some.path', op: 'set', value: '属性.体质' }], CTX);
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
    ...
    it('账户.文 → 500', () => {
        const r = resolveDeltaValues([{ path: 'some.path', op: 'add', value: '账户.文' }], CTX);
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
...
describe('P7.b-4 · DSL 算术表达式', () => {
    it('属性.体质 * 2 → 120', () => {
        const r = resolveDeltaValues([{ path: 'p', op: 'set', value: '属性.体质 * 2' }], CTX);
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
    ...
    it('属性.体质 + 属性.智慧 → 100', () => {
        const r = resolveDeltaValues([{ path: 'p', op: 'add', value: '属性.体质 + 属性.智慧' }], CTX);
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
    ...
    it('字面数值串 50 → 50（整数 token）', () => {
        const r = resolveDeltaValues([{ path: 'p', op: 'set', value: '50' }], CTX);
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
...
describe('P7.b-5 · 解析失败 → ok:false', () => {
    it('无效表达式串 → ok:false', () => {
        const r = resolveDeltaValues([{ path: 'p', op: 'set', value: '!!! 无效 DSL' }], CTX);
...
describe('P7.b-6 · 除零保护', () => {
    it('属性.体质 / 0 → 0（DSL v1 除零→0·isFinite·ok:true）', () => {
        const r = resolveDeltaValues([{ path: 'p', op: 'set', value: '属性.体质 / 0' }], CTX);
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
...
describe('P7.b-7 · max_delta 保留（DSL value path）', () => {
    it('string value + max_delta → resolved 保留 max_delta', () => {
        const r = resolveDeltaValues([{ path: 'p', op: 'add', value: '属性.体质', max_delta: 20 }], CTX);
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
        expect(r.resolved[0]?.value).toBe(60);
