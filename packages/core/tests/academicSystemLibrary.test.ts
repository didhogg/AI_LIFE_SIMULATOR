...
import {
  ...
  学业制式库Schema,
  学业制式ID正则,
} from '../schema/academicSystemLibrary.js';
...
function mkAcademic(name: string, overrides: Partial<学业制式定义条目Type> = {}): 学业制式定义条目Type {
  return { 名称: name, ...overrides };
}

function 条目ToPackEnvelope(entry: 学业制式定义条目Type): Record<string, unknown> {
...
describe('学业制式库 · 学业制式定义条目Schema · 独立 parse', () => {
  it('最小条目（仅 名称）→ parse 成功', () => {
    ...
    if (r.success) expect(r.data.名称).toBe('初级学业');
  });
  it('缺 名称 → parse 失败', () => {
...
describe('学业制式库 · resolve 挂载 + by-ID 加载', () => {
  ...
  it('冰箱中 toString 作 handle → 解引用 own-property guard 拦截', () => {
    ...
    const 成品: Record<string, unknown> = { 学业制式库: { imperial_exam: entry } };
    const ref = 创建引用('学业制式', 'imperial_exam');
    expect(解引用(ref, 成品)).toBeDefined();
    const protoRef = { __ns: '学业制式' as const, handle: 'toString' };
...
describe('学业制式库 · 不进判定面指纹', () => {
  ...
  it('改学制条目 → hashPresetFingerprint 逐位恒等', () => {
    const lib1: 学业制式库Type = { exam: mkAcademic('科举', { 时长分钟: 1000 }) };
    const lib2: 学业制式库Type = { exam: mkAcademic('科举', { 时长分钟: 9999 }) };
  ...
  it('加学制条目 → hashPresetFingerprint 逐位恒等（证整库不进判定面）', () => {
    const lib: 学业制式库Type = { exam: mkAcademic('科举'), guild: mkAcademic('行会') };
...
// ═══════════════════════════════════════════════════════════════
describe('学业制式库 · 守恒门', () => {
  it('schemaKeys = 53（学业制式库不进 RootSchema·不改顶层键数）', () => {
  ...
  it('命名空间枚举 = 32 項（含学业制式）', () => {
    ...
    expect((命名空间枚举 as readonly string[]).includes('学业制式')).toBe(true);
  });
