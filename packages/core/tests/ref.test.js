...
describe('R4 · 非法命名空间被拒', () => {
    it('引用Schema 传非法命名空间 → throw at schema creation', () => {
        expect(() => 引用Schema('非法命名空间')).toThrow(/非法命名空间/);
    });
    it('创建引用 传非法命名空间 → throw', () => {
        expect(() => 创建引用('不存在的空间', 'handle')).toThrow(/非法命名空间/);
    });
...
describe('R1 · 命中解引用', () => {
    ...
    it('解引用 不同 handle → 返回不同条目', () => {
        ...
        expect(解引用(refA, 成品)['名称']).toBe('基础包');
        expect(解引用(refB, 成品)['名称']).toBe('扩展包');
    });
...
describe('R2 · 未命中守 AA3（返 null，非 strict）', () => {
    ...
    it('冰箱值为 null → 返 null', () => {
        const ref = 创建引用('mod包', 'pack_a');
        expect(解引用(ref, { mod注册表: null })).toBeNull();
    });
...
describe('R3 · 成环不死循环（引用图允许环·解引用单步不递归）', () => {
    it('ref_a ↔ ref_b 相互引用 → 解引用单步查找·不递归·不死循环', () => {
        const ref_a = 创建引用('mod包', 'entry_a');
        const ref_b = 创建引用('mod包', 'entry_b');
        ...
        expect(resultA['引用']).toEqual(ref_b);
        expect(resultB['引用']).toEqual(ref_a);
    });
    it('深链：a→b→c（三节点链）·解引用每步独立·无 stack overflow', () => {
        ...
        expect(解引用(ref_a, 成品)['id']).toBe('a');
        expect(解引用(ref_b, 成品)['id']).toBe('b');
        expect(解引用(ref_c, 成品)['id']).toBe('c');
    });
});
// ── R7 · 确定性（纯函数·同输入恒等输出）──────────────────────────────────────
describe('R7 · 确定性', () => {
    ...
    it('解引用 多次调用·同输入 → 输出相等', () => {
        const ref = 创建引用('mod包', 'pack_a');
        const 成品 = { mod注册表: { pack_a: { ver: '1.0' } } };
        const r1 = 解引用(ref, 成品);
        const r2 = 解引用(ref, 成品);
        expect(r1).toEqual(r2);
    });
