...
import { parseChineseAmount, extractMoneyAmounts, prepareNarrative, isCanonicalUnit, CANONICAL_UNITS, RE_CHANGE_GIVING, RE_DEBT, RE_ADVANCE, RE_TOTAL, CHINESE_NUMBER_RULE_VERSION, } from '../engine/text/chineseNumber.js';
// ── 版本常量 ──────────────────────────────────────────────────────────────────
describe('规则版本', () => {
...
describe('RE_TOTAL：总价词 + 数字 lookahead', () => {
    ...
    it.each([
        '大家共同出资', '共享', '共欢乐',
    ])('"%s" 不命中（数字 lookahead 未匹配）', (s) => expect(RE_TOTAL.test(s)).toBe(false));
});
// ── 单位分类 ──────────────────────────────────────────────────────────────────
...
describe('extractMoneyAmounts: 阿拉伯 + 文（不回归）', () => {
    ...
    it('去重', () => {
        ...
        expect([...r].sort((a, b) => a.value - b.value))
            .toEqual([{ value: 2, unit: '文' }, { value: 3, unit: '文' }]);
    });
...
describe('extractMoneyAmounts: 汉字金额 + 文（M2.5 不回归）', () => {
    ...
    it('混合两文…10文 → 两条规范记录', () => {
        ...
        expect([...r].sort((a, b) => a.value - b.value))
            .toEqual([{ value: 2, unit: '文' }, { value: 10, unit: '文' }]);
    });
});
