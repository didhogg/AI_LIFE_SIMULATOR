...
import { dispatch, assertInvariants, assertLlmNonBlockingTopology, _runTopologyCheckFor, isSafeSeam, SMTransitionError, LLM_WAIT_MODALS, } from '../engine/stateMachine.js';
// ── Helpers ────────────────────────────────────────────────────────────────────
function base() {
    return {
        当前态: 'PLAYING',
...
function go(s, type, extra) {
    return dispatch(s, { type, ...extra }).新机器状态;
}
function effects(s, ev) {
...
describe('P0-4 RP_FOCUS 双时钟', () => {
    ...
    it('退出RP焦点 → effect 世界钟对齐镜头钟，elapsed 正确', () => {
        ...
        const afterLensAdvance = {
            ...inRP,
            双时钟: { 世界钟: 100, 镜头钟: 350 },
        };
        const fx = effects(afterLensAdvance, { type: '退出RP焦点' });
        const syncEff = fx.find(f => f.type === '世界钟对齐镜头钟');
...
describe('P0-4 LIFE_SUMMARY 终态', () => {
    ...
    it('HOST事件 in LIFE_SUMMARY → 通过（终态唯一合法事件）', () => {
        const terminal = { ...base(), 当前态: 'LIFE_SUMMARY' };
...
describe('P0-4 assertInvariants', () => {
    ...
    it('DEATH_INTERCEPT 是合法基础态（不违反 Invariant 1）', () => {
        const s = { ...base(), 当前态: 'DEATH_INTERCEPT' };
        expect(() => assertInvariants(s)).not.toThrow();
    });
    it('模态栈含未知模态 → invariant 2 抛错', () => {
        const bad = { ...base(), 模态栈: ['UNKNOWN_MODAL'] };
    ...
    it('双时钟非整数 → invariant 4 抛错', () => {
        const bad = { ...base(), 双时钟: { 世界钟: 1.5, 镜头钟: 0 } };
        ...
        for (let round = 0; round < 200; round++) {
            ...
            for (let step = 0; step < 30; step++) {
                ...
                try {
                    ...
                    if (!(e instanceof SMTransitionError))
                        throw e;
                }
            }
        }
...
describe('P0-4 F2·拍生命周期单飞锁（6.56）', () => {
    ...
    it('完整生命周期 + assertInvariants 每步均通过', () => {
        let s = base();
        for (const ev of ['拍推进', '结算完成', '呈现确认', '选择完成', '关账完成']) {
            s = go(s, ev);
            expect(() => assertInvariants(s)).not.toThrow();
        }
    });
...
describe('P0-4 F1·元层写排队（6.56·M5 收紧版）', () => {
    ...
    it('指令组边界 → 队列整批交宿主 + 清空队列', () => {
        ...
        const batchFx = result.效果指令.find(f => f.type === '执行元层写队列');
    ...
    it('UI 回执效果含操作名称与幂等键', () => {
        ...
        const ui = result.效果指令.find(f => f.type === '元层写UI回执');
...
describe('P0-4 F6·同类型模态禁重入（6.56）', () => {
    ...
    it('切换对话对象 in RP_FOCUS → 不触模态栈（模态内切换）+ 切换RP焦点对象效果', () => {
        ...
        const fx = result.效果指令.find(f => f.type === '切换RP焦点对象');
...
describe('P0-4 C4·席位掉线 AI 托管降级接口（6.53）', () => {
    it('席位掉线 → 发起AI托管降级效果（状态机不变·单机路径宿主 no-op）', () => {
        ...
        const fx = result.效果指令.find(f => f.type === '发起AI托管降级');
    ...
    it('席位回连 → 交还席位效果', () => {
        ...
        const fx = result.效果指令.find(f => f.type === '交还席位');
...
describe('P0-4 D2·死亡时刻双登记接口（6.54）', () => {
    it('主角死亡 → 效果含 双登记死亡时刻（全局时刻=世界钟·域钟读数=镜头钟）', () => {
        ...
        const dual = fx.find(f => f.type === '双登记死亡时刻');
...
describe('P0-4 G1·元层开关走组边界（I-a·专项）', () => {
    ...
    it('同一拍内多组 元层写入→指令组边界 循环确定性（纯函数·跨调用恒等）', () => {
        ...
        const batchFx = rC.效果指令.find(f => f.type === '执行元层写队列');
