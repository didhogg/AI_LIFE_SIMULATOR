...
// ─── Type-level helpers (project test paradigm) ───────────────────────────────
...
function entry(deps: string[]): { 依赖: string[] } {
  return { 依赖: deps };
}
...
describe('buildModGraph — empty registry', () => {
  it('returns empty result', () => {
    // Type-level: ModGraphResult has exactly the four expected keys.
    type Assert = _Expect<Equals<keyof ModGraphResult, 'sccs' | 'selfLoops' | 'cycles' | 'danglingDeps'>>;
...
describe('buildModGraph — large flat registry (stack safety)', () => {
  ...
  it('handles 500-node linear chain without stack overflow', () => {
    const keys: string[] = [];
    for (let i = 0; i < 500; i++) keys.push(`n${String(i).padStart(4, '0')}`);
    const reg: ModRegistry = {};
...
describe('buildModGraph — 冲突[] ignored', () => {
  it('entries with 冲突 field are accepted without error if ModRegistry omits it', () => {
    ...
    const reg: ModRegistry = { a: { 依赖: ['b'] }, b: { 依赖: [] } };
