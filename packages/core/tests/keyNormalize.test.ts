...
import {
  normalizeRegistryKeyNames,
  assertGovernedKeysNormalized,
  type GovernedKeyViolation,
} from '../interfaces/keyNormalize.js';
...
describe('B5 · S3 · normalizeRegistryKeyNames · no-op 场景', () => {
  it('无注册表键 → 返回同一引用', () => {
    const raw: Record<string, unknown> = { 属性: { 体质: 10 } };
    expect(normalizeRegistryKeyNames(raw)).toBe(raw);
  });
  it('受治理键空间注册表为空对象 → 返回同一引用', () => {
    const raw: Record<string, unknown> = { 受治理键空间注册表: {}, 键空间归并表: {} };
    expect(normalizeRegistryKeyNames(raw)).toBe(raw);
  });
...
describe('B5 · S3 · normalizeRegistryKeyNames · 规范化变换', () => {
  it('受治理键空间注册表.键条目[].规范键 含全角字符 → 规范化', () => {
    ...
    const reg = result['受治理键空间注册表'] as Record<string, unknown>;
    const entries = reg['键条目'] as Record<string, unknown>[];
  ...
  it('受治理键空间注册表.键条目[].别名 含零宽字符 → 去除', () => {
    ...
    const reg = result['受治理键空间注册表'] as Record<string, unknown>;
    const entries = reg['键条目'] as Record<string, unknown>[];
    expect((entries[0]?.['别名'] as string[])[0]).toBe('体质');
  });
  it('归并表.归并条目[].别名 含全角 → 规范化', () => {
    ...
    const result = normalizeRegistryKeyNames(raw);
    expect(result).not.toBe(raw);
    const merge = result['键空间归并表'] as Record<string, unknown>;
    const entries = merge['归并条目'] as Record<string, unknown>[];
...
describe('B5 · S3 · normalizeRegistryKeyNames · 幂等', () => {
  it('规范化后再次规范化 → 结果相同（幂等）', () => {
    ...
    const reg1 = (once['受治理键空间注册表'] as Record<string, unknown>)['键条目'] as Record<string, unknown>[];
    const reg2 = (twice['受治理键空间注册表'] as Record<string, unknown>)['键条目'] as Record<string, unknown>[];
...
describe('B5 · S3 · assertGovernedKeysNormalized · 违例报告', () => {
  it('注册表.规范键 非规范态 → 违例含 field/raw/normalized', () => {
    ...
    const v = violations[0] as GovernedKeyViolation;
