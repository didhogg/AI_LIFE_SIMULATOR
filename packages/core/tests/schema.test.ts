...
describe('4.7 Economy layer', () => {
  ...
  it('valid: 持有 负值（透支档 6.25）', () => {
    expect(货币系统Schema.safeParse({ 账户: { 持有: { 两: -200 } } }).success).toBe(true);
  });
  it('valid: 持有 零值', () => {
    expect(货币系统Schema.safeParse({ 账户: { 持有: { 两: 0 } } }).success).toBe(true);
  });
  ...
  // ── 账户.负债（约定库引用）──────────────────────────────────────────────────
  it('valid 负债{债务ID:约定库键}（6.25 大额借贷走约定库）', () => {
    expect(() => 货币系统Schema.parse({
      账户: {
        负债: {
  ...
  it('valid: 负债 empty record', () => {
    expect(() => 货币系统Schema.parse({ 账户: { 负债: {} } })).not.toThrow();
  });
  ...
  it('valid: full 货币系统 parse', () => {
    expect(() => 货币系统Schema.parse({
      ...
      账户: {
        ...
        负债: { debt_001: 'pact_借款_001' }, 被动收入来源: { 商铺: 100 },
