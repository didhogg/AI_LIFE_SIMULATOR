...
import {
  ...
  二审维度库Schema,
  二审维度ID正则,
} from '../schema/reviewDimensionLibrary.js';
...
function mkReview(name: string, overrides: Partial<二审维度定义条目Type> = {}): 二审维度定义条目Type {
  return { 名称: name, ...overrides };
}

function 条目ToPackEnvelope(entry: 二审维度定义条目Type): Record<string, unknown> {
...
describe('二审维度库 · 二审维度定义条目Schema · 独立 parse', () => {
  it('最小条目（仅 名称）→ parse 成功', () => {
    ...
    if (r.success) expect(r.data.名称).toBe('话题偏移检测');
  });
  it('缺 名称 → parse 失败', () => {
...
describe('二审维度库 · resolve 挂载 + by-ID 加载', () => {
  ...
  it('冰箱中 toString 作 handle → 解引用 own-property guard 拦截', () => {
    const 成品: Record<string, unknown> = { 二审维度库: { topic_drift: mkReview('话题偏移') } };
    expect(解引用(创建引用('二审维度', 'topic_drift'), 成品)).toBeDefined();
    expect(解引用({ __ns: '二审维度' as const, handle: 'toString' }, 成品)).toBeNull();
  });
});
...
describe('二审维度库 · 不进判定面指纹', () => {
  ...
  it('改二审维度条目 → hashPresetFingerprint 逐位恒等', () => {
    const lib1: 二审维度库Type = { dim: mkReview('x', { 阈值: 0.5 }) };
    const lib2: 二审维度库Type = { dim: mkReview('x', { 阈值: 0.9 }) };
...
describe('二审维度库 · 守恒门', () => {
  ...
  it('命名空间枚举 = 32 項（含二审维度）', () => {
    ...
    expect((命名空间枚举 as readonly string[]).includes('二审维度')).toBe(true);
  });
  it('冰箱绑定表含 二审维度·解析器键 = 二审维度库', () => {
    expect(冰箱绑定表['二审维度'].解析器键).toBe('二审维度库');
  });
  it('0 重定基验证', () => {
    const lib: 二审维度库Type = { topic_drift: mkReview('话题偏移', { 检测方式: '机械' }) };
