...
import { buildCurrencyRegistry, DEFAULT_CURRENCY_REGISTRY, } from '../engine/currencyRegistry.js';
import { CANONICAL_UNITS, extractMoneyAmountsFor, isCanonicalUnitIn, extractMoneyAmounts, } from '../engine/text/chineseNumber.js';
import { buildOptionId, buildMenuOptionIds, } from '../engine/aohp.js';
import { getNetAsset, BASE_CURRENCY } from '../engine/netAsset.js';
import { gateStructural, gateCoverage, } from '../../../hosts/slice/ledger/gate.js';
...
// ── 辅助 fixture ────────────────────────────────────────────────────────────
function make货币系统(baseCurrency, aliases) {
...
function makeAccount(currency, amount) {
    return {
        持有: { [currency]: amount },
        储蓄: {},
        资产: [],
...
describe('R5: getNetAsset with baseCurrency', () => {
    ...
    it('R5-2 显式传 \"文\" → 同无参数结果', () => {
        const acct = makeAccount('文', 100);
        expect(getNetAsset(acct, undefined, '文')).toBe(100);
    });
    it('R5-3 自定义 baseCurrency=\"灵石\" → 读取灵石账户', () => {
        const acct = makeAccount('灵石', 50);
        expect(getNetAsset(acct, undefined, '灵石')).toBe(50);
    });
    it('R5-4 账户有多币种·baseCurrency 只读对应键', () => {
        ...
        expect(getNetAsset(acct, undefined, '文')).toBe(30);
        expect(getNetAsset(acct, undefined, '灵石')).toBe(50);
    });
