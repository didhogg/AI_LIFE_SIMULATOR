 ...
 * 断言⑨  守恒门：schemaKeys=54 / BUNDLE=21 / manifest=87 / 命名空间枚举 15 项
 */
import { describe, it, expect } from 'vitest';
import { 工具条目Schema, 工具库Schema, 工具引用Schema, TOOL_能力类型, TOOL_能力条目Schema, 工具ID正则, } from '../schema/toolLibrary.js';
...
function tool条目ToPackEnvelope(entry) {
...
describe('工具库 · 工具条目Schema · 独立 parse', () => {
    ...
    it('实现载荷 接受任意 opaque 载荷（不报错·不丢字段）', () => {
        ...
        expect(r.success).toBe(true);
        if (r.success) {
            expect(r.data.实现载荷['prompt']).toBe('你是一个助手');
        }
    });
...
describe('工具库 · 安全硬化：原型名 工具ID → null', () => {
    it('工具ID = constructor → own-property guard 拦截（不进 工具成品）', () => {
        const benign = { roll_tool: mkTool('roll_tool') };
    ...
    it('冰箱中 toString 作 handle → 解引用 own-property guard 拦截', () => {
        ...
        // 原型成员非自有属性 → null
        const protoRef = { __ns: '工具', handle: 'toString' };
...
// ═══════════════════════════════════════════════════════════════════
describe('工具库 · 守恒门', () => {
    it('schemaKeys = 54（工具库不进 RootSchema·不改顶层键数）', () => {
