...
const BASE: Record<string, unknown> = Object.freeze({
...
function expectDeltaError(
  ...
  try { fn(); } catch (e) {
    ...
    expect((e as ComputeDeltaError).code).toBe(code);
  }
...
describe('computeDelta — lock op', () => {
  ...
  it('non-lock op on locked path throws lock-violation', () => {
    const locked = new Set(['持有.文']);
    expectDeltaError(
      () => computeDelta(BASE, entry('持有.文', 'add', 5), { lockedPaths: locked }),
      'lock-violation',
    );
  });
});
...
describe('computeDelta — type-mismatch', () => {
  ...
  it('set with wrong type (number → string) throws type-mismatch', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('等级', 'set', '五级')),
      'type-mismatch',
    );
  });
  ...
  it('set with wrong type (string → number) throws type-mismatch', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('名称', 'set', 99)),
      'type-mismatch',
    );
  });
});
...
describe('computeDelta — integer safety', () => {
  it('add with float value throws non-integer-value', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('持有.文', 'add', 0.5)),
      'non-integer-value',
    );
  });
  ...
  it('sub with float value throws non-integer-value', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('持有.文', 'sub', 1.1)),
      'non-integer-value',
    );
  });
  ...
  it('max_delta with float throws non-integer-value', () => {
    expectDeltaError(
      () => computeDelta(BASE, { path: '持有.文', op: 'add', value: 10, max_delta: 2.5 }),
      'non-integer-value',
    );
  });
});
...
describe('computeDelta — AA4 reserved-key segments', () => {
  it('__proto__ in path throws reserved-key-segment', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('__proto__.污染键', 'set', 1)),
  ...
  it('constructor in path throws reserved-key-segment', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('constructor.prototype', 'set', 1)),
      'reserved-key-segment',
    );
  });
  ...
  it('prototype in path throws reserved-key-segment', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('Object.prototype', 'set', 1)),
      'reserved-key-segment',
    );
  });
});
...
describe('computeDelta — forbidden prefix', () => {
  it('_ prefixed segment throws forbidden-prefix', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('_应收.债务01', 'set', '某约定库')),
      'forbidden-prefix',
    );
  });

  it('_ prefix at any segment position throws forbidden-prefix', () => {
    const s = { ...BASE, 账户: { _负债: {} } } as Record<string, unknown>;
    expectDeltaError(
      () => computeDelta(s, entry('账户._负债', 'set', {})),
      'forbidden-prefix',
    );
  });
});
...
describe('computeDelta — path validation', () => {
  ...
  it('consecutive dots throw invalid-path-segment', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('持有..文', 'set', 1)),
  ...
  it('path not found in state throws path-not-found (no auto-vivify)', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('不存在的键', 'set', '值')),
      'path-not-found',
    );
  });
  ...
  it('nested path not found throws path-not-found', () => {
    expectDeltaError(
      () => computeDelta(BASE, entry('持有.不存在', 'add', 1)),
      'path-not-found',
    );
  });
});
...
describe('computeDelta — determinism', () => {
  ...
  it('computeDelta does not mutate the input state', () => {
    const state = { 持有: { 文: 30 } } as Record<string, unknown>;
    computeDelta(state, entry('持有.文', 'add', 5));
    expect((state['持有'] as Record<string, unknown>)['文']).toBe(30);
  });
});
...
describe('setAtPath', () => {
  it('sets a top-level key immutably', () => {
    const orig = { 名称: '林九', 等级: 1 } as Record<string, unknown>;
  ...
  it('sets a nested key immutably', () => {
    const orig = { 持有: { 文: 30 } } as Record<string, unknown>;
    const next = setAtPath(orig, '持有.文', 99);
    expect((next['持有'] as Record<string, unknown>)['文']).toBe(99);
    expect((orig['持有'] as Record<string, unknown>)['文']).toBe(30);
  });

  it('sets a deeply nested key and preserves sibling keys', () => {
    const orig = { 持有: { 文: 30, 银两: 5 } } as Record<string, unknown>;
    const next = setAtPath(orig, '持有.文', 50);
    expect((next['持有'] as Record<string, unknown>)['银两']).toBe(5);
    expect((next['持有'] as Record<string, unknown>)['文']).toBe(50);
  });
