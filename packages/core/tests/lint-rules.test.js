...
function assertBanned(code, selector) {
...
const SEL_DATE_NOW = 'CallExpression[callee.type="MemberExpression"][callee.object.name="Date"][callee.property.name="now"]';
const SEL_NEW_DATE = 'NewExpression[callee.name="Date"]';
const SEL_DATE_UTC = 'CallExpression[callee.type="MemberExpression"][callee.object.name="Date"][callee.property.name="UTC"]';
describe('禁① Date.now / new Date / Date.UTC', () => {
    ...
    it('Date.UTC(...) fires', () => assertBanned('const ms = Date.UTC(2020, 0, 1);', SEL_DATE_UTC));
});
// ── 禁② no-await-in-d-segment ────────────────────────────────────────────────
const SEL_AWAIT = 'AwaitExpression';
describe('禁② AwaitExpression in D-segment', () => {
    it('await inside async function fires', () => assertBanned('async function settle() { await Promise.resolve(1); }', SEL_AWAIT));
});
// ── 禁③ no-platform-math ─────────────────────────────────────────────────────
const mathFns = ['pow', 'exp', 'log', 'expm1', 'log1p'];
describe('禁③ Math.{pow,exp,log,expm1,log1p}', () => {
    for (const fn of mathFns) {
        ...
        it(`Math.${fn}() fires`, () => assertBanned(`const x = Math.${fn}(1);`, sel));
    }
});
// ── 禁④ no-locale-compare ────────────────────────────────────────────────────
const SEL_LOCALE_COMPARE = 'CallExpression[callee.type="MemberExpression"][callee.property.name="localeCompare"]';
const SEL_INTL_COLLATOR = 'NewExpression[callee.type="MemberExpression"][callee.object.name="Intl"][callee.property.name="Collator"]';
describe('禁④ localeCompare / Intl.Collator', () => {
    it('str.localeCompare() fires', () => assertBanned("const r = 'a'.localeCompare('b');", SEL_LOCALE_COMPARE));
    it('new Intl.Collator() fires', () => assertBanned('const c = new Intl.Collator();', SEL_INTL_COLLATOR));
});
// ── 禁⑤ canonical-serialize-only ─────────────────────────────────────────────
const SEL_JSON_STRINGIFY = 'CallExpression[callee.type="MemberExpression"][callee.object.name="JSON"][callee.property.name="stringify"]';
describe('禁⑤ JSON.stringify (bare)', () => {
    it('JSON.stringify({}) fires', () => assertBanned('const s = JSON.stringify({ a: 1 });', SEL_JSON_STRINGIFY));
});
// ── 禁⑥ no-platform-normalize ───────────────────────────────────────────────
const SEL_NORMALIZE = 'CallExpression[callee.type="MemberExpression"][callee.property.name="normalize"]';
describe('禁⑥ .normalize() (bare)', () => {
    it("str.normalize('NFC') fires", () => assertBanned("const n = 'café'.normalize('NFC');", SEL_NORMALIZE));
});
...
const ENGINE_LINT_PATH = path.join(process.cwd(), 'packages/core/engine/_lint_integration_stub.ts');
describe('六禁集成验证: 通过 eslint.config.js 各自独立报红', () => {
    ...
    async function lintAsEngineFile(code) {
    ...
    it('禁②: AwaitExpression fires via real config', async () => {
        const msgs = await lintAsEngineFile('async function f() { await Promise.resolve(1); }');
