...
function makeOpt(overrides: Partial<ActionOptionType> = {}): ActionOptionType {
  return {
...
describe('Test 5 · 动词不在动词Id枚举 10 内 → downgrade:true', () => {
  ...
  it('动词Id枚举 全部 10 项均合法（正向确认）', () => {
    const 动词列表 = ['转移', '缔结', '解除', '赋予', '剥夺', '调整', '披露', '移动', '施加', '植入'] as const;
    for (const verb of 动词列表) {
      const id = `${verb}:npc_wang`;
      const opt: ActionOptionType = { option_id: id, tool_name: '', params: {}, target_choices: ['npc_wang'] };
...
describe('Test 6 · 指纹 manifest 不变 + 确定性验证', () => {
  ...
  it('AOHP選項id集 已在 FINGERPRINT_PRESET_FIELDS（接线已在位）', () => {
    expect(FINGERPRINT_PRESET_FIELDS as readonly string[]).toContain('AOHP選項id集');
  });
