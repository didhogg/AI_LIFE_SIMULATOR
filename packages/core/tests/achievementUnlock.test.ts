...
function mkAch(overrides: Partial<成就条目Type> = {}): 成就条目Type {
  return { 名称: 'テスト', ...overrides };
}

const TICK_ID = (s: string) => `ach-test-${s}`;
...
describe('P8-a-0 · SETTLEMENT_PHASES=17 · 新 phase 在位 · 既有序不变', () => {
  ...
  it('成就解锁 在 媒介拍末取材 之后·原子提交 之前', () => {
    const idx = SETTLEMENT_PHASES.indexOf('成就解锁' as typeof SETTLEMENT_PHASES[number]);
    const idxMedia = SETTLEMENT_PHASES.indexOf('媒介拍末取材' as typeof SETTLEMENT_PHASES[number]);
    const idxCommit = SETTLEMENT_PHASES.indexOf('原子提交' as typeof SETTLEMENT_PHASES[number]);
  ...
  it('既有 14 phase 名称 & 顺序不变', () => {
    ...
    for (const ph of expected14) {
      const cur = SETTLEMENT_PHASES.indexOf(ph as typeof SETTLEMENT_PHASES[number]);
...
describe('P8-a-2 · 解锁条件真 → npc.成就[achId] 写入', () => {
  const lib: Record<string, 成就条目Type> = {
...
describe('P8-a-6 · 多 NPC×多成就 码点序双跑逐位恒等', () => {
  ...
  it('两次独立 runTick（不同 tickId）state JSON 逐位相同', () => {
    ...
    expect(
      JSON.stringify(r1.state.NPC['npc_a']?.成就),
    ).toBe(
      JSON.stringify(r2.state.NPC['npc_a']?.成就),
    );
    expect(
      JSON.stringify(r1.state.NPC['npc_b']?.成就),
    ).toBe(
      JSON.stringify(r2.state.NPC['npc_b']?.成就),
    );
  });
...
describe('P8-a-7 · 原型污染 guard', () => {
  it('原型继承属性不视为成就条目·只读 own-property', () => {
    ...
    const base = Object.create(null) as Record<string, 成就条目Type>;
    base['safe_one'] = mkAch({ 解锁条件引用: '属性.智慧 >= 70' });

    // 子对象：own property = good_one；继承 safe_one
    const child = Object.create(base) as Record<string, 成就条目Type>;
    child['good_one'] = mkAch({ 解锁条件引用: '属性.智慧 >= 70' });
    // child 继承了 safe_one 但它不是 own property

    let r: ReturnType<typeof runTick> | undefined;
    expect(() => {
  ...
  it('npcKey 为 constructor → own-property guard 过滤·不写 NPC', () => {
    ...
    const unlockedKeys = Object.keys(r.state.NPC).filter(k =>
      Object.keys(r.state.NPC[k]?.成就 ?? {}).length > 0,
    );
...
describe('P8-a-9 · 金向量回归·解锁写 NPC.成就 不进指纹', () => {
  it('黄金测试：连续两次独立 runTick 结果确定性', () => {
    ...
    // NPC.成就 状态逐位相同（确定性）
    expect(JSON.stringify(r1.state.NPC['npc_a']?.成就)).toBe(
      JSON.stringify(r2.state.NPC['npc_a']?.成就),
    );
  });
});
...
describe('P8-b-6 · 幂等/重放：同成就跨拍重复·后果仅首拍执行', () => {
  ...
  it('双跑逐位恒等（确定性）', () => {
    ...
    expect(JSON.stringify(rA.state.NPC['npc_a']?.属性)).toBe(
      JSON.stringify(rB.state.NPC['npc_a']?.属性),
    );
  });
});
