...
const PACKS_CONSERVATION: K5DeltaEntry[][] = [[
...
describe('T1 · 合法注入 → 落账成功 + 守恒 + 幂等', () => {
  ...
  it('npc_wang 持有.文 从 100 升至 150', () => {
    const 账户 = result.state.货币系统?.账户 as Record<string, { 持有: Record<string, number> }>;
  ...
  it('原始 BASE_STATE 未被写回（入口深拷）', () => {
    const 账户 = BASE_STATE.货币系统?.账户 as Record<string, { 持有: Record<string, number> }>;
...
describe('T2a · Gate②-whitelist 违反 → 拒绝 + 零写账', () => {
  ...
  it('被 Gate② 拒绝', () => {
    ...
    if (!r?.ok) expect(r?.gate).toBe('②-whitelist');
  });
