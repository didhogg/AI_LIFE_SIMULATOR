...
import { VALID_OVERWRITE_AUTH_SOURCES, DESTINY_CHANNEL_SOURCE, checkM2Violation, writeM2Tombstone, } from '../interfaces/authGate.js';
...
describe('B5 · M2 · checkM2Violation · 无效授权源越权', () => {
    ...
    it('越权时 诊断 字段非空', () => {
        const r = checkM2Violation('');
        if (r.violation)
            expect(r.诊断.length).toBeGreaterThan(0);
    });
});
...
describe('B5 · M2 · checkM2Violation · 天命通道越权', () => {
    ...
    it('天命通道越权诊断含来源字符串本身', () => {
        const r = checkM2Violation(DESTINY_CHANNEL_SOURCE);
        if (r.violation)
            expect(r.诊断).toContain(DESTINY_CHANNEL_SOURCE);
    });
});
