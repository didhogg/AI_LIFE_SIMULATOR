...
function fingerprintOf(ctx) {
    ...
    return hashPresetFingerprint({
        ...
        ...(ctx['AOHP選項id集'] !== undefined ? { AOHP選項id集: ctx['AOHP選項id集'] } : {}),
        ...(ctx['动词选项集哈希'] !== undefined ? { 动词选项集哈希: ctx['动词选项集哈希'] } : {}),
        snapshot: {
            难度系数组: ctx['难度系数组'],
            判定骰型: ctx['判定骰型'],
            暴击映射: ctx['暴击映射'],
...
describe('AA6 gate A-bundle: 判定面整包成员变 → bundle hash 变 → 指纹变', () => {
    for (const field of FINGERPRINT_BUNDLE_MEMBERS) {
        it(`bundle.${field} 改变 → 指纹改变`, () => {
            ...
            const mutated = { ...BASE_CTX, [field]: BUNDLE_MUTATIONS[field] };
...
describe('AA6 gate A-snap: 快照锁定成员变 → 指纹变', () => {
    for (const field of FINGERPRINT_SNAPSHOT_FIELDS) {
        it(`snapshot.${field} 改变 → 指纹改变`, () => {
            const base = fingerprintOf(BASE_CTX);
            const mutated = { ...BASE_CTX, [field]: SNAPSHOT_MUTATIONS[field] };
            expect(fingerprintOf(mutated)).not.toBe(base);
        });
    }
});
...
describe('AA6 gate A-preset: 预设整包成员（生效中内容包集哈希/规则补丁哈希）变 → 指纹变', () => {
    it('B1c: 生效中内容包集哈希 改变 → 指纹改变', () => {
        const base = fingerprintOf(BASE_CTX);
        const mutated = { ...BASE_CTX, 生效中内容包集哈希: PRESET_MUTATIONS['生效中内容包集哈希'] };
        expect(fingerprintOf(mutated)).not.toBe(base);
    });
    it('K5: 规则补丁哈希 存在 → 指纹改变', () => {
        const base = fingerprintOf(BASE_CTX);
        const mutated = { ...BASE_CTX, 规则补丁哈希: PRESET_MUTATIONS['规则补丁哈希'] };
        expect(fingerprintOf(mutated)).not.toBe(base);
    });
    it('P0-8-B4: AOHP選項id集 存在时 → 指纹改变', () => {
        const base = fingerprintOf(BASE_CTX);
        const withIds = { ...BASE_CTX, AOHP選項id集: PRESET_MUTATIONS['AOHP選項id集'] };
        expect(fingerprintOf(withIds)).not.toBe(base);
    });
    it('P0-8-B4: AOHP選項id集 不同值 → 指纹不同', () => {
        const ctx1 = { ...BASE_CTX, AOHP選項id集: ['给钱:npc_wang:5文'] };
        const ctx2 = { ...BASE_CTX, AOHP選項id集: ['给钱:npc_wang:10文'] };
    ...
    it('P0-8-B4: AOHP選項id集 预排序传入·顺序一致 → 指纹恒等（调用方须预排序·sortedOptionIds 保证）', () => {
        ...
        const ctx1 = { ...BASE_CTX, AOHP選項id集: preSorted };
        const ctx2 = { ...BASE_CTX, AOHP選項id集: [...preSorted] };
        expect(fingerprintOf(ctx1)).toBe(fingerprintOf(ctx2));
    });
    it('阶段1: 动词选项集哈希 存在 → 指纹改变', () => {
        const base = fingerprintOf(BASE_CTX);
        const withHash = { ...BASE_CTX, 动词选项集哈希: PRESET_MUTATIONS['动词选项集哈希'] };
        expect(fingerprintOf(withHash)).not.toBe(base);
    });
    it('阶段1: 动词选项集哈希 不同值 → 指纹不同', () => {
        const ctx1 = { ...BASE_CTX, 动词选项集哈希: 'aaaa0000' };
        const ctx2 = { ...BASE_CTX, 动词选项集哈希: 'bbbb1111' };
        expect(fingerprintOf(ctx1)).not.toBe(fingerprintOf(ctx2));
    });
    ...
    it('判定面整包: 直接传不同 hash → 指纹改变', () => {
        ...
        const baseFP = hashPresetFingerprint({
            判定面整包: hashJudgmentBundle({ ...BASE_CTX }),
            生效中内容包集哈希: '',
...
describe('DSL · 改版本 → 指纹变 property (M·1 指纹取材集)', () => {
    it('DSL文法版本 bump ("1.0"→"2.0") → 指纹改变', () => {
        const base = fingerprintOf(BASE_CTX);
        const mutated = { ...BASE_CTX, DSL文法版本: PRESET_MUTATIONS['DSL文法版本'] };
        expect(fingerprintOf(mutated)).not.toBe(base);
    });
    it('求值器函数库版本 bump (1→2) → 指纹改变', () => {
        const base = fingerprintOf(BASE_CTX);
        const mutated = { ...BASE_CTX, 求值器函数库版本: PRESET_MUTATIONS['求值器函数库版本'] };
        expect(fingerprintOf(mutated)).not.toBe(base);
    });
});
// ── Gate B: 排除名单成员变 → 指纹不变 ───────────────────────────────────────
describe('AA6 gate B: 排除名单成员变 → 指纹不变', () => {
    for (const field of FINGERPRINT_EXCLUDED_FIELDS) {
        it(`excluded.${field} 改变 → 指纹不变`, () => {
            const base = fingerprintOf(BASE_CTX);
            const mutated = { ...BASE_CTX, [field]: EXCLUDED_MUTATIONS[field] };
...
describe('F-c 层1 pre-wiring: 引擎版本 / Schema版本 指纹特异性（U3·字段已在位·分段触发逻辑随P0-3）', () => {
    it('特异性①: 引擎版本 非空 ("v2.0.0") → 指纹变', () => {
        const base = fingerprintOf(BASE_CTX);
        const mutated = { ...BASE_CTX, 引擎版本: PRESET_MUTATIONS['引擎版本'] };
        expect(fingerprintOf(mutated)).not.toBe(base);
    });
    it('特异性②: Schema版本 非空 ("2") → 指纹变', () => {
        const base = fingerprintOf(BASE_CTX);
        const mutated = { ...BASE_CTX, Schema版本: PRESET_MUTATIONS['Schema版本'] };
        expect(fingerprintOf(mutated)).not.toBe(base);
    });
...
import { computeSegmentHeadHash, openSegment, verifySegmentChain, shouldOpenNewSegment, } from '../engine/segment.js';
