...
import { z } from 'zod';
import { scheduleLodPhase, LOD_PROMOTE_BUDGET, LOD_DRIFT_N, resolveLodPredicate, seededSortKey, } from '../engine/lodPhase.js';
import { detectLodTrigger, } from '../engine/lodScheduler.js';
import { registerLodMount, clearLodRegistry, } from '../engine/lodMount.js';
...
describe('D-1: resolveLodPredicate + opt-in 触发谓词驱动', () => {
    ...
    it('Tier B 监测轴=声望·触发阈值=>50% 与 Tier A 直接谓词等价（计数相同）', () => {
        ...
        const sA = makeS();
        scheduleLodPhase(sA, 1, 1, undefined, presetA);
        const sB = makeS();
        scheduleLodPhase(sB, 1, 1, undefined, presetB);
