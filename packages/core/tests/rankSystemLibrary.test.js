...
import { 职级定义条目Schema, 职级体系库Schema, 职级体系ID正则, } from '../schema/rankSystemLibrary.js';
...
function mkRank(name, overrides = {}) {
...
describe('职级体系库 · resolve 挂载 + by-ID 加载', () => {
    ...
    it('冰箱中 toString 作 handle → 解引用 own-property guard 拦截', () => {
        ...
        const 成品 = { 职级体系库: { county_magistrate: entry } };
        const ref = 创建引用('职级体系', 'county_magistrate');
        expect(解引用(ref, 成品)).toBeDefined();
        const protoRef = { __ns: '职级体系', handle: 'toString' };
...
describe('职级体系库 · 不进判定面指纹', () => {
    ...
    it('改职级条目 → hashPresetFingerprint 逐位恒等', () => {
        const lib1 = { rank: mkRank('知县', { 薪资系数: 1 }) };
        const lib2 = { rank: mkRank('知县', { 薪资系数: 9 }) };
...
describe('职级体系库 · 守恒门', () => {
    ...
    it('命名空间枚举 = 32 項（含职级体系）', () => {
        ...
        expect(命名空间枚举.includes('职级体系')).toBe(true);
    });
    it('冰箱绑定表含 职级体系·解析器键 = 职级体系库', () => {
        expect(冰箱绑定表['职级体系'].解析器键).toBe('职级体系库');
    });
    it('0 重定基验证', () => {
        const lib = { county_magistrate: mkRank('知县', { 薪资系数: 1.5 }) };
