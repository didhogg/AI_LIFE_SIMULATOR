...
import { EPOCH_ANCHOR_YEAR, MINUTES_PER_DAY, MINUTES_PER_MONTH, MINUTES_PER_YEAR, MIGRATION_TICK_MINUTES, GRANULARITY_STACK_MAX, MONTH_NAMES, isLeapYear, gregorianToEpochMin, epochMinToGregorian, parseChineseDateToEpochMin, getTickMinutes, makePeriodConverter, renderCalendar, pushGranularity, popGranularity, makeDualClock, enterRpFocus, exitRpFocus, advanceWorld, advanceLens, decayLinear, decayStep, compound, probOverSpan, isExpired, getSeason, getAge, dayFloor, timeOfDayMinutes, writeEpochMinute, earliestDeadline, minutesSinceLast, nextMonthStart, nextYearSameDay, computeTickSpan, } from '../engine/time.js';
...
describe('getSeason', () => {
    it('北温带: 月份3/4/5 → 春', () => {
        for (const m of [3, 4, 5])
            expect(getSeason(m, '北温带')).toBe('春');
    });
    it('北温带: 月份6/7/8 → 夏', () => {
        for (const m of [6, 7, 8])
            expect(getSeason(m, '北温带')).toBe('夏');
    });
    it('北温带: 月份9/10/11 → 秋', () => {
        for (const m of [9, 10, 11])
            expect(getSeason(m, '北温带')).toBe('秋');
    });
    it('北温带: 月份12/1/2 → 冬', () => {
        for (const m of [12, 1, 2])
            expect(getSeason(m, '北温带')).toBe('冬');
    });
    it('默认（空字符串气候带）→ 北半球季节', () => {
    ...
    it('热带: 任意月份 → 无四季', () => {
        for (const m of [1, 6, 12])
            expect(getSeason(m, '热带')).toBe('无四季');
    });
    it('南北半球季节完全相反（6月）', () => {
