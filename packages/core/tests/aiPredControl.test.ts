...
describe('DSL-AI: resolveEffectivePredicate — 优先级规则', () => {
  ...
  it('作者控制表[key]=false → 永远返回 base（无视玩家/全局/override）', () => {
    expect(resolveEffectivePredicate(KEY, BASE, true,
      { [KEY]: false }, { [KEY]: true }, { [KEY]: OVERRIDE },
    )).toBe(BASE);
  });

  it('作者红线·全局关闭时依然返回 base', () => {
    expect(resolveEffectivePredicate(KEY, BASE, false,
      { [KEY]: false }, undefined, { [KEY]: OVERRIDE },
    )).toBe(BASE);
  });
  ...
  it('玩家控制表[key]=true，有 override → 返回 override', () => {
    expect(resolveEffectivePredicate(KEY, BASE, false,
      undefined, { [KEY]: true }, { [KEY]: OVERRIDE },
    )).toBe(OVERRIDE);
  });

  it('玩家控制表[key]=true，无 override → fail-safe 返回 base', () => {
    expect(resolveEffectivePredicate(KEY, BASE, false,
      undefined, { [KEY]: true }, undefined,
    )).toBe(BASE);
  });

  it('玩家控制表[key]=false → 关闭·返回 base（无视全局和 override）', () => {
    expect(resolveEffectivePredicate(KEY, BASE, true,
      undefined, { [KEY]: false }, { [KEY]: OVERRIDE },
    )).toBe(BASE);
  });
  ...
  it('玩家未设置，作者控制表[key]=true，有 override → 返回 override', () => {
    expect(resolveEffectivePredicate(KEY, BASE, false,
      { [KEY]: true }, undefined, { [KEY]: OVERRIDE },
    )).toBe(OVERRIDE);
  });

  it('玩家未设置，作者未设置，全局开=true，有 override → 返回 override', () => {
    expect(resolveEffectivePredicate(KEY, BASE, true,
      undefined, undefined, { [KEY]: OVERRIDE },
    )).toBe(OVERRIDE);
  });
  ...
  it('其他键的 override 不影响当前键', () => {
    expect(resolveEffectivePredicate(KEY, BASE, true,
      undefined, undefined, { 'lore:hanfu:other': OVERRIDE },
    )).toBe(BASE);
  });

  // 空 base 串
  it('base 为空串·全局开·有 override → 返回 override', () => {
    expect(resolveEffectivePredicate(KEY, '', true,
      undefined, undefined, { [KEY]: OVERRIDE },
    )).toBe(OVERRIDE);
  });

  it('base 为空串·全局关 → 返回空串', () => {
    expect(resolveEffectivePredicate(KEY, '', false,
      undefined, undefined, { [KEY]: OVERRIDE },
    )).toBe('');
  });
});
...
describe('DSL-AI: 铁律③ $AI创作状态 进存档（serializeArchive JSON 验证）', () => {
  ...
  it('$AI创作状态 进 JSON snapshot·不被 serializeArchive 排除', () => {
    ...
    const result = serializeArchive(state as Parameters<typeof serializeArchive>[0]);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    // 解析 JSON 信封，检查 snapshot 中包含 $AI创作状态
    const envelope = JSON.parse(result.json) as Record<string, unknown>;
    ...
    const aiState = snapshot['$AI创作状态'] as Record<string, unknown>;
    expect((aiState?.['谓词override表'] as Record<string, string>)?.['lore:hanfu:交领']).toBe('属性.体质 >= 3');
    expect((aiState?.['条目AI控制表'] as Record<string, boolean>)?.['effectPack:ep001']).toBe(false);
  });
