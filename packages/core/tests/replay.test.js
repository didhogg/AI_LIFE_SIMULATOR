...
// ── Helpers ───────────────────────────────────────────────────────────────────
function makeDefaultState(tickId) {
...
function makeNsfwPreemptState(tickId) {
    const base = RootSchema.parse({
...
function makeEntry(tickId, routedVia, modelKey, explicitReason, salt) {
    return {
        tick_id: tickId,
        拍计数: 1,
        结果摘要: 'test',
...
function makeInput(entry, overrides = {}) {
    return {
...
describe('REPLAY-01 · 场景一 · 悔棋后旧任务迟到', () => {
    it('断言③: 重放历史拍路由逐位恒等（不受悔棋后偏好变化影响）', () => {
        ...
        const liveAfterUndo = {
            ...recorded,
            $玩家偏好: { ...recorded.$玩家偏好, NSFW降级模型: { 启用: true, 触发模式: '失败兜底' } },
        };
...
describe('REPLAY-01 · 场景二 · fork 瞬间在途 AA1 世代号丢弃', () => {
    it('fork 后所有 pre-fork 世代工单被丢弃·post-fork 工单保留·路由仍读冻结值', () => {
        ...
        const forkEntry = { ...baseEntry, 盐值: 99 };
        const preForkTickets = [
...
describe('REPLAY-01 · 场景六 · 跨版本段×难度段（AA8 版本段分段）', () => {
    it('不同系数组指纹的历史条目·各自路由/盐值独立·互不影响', () => {
        const entryEasy = { ...makeEntry('tick-easy', 'default', null, '简单段', 5), 系数组指纹: 'fp-easy' };
        const entryHard = { ...makeEntry('tick-hard', 'default', null, '困难段', 15), 系数组指纹: 'fp-hard' };
