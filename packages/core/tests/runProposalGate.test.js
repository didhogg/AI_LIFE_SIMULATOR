...
function getHoldings(state, acctKey) {
    const 货币系统 = state.货币系统;
    const 账户 = 货币系统['账户'];
    const acct = 账户[acctKey];
    return acct['持有'];
}
...
describe('Gate② — whitelist', () => {
    it('rejects path not in whitelist', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: '危险路径.foo', op: 'add', value: 1 }]));
        expect(r.ok).toBe(false);
        if (!r.ok) {
    ...
    it('accepts whitelisted path', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: WL_PATH, op: 'add', value: 10 }]));
        expect(r.ok).toBe(true);
    });
    it('returns original state snapshot on whitelist failure', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: '不存在.xxx', op: 'set', value: 1 }]));
        expect(r.ok).toBe(false);
        // state returned = snapshot of original (no mutations)
        if (!r.ok)
            expect(r.state).not.toBe(BASE_STATE); // structuredClone
    });
});
...
describe('Gate② — C6 seat scope', () => {
    it('single-player degrades to eligible (seat table size ≤1)', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: WL_PATH, op: 'add', value: 5 }]));
        expect(r.ok).toBe(true);
    });
    it('multi-seat: wrong seat target → C6 ineligible', () => {
        const multiSeatState = RootSchema.parse({
            ...BASE_STATE,
            _席位表: {
        ...
        const r = runProposalGate(ENV_BASIC, multiSeatState, 'seat_a', '系统', pack([{ path: 'NPC.npc_wang.当前作息模式', op: 'set', value: '营业中' }]));
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.gate).toBe('②-C6');
    });
});
// ─── Gate③: M3 prefix ────────────────────────────────────────────────────────
describe('Gate③ — M3 structural invariant', () => {
    it('rejects hard-excluded path (first segment starts with _)', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: '_作弊标记', op: 'add', value: 1 }]));
        expect(r.ok).toBe(false);
        // Gate②-whitelist fires before Gate③ (path not in whitelist either)
        if (!r.ok)
            expect(r.gate).toMatch(/②|③/);
    });
    it('rejects forward-only path with sub op (M3 forward-only violation)', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: '编年史.序号', op: 'sub', value: 1 }]));
        expect(r.ok).toBe(false);
    });
});
...
describe('Gate④ — K5 delta application', () => {
    it('add op applies to state correctly', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: WL_PATH, op: 'add', value: 50 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(getHoldings(r.state, 'npc_wang')['文']).toBe(250); // 200 + 50
    });
    it('sub op applies to state correctly', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: WL_PATH, op: 'sub', value: 30 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(getHoldings(r.state, 'npc_wang')['文']).toBe(170); // 200 - 30
    });
    it('clamp op caps value at ceiling via clampLedger', () => {
        // Current value is 200, clamp ceiling is 150 → result should be 150.
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: WL_PATH, op: 'clamp', value: 150 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(getHoldings(r.state, 'npc_wang')['文']).toBe(150);
    });
    it('clamp op above current value is no-op', () => {
        // Current value is 200, clamp ceiling is 500 → value unchanged.
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: WL_PATH, op: 'clamp', value: 500 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
    ...
    it('computeDelta path-not-found in Gate④ fails with gate=④-delta', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: '货币系统.账户.npc_missing.持有.文', op: 'add', value: 1 }]));
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.gate).toBe('④-delta');
    });
    it('multi-pack merge: cross-pack clamp takes min ceiling', () => {
        ...
        const p1 = [{ path: WL_PATH, op: 'clamp', value: 300 }];
        const p2 = [{ path: WL_PATH, op: 'clamp', value: 120 }];
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', [p1, p2]);
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(getHoldings(r.state, 'npc_wang')['文']).toBe(120);
    });
    it('max_delta cap is respected in merged delta', () => {
        // add value=100, max_delta=20 → only +20 applied.
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: WL_PATH, op: 'add', value: 100, max_delta: 20 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(getHoldings(r.state, 'npc_wang')['文']).toBe(220); // 200 + 20
    });
});
// ─── Gate⑤: audit log ────────────────────────────────────────────────────────
...
describe('§2 · ±Infinity safety — clamp lo=-Infinity / hi=Infinity', () => {
    it('extreme negative current value: lo=-Infinity does not misfire (overdraft preserved)', () => {
        ...
        const r = runProposalGate(ENV_BASIC, negState, 'seat1', '系统', pack([{ path: WL_PATH, op: 'clamp', value: 0 }]));
        expect(r.ok).toBe(true);
        if (r.ok) {
            const val = getHoldings(r.state, 'npc_wang')['文'];
...
describe('§3 · atomic rollback + DSL串拒 + mutation+clamp combo', () => {
    ...
    it('§3-4 · string value in add op → Gate④ type-mismatch fail-closed (DSL 串到 add 拒)', () => {
        ...
        const entry = { path: WL_PATH, op: 'add', value: 'dsl:ceil*0.5' };
    ...
    it('§3-5 · add then clamp same path = min(current+delta, ceiling)', () => {
        ...
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(getHoldings(r.state, 'npc_wang')['文']).toBe(220); // min(250, 220)
    });
});
// ─── ⊕-3 · Gate③-M2 覆写授权源认证 ──────────────────────────────────────────
describe('⊕-3 · Gate③-M2 — 覆写授权源认证（M2 pre-check）', () => {
    ...
    it('M2 拒 → state===snapshot（零状态写·原子回滚）', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '无效授权源', pack([{ path: WL_PATH, op: 'add', value: 999 }]));
        expect(r.ok).toBe(false);
        if (!r.ok) {
...
describe('⊕-3 · Gate② — C6 Rule②③④ 补全', () => {
    const MULTI_SEAT = RootSchema.parse({
    ...
    it('Rule② · seatId 不在席位表 → ②-C6（无效席位）', () => {
        const r = runProposalGate(ENV_BASIC, MULTI_SEAT, 'seat_ghost', '系统', pack([{ path: 'NPC.npc_wang.姓名', op: 'set', value: '王' }]));
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.gate).toBe('②-C6');
    });
    it('Rule③ · 焦点角色键===targetCharKey → 多席正向合格（C6 不拦）', () => {
        ...
        const r = runProposalGate(ENV_BASIC, MULTI_SEAT, 'seat_a', '系统', pack([{ path: 'NPC.npc_wang.姓名', op: 'set', value: '王' }]));
        // C6 must NOT block; may fail at Gate④ (path-not-found in test state)
        if (!r.ok)
            expect(r.gate).not.toBe('②-C6');
    });
    it('Rule④ · 焦点角色键===\'\' → ②-C6（无焦点席位）', () => {
        const noFocusState = RootSchema.parse({
            货币系统: { 账户: { npc_wang: { 持有: { 文: 200 } } } },
            _状态机: { 双时钟: { 世界钟: 100 } },
            _席位表: {
        ...
        const r = runProposalGate(ENV_BASIC, noFocusState, 'seat_a', '系统', pack([{ path: 'NPC.npc_wang.姓名', op: 'set', value: '王' }]));
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.gate).toBe('②-C6');
    });
...
describe('⊕-3 · K5 多包合并 — content op 后载覆盖', () => {
    it('两包同路径 add：后包 value 覆盖先包（last-writer-wins·最终+30）', () => {
        ...
        const p1 = [{ path: WL_PATH, op: 'add', value: 10 }];
        const p2 = [{ path: WL_PATH, op: 'add', value: 30 }];
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', [p1, p2]);
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(getHoldings(r.state, 'npc_wang')['文']).toBe(230); // 200+30
    });
});
// ─── ⊕-4 e2e · 提案闸端到端验收（收官） ─────────────────────────────────────────
describe('⊕-4 e2e-1 · happy path：全链 ①→⑤ 含多包 add+clamp+audit', () => {
    it('add 50 → clamp 220 → final=220; assertConservation 通过; 指纹 manifest 恒等', () => {
        ...
        expect(FINGERPRINT_BUNDLE_MEMBERS.length + FINGERPRINT_PRESET_FIELDS.length +
            FINGERPRINT_SNAPSHOT_FIELDS.length + FINGERPRINT_EXCLUDED_FIELDS.length).toBe(89);
    });
});
describe('⊕-4 e2e-2 · fail-closed Gate② → 全状态 deepEqual（零写）', () => {
    it('非白名单路径 reject → JSON.stringify(result.state) 逐字节 === snapshot', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: '非白名单路径.forbidden', op: 'add', value: 1 }]));
        expect(r.ok).toBe(false);
        if (r.ok)
            return;
        expect(r.gate).toBe('②-whitelist');
...
describe('⊕-4 e2e-4 · M3 forward-only set 回退：orchestrator 全链拦截（state 零写）', () => {
    it('编年史.序号 set 回退被 orchestrator 拒收·state 逐字段不变', () => {
        ...
        expect(M3_FORWARD_ONLY_PATHS).toContain('编年史.序号');
        const forwardOnlyState = JSON.parse(JSON.stringify(BASE_STATE));
        forwardOnlyState['编年史'] = { 序号: 100 };
        const r = runProposalGate(ENV_BASIC, forwardOnlyState, 'seat1', '系统', pack([{ path: '编年史.序号', op: 'set', value: 50 }]));
        expect(r.ok).toBe(false);
        if (r.ok)
            return;
...
describe('⊕-4 e2e-5 · 透支负值：sub 致余额为负·lo=-Infinity 不 floor·原子提交成功', () => {
    it('200 - 300 = -100（透支合法域·clampLedger lo=-Infinity·not floored to 0）', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: WL_PATH, op: 'sub', value: 300 }]));
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
        const val = getHoldings(r.state, 'npc_wang')['文'];
...
describe('⊕-4 e2e-6 · 全 fail-closed 路径：BASE_STATE 入参在任何拒收后不被 mutate', () => {
    it('Gate①②③④ 各拒收路径后 BASE_STATE 逐字节不变（入参不可变性）', () => {
        ...
        runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: '禁止路径.forbidden', op: 'add', value: 1 }]));
        ...
        runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', '系统', pack([{ path: '货币系统.账户.npc_nobody.持有.文', op: 'add', value: 1 }]));
...
describe('Gate③-L15 · 存活状态/物品状态 不可逆转移', () => {
    const STATE_DEAD_NPC = RootSchema.parse({
        NPC: { npc_ghost: { 存活状态: '已故' } },
        _席位表: {},
    });
    const STATE_DESTROYED_ITEM = RootSchema.parse({
