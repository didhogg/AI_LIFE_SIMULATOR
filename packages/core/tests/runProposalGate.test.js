...
describe('Gate② — whitelist', () => {
    it('rejects path not in whitelist', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: '危险路径.foo', op: 'add', value: 1 }]));
        expect(r.ok).toBe(false);
        if (!r.ok) {
    ...
    it('accepts whitelisted path', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: WL_PATH, op: 'add', value: 10 }]));
        expect(r.ok).toBe(true);
    });
    it('returns original state snapshot on whitelist failure', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: '不存在.xxx', op: 'set', value: 1 }]));
        expect(r.ok).toBe(false);
...
describe('Gate② — C6 seat scope', () => {
    it('single-player degrades to eligible (seat table size ≤1)', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: WL_PATH, op: 'add', value: 5 }]));
        expect(r.ok).toBe(true);
    });
    it('multi-seat: wrong seat target → C6 ineligible', () => {
        ...
        const r = runProposalGate(ENV_BASIC, multiSeatState, 'seat_a', 'test', pack([{ path: 'NPC.npc_wang.当前作息模式', op: 'set', value: '营业中' }]));
        expect(r.ok).toBe(false);
        if (!r.ok)
...
describe('Gate③ — M3 structural invariant', () => {
    it('rejects hard-excluded path (first segment starts with _)', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: '_作弊标记', op: 'add', value: 1 }]));
        expect(r.ok).toBe(false);
    ...
    it('rejects forward-only path with sub op (M3 forward-only violation)', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: '编年史.序号', op: 'sub', value: 1 }]));
        expect(r.ok).toBe(false);
    });
});
...
describe('Gate④ — K5 delta application', () => {
    it('add op applies to state correctly', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: WL_PATH, op: 'add', value: 50 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(getHoldings(r.state, 'npc_wang')['文']).toBe(250); // 200 + 50
    });
    it('sub op applies to state correctly', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: WL_PATH, op: 'sub', value: 30 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
    ...
    it('clamp op caps value at ceiling via clampLedger', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: WL_PATH, op: 'clamp', value: 150 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
    ...
    it('clamp op above current value is no-op', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: WL_PATH, op: 'clamp', value: 500 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
    ...
    it('computeDelta path-not-found in Gate④ fails with gate=④-delta', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: '货币系统.账户.npc_missing.持有.文', op: 'add', value: 1 }]));
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.gate).toBe('④-delta');
    });
    ...
    it('max_delta cap is respected in merged delta', () => {
        ...
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: WL_PATH, op: 'add', value: 100, max_delta: 20 }]));
        expect(r.ok).toBe(true);
        if (r.ok)
...
describe('Gate⑤ — audit log append', () => {
    ...
    it('log entry 授权源 matches parameter', () => {
        ...
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(getLog(r.state)[0]?.['授权源']).toBe('gm_override');
    });
    ...
    it('no log entry appended on Gate② failure', () => {
        const r = runProposalGate(ENV_BASIC, BASE_STATE, 'seat1', 'test', pack([{ path: '非白名单路径.x', op: 'add', value: 1 }]));
        expect(r.ok).toBe(false);
