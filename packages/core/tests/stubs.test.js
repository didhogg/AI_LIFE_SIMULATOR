...
import { 提案单Schema, 提案单条目Schema, 方向槽枚举, 指令信封Schema, 失败工单条目Schema, 失败工单Schema, } from '../schema/proposal.js';
...
describe('P0-1x Stub: CombatResolver（6.63·三段·未实装）', () => {
    ...
    it('step 签名抛出「未实装」', () => {
        ...
        expect(() => CombatResolver.step({}, ['攻击'], [])).toThrow('未实装');
    });
    it('settle 签名抛出「未实装」', () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(() => CombatResolver.settle({})).toThrow('未实装');
    });
});
...
describe('校验⑧ 语义双写核查: P0-1 新字段不存储可派生真相', () => {
    ...
    const P0_1_NEW_FIELDS = [
        ...
        ['_来源包', 'mod归属键·血统元数据·不可派生'], // D-3过渡期：行动卡侧保留 _ 前缀·computeDelta只读保护；种子侧规范字段为 来源包（dollar.ts 延时种子.来源）
    ...
    it('P0-1 新字段名均不在已知可派生量集合中', () => {
        for (const [field] of P0_1_NEW_FIELDS) {
            expect(KNOWN_DERIVABLE.has(field), `'${field}' 疑似可派生量·违反 AA8「能派生的不存储」`).toBe(false);
        }
    });
...
describe('P0-1x 签名冻结断言 stub（三件套第③件·编译期口径锁）', () => {
    ...
    it('findRoute 返回类型冻结: string[] | null', () => {
        expectTypeOf(findRoute).returns.toEqualTypeOf();
    });
    it('findRoute 参数口径冻结: (unknown, string, string, string)', () => {
        expectTypeOf(findRoute).parameters.toEqualTypeOf();
    });
    // ── CombatResolver ──────────────────────────────────────────────────────────
    it('CombatResolver.init 返回类型冻结: 战局状态', () => {
        expectTypeOf(CombatResolver.init).returns.toEqualTypeOf();
    });
    it('CombatResolver.step 返回类型冻结: { 战局状态; 回合事件: string[] }', () => {
        expectTypeOf(CombatResolver.step).returns.toEqualTypeOf();
    });
    it('CombatResolver.settle 返回类型冻结: CombatSettleResult', () => {
        expectTypeOf(CombatResolver.settle).returns.toEqualTypeOf();
    });
    // ── 赌局Resolver ────────────────────────────────────────────────────────────
    it('赌局Resolver.resolve 返回类型冻结: CombatSettleResult（同构）', () => {
        expectTypeOf(赌局Resolver.resolve).returns.toEqualTypeOf();
    });
    // ── 离场补结 ────────────────────────────────────────────────────────────────
    it('离场补结.初始化 返回类型冻结: 演化状态', () => {
        expectTypeOf(离场补结.初始化).returns.toEqualTypeOf();
    });
    it('离场补结.推进 返回类型冻结: { 演化状态; 区间事实: unknown[] }', () => {
        expectTypeOf(离场补结.推进).returns.toEqualTypeOf();
    });
...
describe('Task 4A stub: AOHP option_id 稳定键（编译期锁·重渲逐字节恒等）', () => {
    ...
    it('重渲不变量: 相同 ActionOption → option_id 逐字节恒等', () => {
        const opt1 = ActionOptionSchema.parse({ option_id: 'attack::npc_guard::melee', tool_name: 'attack', params: { type: 'melee' }, target_choices: ['npc_guard'] });
        const opt2 = ActionOptionSchema.parse({ option_id: 'attack::npc_guard::melee', tool_name: 'attack', params: { type: 'melee' }, target_choices: ['npc_guard'] });
...
describe('Task 4B stub: irreversible 重放守卫（对撞⑤·未实装）', () => {
    it('replayIrreversible 抛出「未实装」', () => {
        const p = { tick_id: 'tick-1', call_type: 'web_search', frozen_at: 0, output: null };
...
describe('Task 4C stub: 受控接口能力集 [TOOL]（对撞·未实装）', () => {
    const VALID_TYPES = ['code', 'llm', 'roll_dice', 'json_schema', 'trigger', 'output_tag'];
    it('executeToolCapability 抛出「未实装」', () => {
        const d = { type: 'roll_dice' };
...
describe('Task 4D: 互斥组/probability 确定性总纲（禁 Math.random·seeded RNG）', () => {
    ...
    it('pickMutuallyExclusive: 不同 salt → 可能不同（非退化）', () => {
        ...
        for (let s = 0; s < 20; s++)
            results.add(pickMutuallyExclusive(c, 'tick-x', 1, 'ch', s));
