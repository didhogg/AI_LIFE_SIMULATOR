...
import { M3_HARD_EXCLUDED_PREFIXES, M3_FORWARD_ONLY_PATHS, isM3HardExcluded, isM3ForwardOnlyViolation, getM3Violation, } from '../interfaces/patchInvariant.js';
...
describe('M3 dead-defense · forward-only ∩ whitelist 防回归守卫', () => {
    it('M3_FORWARD_ONLY_PATHS 与静态白名单 writable 路径集合交集为空', () => {
        const writablePaths = new Set(deriveWritableWhitelist()
            .filter(e => e.layer === 'writable')
            .map(e => e.path));
        const intersection = M3_FORWARD_ONLY_PATHS.filter(p => writablePaths.has(p));
