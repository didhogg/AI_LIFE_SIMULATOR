...
describe('extensionParams · validateExtensionEntry · 单键类型闸', () => {
    const numDecl = { 类型: '数字', 默认值: 0 };
    const strDecl = { 类型: '字符串', 默认值: '' };
    const boolDecl = { 类型: '布尔', 默认值: false };
...
describe('extensionParams · validateExtensionParams · 批量校验', () => {
    ...
    it('类型全匹配 → 无违规', () => {
        const violations = validateExtensionParams({ 生命值: 50, 昵称: 'Alice', 已激活: true }, template);
    ...
    it('多键类型不匹配 → 报多项违规', () => {
        const violations = validateExtensionParams({ 生命值: '50', 昵称: 42, 已激活: 'yes' }, template);
