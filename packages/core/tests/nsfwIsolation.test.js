...
import { NSFW_BASELINE, CONTENT_RATING_SUBSETS, DEFAULT_NSFW_CONTENT, availableContentRatings, } from '../engine/nsfwContent.js';
...
describe('B-NSFW · availableContentRatings', () => {
    ...
    it('sfw 子集 ⊂ light 子集 ⊂ full 子集（单调递增）', () => {
        ...
        for (const r of sfw)
            expect(light.has(r)).toBe(true);
        for (const r of light)
            expect(full.has(r)).toBe(true);
    });
...
describe('B-NSFW · 隔离防回归：非指定模块无非空 NSFW 默认串', () => {
    ...
    it('prompt/index.ts: 无硬编码 NSFW 内容串（路由逻辑仅用 import 来的常量）', () => {
        ...
        const longChinese = codeLines.filter(l => /['"`][^'"`]{30,}[一-鿿][^'"`]{10,}['"`]/.test(l));
        // 允许 explicitReason 字段的中文注释性文案（这是路由日志，不是 NSFW 内容）
        // 规则：如果超长中文串含「NSFW」且不是路由日志/理由，则标记
        const suspicious = longChinese.filter(l => !l.includes('explicitReason') && !l.includes('//') && l.includes('NSFW'));
    ...
    it('nsfwContent.ts 是 DEFAULT_NSFW_CONTENT 的唯一定义源', () => {
        ...
        for (const [name, src] of [['dollar.ts', dollar], ['memory.ts', memory], ['prompt/index.ts', promptIdx]]) {
