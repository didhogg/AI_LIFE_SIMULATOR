...
import {
  parseExpr, parsePred,
  tryParseExpr, tryParsePred,
  DslParseError,
  DSL_GRAMMAR_VERSION,
  DSL_EVALUATOR_VERSION,
} from '../engine/dsl/parser.js';
...
import type { DslExpr, DslPred } from '../engine/dsl/fuzzer.js';
...
const UNI  = (e: DslExpr): DslExpr => ({ kind: 'unary', op: '-', expr: e });
const CALL = (fn: 'min' | 'max' | 'clamp' | 'pow' | 'sqrt', ...args: DslExpr[]): DslExpr => ({ kind: 'call', fn, args });
const CMP  = (op: '==' | '!=' | '<' | '<=' | '>' | '>=' | 'in', l: DslExpr, r: DslExpr): DslPred => ({ kind: 'compare', op, left: l, right: r });
const LOG  = (op: 'and' | 'or', l: DslPred, r: DslPred): DslPred => ({ kind: 'logical', op, left: l, right: r });
...
describe('S-1 · fixture gate: v1 表达式 parse + eval 逐位恒等', () => {
  ...
  const EXPR_FIXTURES: Array<{ expr: string; expected: number }> = [
  ...
  // 谓词 fixture
  const PRED_FIXTURES: Array<{ pred: string; expected: boolean }> = [
...
describe('K-a · expandVerbTarget DSL 谓词筛选', () => {
  ...
  const CTXS: Record<string, DslContext> = {
    '角色甲': { 属性: { 体质: 80, 魅力: 60 } },
    '角色乙': { 属性: { 体质: 40, 魅力: 90 } },
    '角色丙': { 属性: { 体质: 60, 魅力: 50 } },
  };
  const resolver = (key: string): DslContext | null => CTXS[key] ?? null;
  ...
  it('resolver 返回 null 的实体被跳过', () => {
    const partialResolver = (key: string): DslContext | null =>
      key === '角色甲' ? CTXS['角色甲']! : null;
