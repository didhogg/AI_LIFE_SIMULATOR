...
import { z } from 'zod';
import { 选项集定义条目Schema, 选项集库Schema, 选项集ID正则, 投影选项集库, } from '../schema/optionSetLibrary.js';
...
function mkVerb(verb, overrides = {}) {
    return 动词选项条目Schema.parse({ verb, ...overrides });
}
function mkEntry(name, overrides = {}) {
...
describe('选项集库 · 指纹金测（0 重定基）', () => {
    ...
    it('改投影内容（添加条目） → hashCanonical 变化', () => {
        const lib1 = 选项集库Schema.parse({ opts: { 名称: 'x', 条目: [原始条目[0]] } });
        const lib2 = 选项集库Schema.parse({ opts: { 名称: 'x', 条目: 原始条目 } });
...
describe('选项集库 · resolve 挂载 + by-ID 加载', () => {
    ...
    it('冰箱中 toString 作 handle → 解引用 own-property guard 拦截', () => {
        const 成品 = { 选项集库: { movement_set: mkEntry('移动') } };
        expect(解引用(创建引用('选项集', 'movement_set'), 成品)).toBeDefined();
        expect(解引用({ __ns: '选项集', handle: 'toString' }, 成品)).toBeNull();
    });
});
...
describe('选项集库 · 守恒门', () => {
    ...
    it('命名空间枚举 = 32 項（含选项集）', () => {
        ...
        expect(命名空间枚举.includes('选项集')).toBe(true);
    });
    ...
    it('0 重定基验证：加载选项集库不改 hashPresetFingerprint', () => {
        const lib = { movement: mkEntry('移动', { 条目: [mkVerb('move')] }) };
