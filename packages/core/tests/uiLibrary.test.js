...
import { UI条目Schema, UI库Schema, UI_ID正则, } from '../engine/preset/uiLibrary.js';
...
describe('UI库 · UI条目Schema · 独立 parse', () => {
    ...
    it('配置/渲染位 接受任意 opaque 载荷（不报错·不丢字段）', () => {
        ...
        expect(r.success).toBe(true);
        if (r.success) {
            expect(r.data.配置['color']).toBe('red');
            expect(r.data.配置['flag']).toBe(true);
            expect(r.data.渲染位['hero_image']).toBeNull();
        }
    });
...
describe('UI库 · 四类引用 fail-open（冰箱待建）', () => {
    ...
    it('UI组件 冰箱键存在·条目命中 → 返回 UI条目', () => {
        ...
        expect(result.名称).toBe('panel_main');
    });
    it('UI组件 解引用·UI成品作冰箱·多条目命中', () => {
        ...
        expect(entry.类型).toBe('button');
        expect(entry.配置['label']).toBe('确定');
    });
});
// ═══════════════════════════════════════════════════════════════════
...
describe('UI库 · 安全硬化：原型名 UI_ID/句柄 → null', () => {
    it('UI_ID = constructor → 不作为 own-property 进入 UI成品（BFS own-property guard）', () => {
        ...
        const benign = { btn_ok: mkUI('btn_ok') };
    ...
    it('冰箱中 constructor/__proto__ key → 解引用 own-property guard 拦截', () => {
        ...
        const protoRef = { __ns: 'UI组件', handle: 'toString' };
...
describe('UI库 · 不进判定面指纹', () => {
    ...
    it('UI库 不在 FINGERPRINT_EXCLUDED_FIELDS（未被显式排除但也不进指纹·通过不进输入实现）', () => {
        ...
        const excludedSet = new Set(FINGERPRINT_EXCLUDED_FIELDS);
...
function ui条目ToPackEnvelope(entry) {
