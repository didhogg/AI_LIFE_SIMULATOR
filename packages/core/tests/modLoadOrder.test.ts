...
function mod(
  deps: string[] = [],
  opts: { pack_id?: string; 启用?: boolean; 优先级?: number; 冲突?: string[] } = {},
) {
  return { 依赖: deps, ...opts };
}
...
describe('LoadOrderResult shape', () => {
  it('has required keys', () => {
    type Assert = _Expect<
      Equals<
        keyof LoadOrderResult,
        'flattenedLoadOrder' | 'orderedGroups' | 'rejected' | 'disabled' | 'conflicts' | 'graph'
      >
    >;
...
describe('conflict — mutual declaration', () => {
  it('both enabled → conflict detected', () => {
    ...
    expect(r.conflicts[0]).toEqual({ a: 'a', b: 'b' } satisfies ConflictPair);
  });
...
describe('key insertion order invariance', () => {
  it('reverse key order → same load order', () => {
    const regAB: ModRegistry = { a: mod(['b']), b: mod([]) };
    const regBA: ModRegistry = { b: mod([]), a: mod(['b']) };
...
describe('六禁 — no localeCompare method call in loader', () => {
  it('modGraph.ts must not call .localeCompare()', async () => {
    ...
    const src = await fs.readFile(
      new URL('../loader/modGraph.ts', import.meta.url),
      'utf8',
    );
...
describe('conflict pair canonical form', () => {
  it('a is always codepoint-smaller than b', () => {
    ...
    expect(r.conflicts[0]).toEqual({ a: 'a', b: 'z' } satisfies ConflictPair);
  });
});
