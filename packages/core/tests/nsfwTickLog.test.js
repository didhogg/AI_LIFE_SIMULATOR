...
import { assembleTickRoute, replayRoute, isNsfwScene, selectNarrativeModel, } from '../prompt/index.js';
// ── Helper ────────────────────────────────────────────────────────────────────
function makeState(overrides = {}) {
    const { enabled = false, mode = '失败兜底', targetKey = 'claude-nsfw', rating = 'off', tickId = 'tick-001', } = overrides;
...
describe('N-2 · live 偏好 ⊥ tick_log 冻结路由', () => {
    ...
    it('场景预判历史拍 replay → 切到关态后仍读冻结 nsfw-preempt 路由', () => {
        ...
        const liveOff = {
            ...recorded,
            $玩家偏好: { ...recorded.$玩家偏好, NSFW降级模型: { 启用: false, 触发模式: '失败兜底' } },
        };
