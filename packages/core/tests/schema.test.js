...
import { lore条目Schema, lore知识库Schema, } from '../schema/lore.js';
...
describe('4.1 System layer', () => {
    ...
    it('valid NarrativeSettingSchema with 叙事偏好', () => {
        ...
        expect(res.success).toBe(true);
        expect(res.data.叙事偏好).toBe('偏向宫斗与权谋，少写打斗细节');
    });
...
describe('B5.6 · backfill货币账户PerEntity — _应收/_费用 幂等 + 双机恒等', () => {
    function makePerEntityRaw(hasReceivable) {
        const entity = (cash) => hasReceivable
    ...
    it('一次迁移：缺 _应收 → 逐实体补填 _应收:{}/_费用:{} + migration_version +1', () => {
        ...
        const sys = result['_系统'];
        expect(sys['migration_version']).toBe(6);
    });
    it('二次迁移幂等：含 _应收 时 no-op（migration_version 不再 +1·输出完全相同）', () => {
        const raw = makePerEntityRaw(false);
        const once = backfill货币账户PerEntity(raw);
        const twice = backfill货币账户PerEntity(once);
        const sysOnce = once['_系统']['migration_version'];
        const sysTwice = twice['_系统']['migration_version'];
...
describe('4.8 Memory / Schedule layer', () => {
    ...
    it('落账记录: actor_source 全四值通过', () => {
        const 四值 = ['玩家', '玩家确认', '模型代写', 'NPC自主/系统驱动'];
        for (const v of 四值) {
    ...
    it('mod条目: 轨道 四种合法值均通过', () => {
        for (const t of ['gameplay', 'cosmetic', 'view', 'macro']) {
    ...
    it('intervention_pack_v1: deltas[] 合法条目（op=set/add/sub/clamp/lock 均通过）', () => {
        for (const op of ['set', 'add', 'sub', 'clamp', 'lock']) {
            expect(intervention_pack_v1Schema.safeParse({
                pack_id: 'test_e',
...
describe('blueprint ↔ schema consistency', () => {
    it('RootSchema.shape keys match blueprint 4.0 (diff = zero)', () => {
        ...
        const blueprintKeys = new Set(BLUEPRINT_KEYS);
        ...
        expect(schemaKeys.size).toBe(53); // P0-1 BatchA: +$生图配置 +$语音配置 +$RAG配置; 对撞⑥: +$临时会话; B2·S1: +_mod墓碑库; B5·S1+S1b: +受治理键空间注册表 +键空间归并表
    });
...
describe('6.43 叙事流条目Schema（不进 RootSchema）', () => {
    const 基础条目 = { 序号: 1, 来源: 'AI叙事', 正文: '一行叙事' };
    ...
    it('叙事流: 来源合法枚举全部通过', () => {
        const sources = ['AI叙事', '玩家输入', '引擎系统行', '降级文本'];
    ...
    it('叙事流: strip 姿态——未知字段被剥除不报错', () => {
        ...
        expect(res.success).toBe(true);
        if (res.success)
            expect(res.data['未知字段']).toBeUndefined();
    });
    it('叙事流: 缺 序号 拒收', () => {
...
describe('K4 _mod墓碑库（B2·S1·schema-only）', () => {
    ...
    it('所有合法原因枚举值均通过', () => {
        for (const 原因 of mod墓碑原因枚举) {
            expect(_mod墓碑库Schema.safeParse({ m: { 记录键: 'm', 原因 } }).success, `原因="${原因}" 应通过`).toBe(true);
        }
    });
    it('原因枚举越界拒收', () => {
        expect(_mod墓碑库Schema.safeParse({
...
describe('B-1 lore 知识库 · BLUEPRINT_KEYS 一致性', () => {
    ...
    it('BLUEPRINT_KEYS 与 RootSchema 顶层键完全对齐（一致性检查）', () => {
        ...
        for (const k of schemaKeys) {
            expect(BLUEPRINT_KEYS, `RootSchema.shape 含 "${k}" 但 BLUEPRINT_KEYS 未列`).toContain(k);
        }
    });
...
describe('P0-1 Fix3 · $玩家偏好.内容分级 enum 英文化', () => {
    ...
    it('接受 off / light / explicit / community', () => {
        for (const v of ['off', 'light', 'explicit', 'community']) {
...
describe('6.59 受治理键空间注册表 S1', () => {
    it('空表 {} parse 通过（键条目 absent）', () => {
        ...
        expect(res.success).toBe(true);
        if (res.success)
            expect(res.data.键条目).toBeUndefined();
    });
...
describe('6.59 归并表 S1b（键空间归并表）', () => {
    it('空表 {} parse 通过（归并条目 absent）', () => {
        ...
        expect(res.success).toBe(true);
        if (res.success)
            expect(res.data.归并条目).toBeUndefined();
    });
    it('单条目: 最小合法条目 parse 通过', () => {
...
describe('6.59 母题注册表（schema-only·不进 RootSchema）', () => {
    it('空表 {} parse 通过（母题条目 absent）', () => {
        ...
        expect(res.success).toBe(true);
        if (res.success)
            expect(res.data.母题条目).toBeUndefined();
    });
    it('单条目: 最小合法条目 parse 通过', () => {
...
describe('6.59 地点类别注册表（schema-only·不进 RootSchema·map.ts 零迁移）', () => {
    it('空表 {} parse 通过（地点类别条目 absent）', () => {
        ...
        expect(res.success).toBe(true);
        if (res.success)
            expect(res.data.地点类别条目).toBeUndefined();
    });
    it('单条目: 最小合法条目 parse 通过', () => {
...
describe('B5·S1+S1b · RootSchema 挂载 · 整体可空·零迁移', () => {
    ...
    it('🛡️ 隐性排除：受治理键空间注册表 不在 FINGERPRINT_BUNDLE_MEMBERS', () => {
        expect(FINGERPRINT_BUNDLE_MEMBERS).not.toContain('受治理键空间注册表');
    });
    it('🛡️ 隐性排除：键空间归并表 不在 FINGERPRINT_BUNDLE_MEMBERS', () => {
        expect(FINGERPRINT_BUNDLE_MEMBERS).not.toContain('键空间归并表');
    });
    it('🛡️ 隐性排除：受治理键空间注册表 不在 FINGERPRINT_EXCLUDED_FIELDS', () => {
        expect(FINGERPRINT_EXCLUDED_FIELDS).not.toContain('受治理键空间注册表');
    });
    it('🛡️ 隐性排除：键空间归并表 不在 FINGERPRINT_EXCLUDED_FIELDS', () => {
        expect(FINGERPRINT_EXCLUDED_FIELDS).not.toContain('键空间归并表');
    });
});
...
describe('L-25 · 发育阶段Schema · 跨字段语义 superRefine', () => {
    const makeRace = (stages) => 种族模板Schema.safeParse({ race_human: { 发育阶段表: stages } });
...
describe('L-2b 当时快照 + L-22 来源类型 · 印象条目 optional', () => {
    ...
    const wrap = (imp) => 认知档案Schema.safeParse({ obs: { tgt: { 印象: [imp] } } });
...
describe('L-8 · 二审维度条目 越界类型 enum（Off-Topic/Cheating）', () => {
    it('valid: 无越界类型字段（默认）', () => {
        ...
        expect(res.success).toBe(true);
        if (res.success)
            expect(res.data.越界类型).toBeUndefined();
    });
