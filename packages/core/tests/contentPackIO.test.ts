...
function mkPack(packId: string, extra: Record<string, unknown> = {}) {
  return 内容包条目Schema.parse({ pack_id: packId, 依赖: [], 冲突: [], ...extra });
}

function mkLib(packs: Record<string, unknown>) {
  return 内容包库Schema.parse(packs);
}

function emptyInstalled(): 已安装内容库 {
  return { 内容包库: mkLib({}) };
}
...
describe('PR-5d ② HASH · content_hash 逐位恒等', () => {
  it('HASH-1 导出条目 内容哈希 === computeEffectPackHash 重算值', () => {
    ...
    const exported = r.信封.内容包库['gamma'] as Record<string, unknown>;
    const stored = exported['内容哈希'] as string;
    const recomputed = computeEffectPackHash(
      Object.fromEntries(Object.entries(exported).filter(([k]) => k !== '内容哈希')),
    );
  ...
  it('HASH-2 导出后 内容哈希 是稳定的 8 位 hex 字符串', () => {
    ...
    const h1 = (r1.信封.内容包库['delta'] as Record<string, unknown>)['内容哈希'];
    const h2 = (r2.信封.内容包库['delta'] as Record<string, unknown>)['内容哈希'];
    expect(h1).toBe(h2);
  });
...
describe('PR-5d ③ TOMB · 非法条目进墓碑', () => {
  ...
  it('TOMB-2 条目内容篡改（保持 内容哈希 不变）→ per-entry 检拦截 → 条目进墓碑', () => {
    ...
    const deepCopy = JSON.parse(JSON.stringify(r.信封)) as typeof r.信封;
    const entry = deepCopy.内容包库['eta'] as Record<string, unknown>;
...
describe('PR-5d ④ COLL · 撞库检测', () => {
  ...
  it('COLL-2 diff hash 撞库 → 禁覆盖·进 _mod墓碑库', () => {
    ...
    // 已安装仍是 v1（未被覆盖）
    const existHash = (imp2.已安装.内容包库['lambda'] as Record<string, unknown>)?.['内容哈希'];
    const v1Hash = (imp1.已安装.内容包库['lambda'] as Record<string, unknown>)?.['内容哈希'];
