...
function makeEntry(overrides: Partial<动词选项条目Type> = {}): 动词选项条目Type {
...
describe('② PRESET 指纹：动词选项集哈希 变化 → 指纹变', () => {
  ...
  const BASE_FP_ARGS = {
    ...
    生效中内容包集哈希: '',
    snapshot: {
      难度系数组: {}, 判定骰型: 100 as 100 | 20, 暴击映射: '关' as '关',
      钳制表: {}, 预设数值面域上下界: [],
    },
  } as const;
...
describe('⑥ 子集采样：maxPerTick=N → 返回恰好 N 条', () => {
  const declared: 动词选项条目Type[] = Array.from({ length: 5 }, (_, i) =>
    makeEntry({ verb: '转移', target_choices: [`npc_${i}`], salient_args: `item${i}` }),
  );
