...
describe('P7.a-1 · entity scope (npc_a)', () => {
  ...
  it('属性.体质 = 60', () => {
    expect((ctx['属性'] as Record<string, number>)['体质']).toBe(60);
  });

  it('属性.魅力 = 30', () => {
    expect((ctx['属性'] as Record<string, number>)['魅力']).toBe(30);
  });

  it('技能.格斗 = 3（等级提取）', () => {
    expect((ctx['技能'] as Record<string, number>)['格斗']).toBe(3);
  });

  it('技能.医术 = 7', () => {
    expect((ctx['技能'] as Record<string, number>)['医术']).toBe(7);
  });

  it('账户.文 = 500', () => {
    expect((ctx['账户'] as Record<string, number>)['文']).toBe(500);
  });

  it('账户.金 = 2', () => {
    expect((ctx['账户'] as Record<string, number>)['金']).toBe(2);
  });

  it('全局.拍计数 = 10', () => {
    expect((ctx['全局'] as Record<string, number>)['拍计数']).toBe(10);
  });

  it('全局.纪元分钟 = 1440', () => {
    expect((ctx['全局'] as Record<string, number>)['纪元分钟']).toBe(1440);
  });
});
