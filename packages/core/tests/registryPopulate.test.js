...
import { checkS6UnregisteredHandlerRefs, checkGoverneRegistryMembership, checkMotifRegistration, checkDisabledRuleKeyRefs, checkPackIdAliases, } from '../migration/migrate.js';
...
describe('populateGoverneKeyRegistry — G-b populate', () => {
    it('A1: empty mod registry → existing registry unchanged (same ref)', () => {
        ...
        const result = populateGoverneKeyRegistry({}, reg);
        expect(result).toBe(reg); // same reference = fast path
    });
    it('IM3-D1: enabled mod pack_id auto-enrolled in mod包 namespace', () => {
        const result = populateGoverneKeyRegistry({ my_mod: makeMod('my_mod') }, emptyRegistry());
    ...
    it('IM3-D1: disabled mod NOT enrolled', () => {
        const result = populateGoverneKeyRegistry({ off_mod: makeMod('off_mod', { 启用: false }) }, emptyRegistry());
    ...
    it('A1: explicit 来源包 in 命名空间键声明 preserved', () => {
        const result = populateGoverneKeyRegistry({
            my_mod: makeMod('my_mod', {
                命名空间键声明: [
                    { 规范键: 'coin_x', 命名空间: '币种', 来源包: 'other_mod' },
                ],
            }),
        }, emptyRegistry());
    ...
    it('existing hand-crafted entries take priority over mod-derived (same key)', () => {
        ...
        const result = populateGoverneKeyRegistry({
            mod_a: makeMod('mod_a', {
                命名空间键声明: [{ 规范键: 'gold', 命名空间: '币种', 来源包: 'mod_a' }],
            }),
        }, existing);
        const entries = result.键条目 ?? [];
        ...
        expect(goldEntries[0]['不可变']).toBe(true);
    });
});
...
describe('populateGoverneKeyRegistry — G-c conflict resolution', () => {
    it('higher 优先级 wins when same 规范键+命名空间 claimed by two mods', () => {
        const result = populateGoverneKeyRegistry({
            low_prio: makeMod('low_prio', {
                优先级: 0,
                命名空间键声明: [{ 规范键: 'rare_gem', 命名空间: '稀有度', 来源包: 'low_prio' }],
            }),
            high_prio: makeMod('high_prio', {
                优先级: 10,
                命名空间键声明: [{ 规范键: 'rare_gem', 命名空间: '稀有度', 来源包: 'high_prio' }],
            }),
        }, emptyRegistry());
    ...
    it('equal 优先级 → lower pack_id (codepoint asc) wins', () => {
        const result = populateGoverneKeyRegistry({
            mod_b: makeMod('mod_b', {
                优先级: 5,
                命名空间键声明: [{ 规范键: 'thing', 命名空间: '特质子类', 来源包: 'mod_b' }],
            }),
            mod_a: makeMod('mod_a', {
                优先级: 5,
                命名空间键声明: [{ 规范键: 'thing', 命名空间: '特质子类', 来源包: 'mod_a' }],
            }),
        }, emptyRegistry());
...
describe('populateGoverneKeyRegistry — A4 determinism', () => {
    it('same input → same output (bit-identical)', () => {
        const mods = {
            bravo: makeMod('bravo', {
                命名空间键声明: [{ 规范键: 'item_b', 命名空间: '特质子类' }],
            }),
            alpha: makeMod('alpha', {
                命名空间键声明: [{ 规范键: 'item_a', 命名空间: '特质子类' }],
            }),
        };
        const r1 = populateGoverneKeyRegistry(mods, emptyRegistry());
        const r2 = populateGoverneKeyRegistry(mods, emptyRegistry());
...
describe('checkS6UnregisteredHandlerRefs — S6 fail-open', () => {
    ...
    it('A3: registry has sideEffect句柄 but state has no lore → no logs', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'combat_start', 命名空间: 'sideEffect句柄' }],
            },
        });
        const log = [];
        checkS6UnregisteredHandlerRefs(state, log);
    ...
    it('A3: unregistered side_effects handler in lore → warn log', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'known_handler', 命名空间: 'sideEffect句柄' }],
            },
    ...
    it('A3: unregistered cascade_on_change handler → warn log', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'known_cascade', 命名空间: 'cascade句柄' }],
            },
            _lore知识库: {
    ...
    it('A3: unregistered 解除通道 handler → warn log', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'legit_interceptor', 命名空间: '拦截器句柄' }],
            },
            _lore知识库: {
    ...
    it('A3: 停用 registry entry counts as absent → unregistered warn for that key', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [
                    { 规范键: 'disabled_h', 命名空间: 'sideEffect句柄', 停用: true },
                    { 规范键: 'active_h', 命名空间: 'sideEffect句柄' },
                ],
            },
            _lore知识库: {
    ...
    it('A3: nested lore structure → recursively scanned', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'known_h', 命名空间: 'sideEffect句柄' }],
            },
            _lore知识库: {
                entry1: {
                    nested: {
...
describe('populateGoverneKeyRegistry — C1 G-c conflict logging', () => {
    it('C1: no conflict → onConflict never called', () => {
        ...
        populateGoverneKeyRegistry({
            a_mod: makeMod('a_mod', {
                命名空间键声明: [{ 规范键: 'gold', 命名空间: '币种' }],
            }),
            b_mod: makeMod('b_mod', {
                命名空间键声明: [{ 规范键: 'silver', 命名空间: '币种' }],
            }),
        }, emptyRegistry(), (key, ns, winner, loser) => conflicts.push({ key, ns, winner, loser }));
    ...
    it('C1: same key+ns claimed by two mods → onConflict called for loser', () => {
        ...
        populateGoverneKeyRegistry({
            ...
            a_mod: makeMod('a_mod', {
                优先级: 0,
                命名空间键声明: [{ 规范键: 'gold', 命名空间: '币种', 来源包: 'a_mod' }],
            }),
            b_mod: makeMod('b_mod', {
                优先级: 10,
                命名空间键声明: [{ 规范键: 'gold', 命名空间: '币种', 来源包: 'b_mod' }],
            }),
        }, emptyRegistry(), (key, ns, winner, loser) => conflicts.push({ key, ns, winner, loser }));
    ...
    it('C1: equal 优先级 tiebreaker → codepoint lower pack_id wins', () => {
        ...
        populateGoverneKeyRegistry({
            z_mod: makeMod('z_mod', {
                优先级: 5,
                命名空间键声明: [{ 规范键: 'gem', 命名空间: '稀有度', 来源包: 'z_mod' }],
            }),
            a_mod: makeMod('a_mod', {
                优先级: 5,
                命名空间键声明: [{ 规范键: 'gem', 命名空间: '稀有度', 来源包: 'a_mod' }],
            }),
        }, emptyRegistry(), (key, ns, winner, loser) => conflicts.push({ key, ns, winner, loser }));
        expect(conflicts).toHaveLength(1);
    ...
    it('C1: hand-crafted entry wins silently (no conflict log for hand-crafted wins)', () => {
        const conflicts = [];
        const existing = {
            键条目: [{ 规范键: 'gold', 命名空间: '币种', 来源包: 'author' }],
        };
        populateGoverneKeyRegistry({
            mod_a: makeMod('mod_a', {
                命名空间键声明: [{ 规范键: 'gold', 命名空间: '币种' }],
            }),
        }, existing, (key, ns, winner, loser) => conflicts.push({ key, ns, winner, loser }));
    ...
    it('C1: conflict log is deterministic (same input → same conflicts)', () => {
        const mods = {
            z_mod: makeMod('z_mod', {
                优先级: 0,
                命名空间键声明: [{ 规范键: 'coin', 命名空间: '币种', 来源包: 'z_mod' }],
            }),
            a_mod: makeMod('a_mod', {
                优先级: 0,
                命名空间键声明: [{ 规范键: 'coin', 命名空间: '币种', 来源包: 'a_mod' }],
            }),
        };
        const c1 = [];
        populateGoverneKeyRegistry(mods, emptyRegistry(), (k, n, w, l) => c1.push({ key: k, ns: n, winner: w, loser: l }));
        const c2 = [];
        populateGoverneKeyRegistry(mods, emptyRegistry(), (k, n, w, l) => c2.push({ key: k, ns: n, winner: w, loser: l }));
...
describe('checkDisabledRuleKeyRefs — C3/C4 G-e', () => {
    it('C3: no disabled non-handler registry entries → fast exit, no logs', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'gold', 命名空间: '币种' }], // active, not disabled
            },
        });
        const log = [];
    ...
    it('C3: disabled key NOT referenced in lore → no warn', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'old_coin', 命名空间: '币种', 停用: true }],
            },
            _lore知识库: {
    ...
    it('C3: disabled key referenced as string value in lore → warn', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'old_status', 命名空间: '状态子类', 停用: true }],
            },
            _lore知识库: {
    ...
    it('C3: handler namespace (sideEffect句柄/cascade句柄/拦截器句柄) disabled → NOT scanned by G-e (S6 handles)', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'old_handler', 命名空间: 'sideEffect句柄', 停用: true }],
            },
            _lore知识库: {
    ...
    it('C3: disabled key in array in lore → warn for each occurrence', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'deprecated_trait', 命名空间: '特质子类', 停用: true }],
            },
            _lore知识库: {
    ...
    it('C4: deterministic: same input → same log output', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [
                    { 规范键: 'a_dep', 命名空间: '状态子类', 停用: true },
                    { 规范键: 'z_dep', 命名空间: '状态子类', 停用: true },
                ],
            },
            _lore知识库: {
                rule_z: { ref: 'z_dep' },
    ...
    it('C4: fail-open = never throws', () => {
        const state = makeState({
            受治理键空间注册表: {
                键条目: [{ 规范键: 'dep_key', 命名空间: '稀有度', 停用: true }],
            },
            _lore知识库: null,
        });
        const log = [];
...
describe('checkPackIdAliases — D-2 散落别名', () => {
    it('D-2: no mod包 registry entries → fast exit, no logs', () => {
        const state = makeState({
            受治理键空间注册表: { 键条目: [{ 规范键: 'gold', 命名空间: '币种' }] },
    ...
    it('D-2: 延时种子.来源.包id 空串哨兵 → skip, no warn', () => {
        const state = makeState({
            受治理键空间注册表: { 键条目: [{ 规范键: 'my_mod', 命名空间: 'mod包' }] },
    ...
    it('D-2: 行动卡库._来源包 已注册 → no warn', () => {
        const state = makeState({
            受治理键空间注册表: { 键条目: [{ 规范键: 'core_base', 命名空间: 'mod包' }] },
    ...
    it('D-2: deterministic — sorted key order, same input → same log', () => {
        const state = makeState({
            受治理键空间注册表: { 键条目: [{ 规范键: 'known', 命名空间: 'mod包' }] },
            $隐藏记忆库: {
                延时种子: {
    ...
    it('D-2: fail-open = never throws (null state fields)', () => {
        const state = makeState({
            受治理键空间注册表: { 键条目: [{ 规范键: 'pkg', 命名空间: 'mod包' }] },
            $隐藏记忆库: null,
            行动卡库: null,
        });
        const log = [];
