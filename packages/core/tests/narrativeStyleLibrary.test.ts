...
import {
  ...
  文风冰箱Schema,
  文风ID正则,
} from '../schema/narrativeStyleLibrary.js';
...
function mkStyle(name: string, overrides: Partial<文风定义条目Type> = {}): 文风定义条目Type {
  return { 名称: name, ...overrides };
}

function 条目ToPackEnvelope(entry: 文风定义条目Type): Record<string, unknown> {
...
describe('文风库 · 文风定义条目Schema · 独立 parse', () => {
  it('最小条目（仅 名称）→ parse 成功', () => {
    ...
    if (r.success) expect(r.data.名称).toBe('古典文言风');
  });
  it('缺 名称 → parse 失败', () => {
...
describe('文风库 · 渲染载荷 opaque', () => {
  it('渲染载荷 接受任意 opaque 载荷', () => {
    ...
    expect(r.success).toBe(true);
    if (r.success) {
      expect((r.data.渲染载荷 as Record<string, unknown>)['风格提示词']).toBe('以文言文叙述');
    }
  });
  it('渲染载荷 空 record → 通过', () => {
...
describe('文风库 · resolve 挂载 + by-ID 加载', () => {
  ...
  it('resolve manifest.文风 → 文风成品 含引用条目', () => {
    ...
    expect((r.文风成品['classical_style']?.渲染载荷 as Record<string, unknown>)?.['提示词']).toBe('古典');
  });
  it('多条目引用 → 文风成品 含全部命中', () => {
  ...
  it('冰箱中 toString 作 handle → 解引用 own-property guard 拦截', () => {
    const 成品: Record<string, unknown> = { 文风库: { classical_style: mkStyle('文言风') } };
    expect(解引用(创建引用('文风', 'classical_style'), 成品)).toBeDefined();
    expect(解引用({ __ns: '文风' as const, handle: 'toString' }, 成品)).toBeNull();
  });
});
...
describe('文风库 · 不进判定面指纹', () => {
  ...
  it('改渲染载荷 → hashPresetFingerprint 逐位恒等', () => {
    const lib1: 文风冰箱Type = { style: mkStyle('x', { 渲染载荷: { 提示词: 'A' } }) };
    const lib2: 文风冰箱Type = { style: mkStyle('x', { 渲染载荷: { 提示词: 'B', 禁词: ['xxx'] } }) };
...
describe('文风库 · 守恒门', () => {
  ...
  it('manifest 四组总长 = 87', () => {
    expect(FINGERPRINT_BUNDLE_MEMBERS.length + FINGERPRINT_PRESET_FIELDS.length + FINGERPRINT_SNAPSHOT_FIELDS.length + FINGERPRINT_EXCLUDED_FIELDS.length).toBe(88);
  });
  it('命名空间枚举 = 32 項（含文风）', () => {
    expect(命名空间枚举.length).toBe(32);
    expect((命名空间枚举 as readonly string[]).includes('文风')).toBe(true);
  });
  it('冰箱绑定表含 文风·解析器键 = 文风库', () => {
    expect(冰箱绑定表['文风'].解析器键).toBe('文风库');
  });
  it('0 重定基验证', () => {
    const lib: 文风冰箱Type = { classical: mkStyle('文言风', { 渲染载荷: { 提示词: '古典' } }) };
