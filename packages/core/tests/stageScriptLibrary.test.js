...
import { 小剧场剧本定义条目Schema, 小剧场剧本库Schema, 小剧场剧本ID正则, } from '../schema/stageScriptLibrary.js';
...
function mkScript(name, overrides = {}) {
...
describe('小剧场剧本库 · resolve 挂载 + by-ID 加载', () => {
    ...
    it('resolve manifest.小剧场剧本 → 小剧场剧本成品 含引用条目', () => {
        ...
        expect(r.小剧场剧本成品['tavern_encounter']?.渲染载荷?.['分类']).toBe('社交');
    });
    ...
    it('冰箱中 toString 作 handle → 解引用 own-property guard 拦截', () => {
        const 成品 = { 小剧场剧本库: { tavern_encounter: mkScript('酒馆偶遇') } };
        expect(解引用(创建引用('小剧场剧本', 'tavern_encounter'), 成品)).toBeDefined();
        expect(解引用({ __ns: '小剧场剧本', handle: 'toString' }, 成品)).toBeNull();
    });
});
describe('小剧场剧本库 · 不进判定面指纹', () => {
    ...
    it('改渲染载荷 → hashPresetFingerprint 逐位恒等', () => {
        const lib1 = { script: mkScript('x', { 渲染载荷: { 提示词: 'A' } }) };
        const lib2 = { script: mkScript('x', { 渲染载荷: { 提示词: 'B', 额外: true } }) };
...
describe('小剧场剧本库 · 守恒门', () => {
    ...
    it('命名空间枚举 = 32 項（含小剧场剧本）', () => {
        ...
        expect(命名空间枚举.includes('小剧场剧本')).toBe(true);
    });
    it('冰箱绑定表含 小剧场剧本·解析器键 = 小剧场剧本库', () => {
        expect(冰箱绑定表['小剧场剧本'].解析器键).toBe('小剧场剧本库');
    });
    it('0 重定基验证', () => {
        const lib = { tavern: mkScript('酒馆偶遇', { 渲染载荷: { 分类: '社交' } }) };
