...
describe('D-8: 指纹分线守卫', () => {
  ...
  it('漂移绑定策略 在 EXCLUDED_FIELDS（单条·子字段 触发谓词/监测轴/触发阈值 覆盖其中）', () => {
    expect(FINGERPRINT_EXCLUDED_FIELDS).toContain('漂移绑定策略');
  });
...
describe('D-10: 守恒门', () => {
  ...
  it('manifest 总长 = 96（BUNDLE28+PRESET10+SNAPSHOT5+EXCLUDED53·漂移绑定策略+模块绑定策略各单条）', () => {
