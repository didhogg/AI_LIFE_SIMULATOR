...
describe('B4 · 聚合生效中内容包集哈希 · 忽略 content_hash 以外字段', () => {
  it('其他字段不影响聚合结果', () => {
    const h1 = 聚合生效中内容包集哈希([{ content_hash: 'aa', pack_id: 'foo' } as { content_hash?: string }]);
    const h2 = 聚合生效中内容包集哈希([{ content_hash: 'aa', pack_id: 'bar' } as { content_hash?: string }]);
    expect(h1).toBe(h2);
  });
...
describe('B4 · computeEffectPackHash · content_hash 字段排除', () => {
  ...
  it('无 content_hash 与有 content_hash 结果相同', () => {
    expect(computeEffectPackHash(BASE)).toBe(
      computeEffectPackHash({ ...BASE, content_hash: 'anything' }),
    );
  });
