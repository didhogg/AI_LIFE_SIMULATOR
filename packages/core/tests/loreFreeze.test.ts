...
import {
  ...
  freezeLoreTransitionPredicate,
  assertLoreTransitionPredicateFrozen,
  freezeLoreConstraintPredicate,
  assertLoreConstraintPredicateFrozen,
  type FreezableLoreEntry,
  type FreezableLoreEntryFull,
...
describe('D-a-lore: assertLoreTransitionPredicateFrozen (1b)', () => {
  it('已冻结 → 不抛', () => {
    const e = { 触发条件: '场景.地域 == 四川', 触发条件_冻结: true as const };
...
describe('D-a-lore: assertLoreConstraintPredicateFrozen (1c)', () => {
  it('已冻结 → 不抛', () => {
    const e = { 禁令谓词: '角色.年龄 < 18', 禁令谓词_冻结: true as const };
...
describe('D-a-lore: collectLorePredicates (1a+1b+1c 三类聚合)', () => {
  function mkFullEntry(
    mainPred: string,
    transitions: Array<{ pred: string }> = [],
    constraints: Array<{ pred: string }> = [],
  ): FreezableLoreEntryFull {
    return {
  ...
  it('未冻结状态转移 → assertLoreTransitionPredicateFrozen 守卫 throw', () => {
    const bag: Record<string, FreezableLoreEntryFull> = {
      'hanfu:盘扣': {
        ...freezeLorePredicate({}, ''),
        状态转移: [{ 触发条件: '场景.地域 == 苏州' }],
      },
