...
function assertDefaultAbsent(schema, knownDefaultKey) {
    const view = 种子视图(schema);
    const result = view.parse({});
    expect(result).not.toHaveProperty(knownDefaultKey);
}
// ── 工具：fixture 子集逐位恒等 ───────────────────────────────────────────────
function assertSubsetPassthrough(schema, subset) {
    const view = 种子视图(schema);
    const result = view.parse(subset);
...
describe('种子视图 · 组织实体条目Schema', () => {
    ...
    it('③ 用工.岗位 record 递归下穿', () => {
        const r = view.parse({ 用工: { 岗位: { 厨师: { 人数: 3 } } } });
        const 岗位 = r['用工']['岗位'];
        expect(岗位['厨师']).toMatchObject({ 人数: 3 });
        expect(Object.keys(岗位['厨师'])).toEqual(['人数']);
    });
...
describe('种子视图 · 地点条目Schema', () => {
    ...
    it('③ 产出 object 递归下穿', () => {
        ...
        const r = view.parse({ 产出: { 农业: 80 } });
        expect(r['产出']).toMatchObject({ 农业: 80 });
        expect(Object.keys(r['产出'])).toEqual(['农业']);
    });
...
describe('种子视图 · factFragment种子条目Schema', () => {
    ...
    it('③ 全 optional（无 default 注入·仅提供字段返回）', () => {
        const r = view.parse({ 主体: 'npc_wang', 维度: '关系', Δ方向: 1 });
...
describe('种子视图 · 离场演化契约Schema', () => {
    ...
    it('③ 关联声明 array 下穿（optional array → 提供时保留元素）', () => {
        const r = view.parse({ 关联声明: ['信号A', '信号B'] });
...
describe('种子视图 · ZodLazy 自引用专项', () => {
    ...
    const TreeSchema = z.lazy(() => z.object({
        值: z.string().default(''),
        子节点: z.array(TreeSchema).optional(),
    }));
    ...
    it('⑥-5 discriminatedUnion 种子视图保留判别键·其余 optional', () => {
        ...
        const view = 种子视图(DU);
        const rA = view.parse({ 类型: 'A' });
        expect(rA['类型']).toBe('A');
        expect(rA).not.toHaveProperty('字段'); // default 剥除 + optional
        const rB = view.parse({ 类型: 'B', 数值: 42 });
        expect(rB['类型']).toBe('B');
