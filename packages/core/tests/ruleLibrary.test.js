...
import { 规则元数据Schema, 规则面Schema, 规则条目Schema, 规则库Schema, 规则面键集, } from '../engine/preset/ruleLibrary.js';
...
// ── 最小合法规则条目 fixture ──────────────────────────────────────────────────
function mkRule(ruleId, overrides = {}) {
    return {
        rule_id: ruleId,
        版本: '1.0.0',
        依赖: [],
...
describe('底座-2b · resolve() · 规则库路径接线', () => {
    ...
    it('规则 semver 不兼容 → _规则墓碑库', () => {
        const ruleLib = {
            rule_a: mkRule('rule_a', { 基底契约: '>=9.9.9' }),
        };
        const result = resolve({ packs: [], rules: ['rule_a'] }, {}, ruleLib);
    ...
    it('规则自环 → _规则墓碑库 自环', () => {
        const ruleLib = {
            rule_a: mkRule('rule_a', { 依赖: ['rule_a'] }),
        };
        const result = resolve({ packs: [], rules: ['rule_a'] }, {}, ruleLib);
...
describe('底座-2b · 双轨等价 · 逐表验收', () => {
    function mkSingleRuleLib(ruleId, faceData) {
        return {
            [ruleId]: mkRule(ruleId, { 规则面: faceData }),
        };
    }
    it('难度系数组：规则库路径 == 旧轨叠加()', () => {
...
describe('底座-2b · 引用顺序语义', () => {
    it('A→B 与 B→A 顺序不同 → 规则成品不同（后载覆盖先载）', () => {
        const ruleLib = {
            rule_a: mkRule('rule_a', { 规则面: { 难度系数组: { 基础成功率调整: 10 } } }),
            rule_b: mkRule('rule_b', { 规则面: { 难度系数组: { 基础成功率调整: -10 } } }),
        };
        const ab = resolve({ packs: [], rules: ['rule_a', 'rule_b'] }, {}, ruleLib);
        const ba = resolve({ packs: [], rules: ['rule_b', 'rule_a'] }, {}, ruleLib);
        const abVal = ab.规则成品['难度系数组']?.['基础成功率调整'];
        const baVal = ba.规则成品['难度系数组']?.['基础成功率调整'];
...
describe('底座-2b · 指纹 0 漂移', () => {
    it('含规则的 resolve 不影响 hashPresetFingerprint（规则库不进指纹）', () => {
        const ruleLib = {
            rule_a: mkRule('rule_a', { 规则面: { 难度系数组: { 基础成功率调整: 99 } } }),
        };
...
describe('底座-2b · 审计债 · 规则元数据字段结构', () => {
    ...
    it('规则库轻轨 + 可写键 → _规则墓碑库（审计一致）', () => {
        const ruleLib = {
            rule_a: mkRule('rule_a', { 轨道: 'cosmetic', 可写键: ['foo'] }),
        };
        const result = resolve({ packs: [], rules: ['rule_a'] }, {}, ruleLib);
