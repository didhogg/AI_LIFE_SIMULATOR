...
import { 实体模板定义条目Schema, 实体模板冰箱Schema, 实体模板ID正则, } from '../schema/entityTemplateLibrary.js';
...
function mkEntityTpl(name, overrides = {}) {
...
describe('实体模板库 · 实体模板定义条目Schema · 独立 parse', () => {
    ...
    it('全字段条目 parse 通过', () => {
        ...
        expect(r.success).toBe(true);
        if (r.success) {
            expect(r.data.NPC模板.length).toBe(1);
            expect(r.data.组织模板.length).toBe(1);
        }
    });
...
describe('实体模板库 · resolve 挂载 + by-ID 加载', () => {
    ...
    it('冰箱中 toString 作 handle → 解引用 own-property guard 拦截', () => {
        const 成品 = { 实体模板库: { imperial_npcs: mkEntityTpl('帝国NPC集') } };
        expect(解引用(创建引用('实体模板', 'imperial_npcs'), 成品)).toBeDefined();
        expect(解引用({ __ns: '实体模板', handle: 'toString' }, 成品)).toBeNull();
    });
});
describe('实体模板库 · 不进判定面指纹', () => {
    ...
    it('改实体模板条目 → hashPresetFingerprint 逐位恒等', () => {
        const lib1 = { tpl: mkEntityTpl('A', { NPC模板: [{ 姓名: '甲' }] }) };
        const lib2 = { tpl: mkEntityTpl('A', { NPC模板: [{ 姓名: '乙' }, { 姓名: '丙' }] }) };
...
describe('实体模板库 · 守恒门', () => {
    ...
    it('命名空间枚举 = 32 項（含实体模板）', () => {
        ...
        expect(命名空间枚举.includes('实体模板')).toBe(true);
    });
    it('冰箱绑定表含 实体模板·解析器键 = 实体模板库', () => {
        expect(冰箱绑定表['实体模板'].解析器键).toBe('实体模板库');
    });
    it('0 重定基验证', () => {
        const lib = { tpl: mkEntityTpl('帝国NPC集', { NPC模板: [{ 姓名: '王大人' }] }) };
